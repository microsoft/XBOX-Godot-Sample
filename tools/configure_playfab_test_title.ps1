...
$liveTestCommand = if ($SkipCustomIdAccount) {
    "pwsh -NoLogo -NoProfile -ExecutionPolicy Bypass -File .\tools\run_all_tests.ps1 -Hosts tests\godot\playfab -PlayFabTitleId `"$TitleId`""
} else {
    "pwsh -NoLogo -NoProfile -ExecutionPolicy Bypass -File .\tools\run_all_tests.ps1 -Hosts tests\godot\playfab -PlayFabTitleId `"$TitleId`" -PlayFabCustomId `"$CustomId`""
}
if (-not $SkipMultiplayerMatchmakingQueue) {
