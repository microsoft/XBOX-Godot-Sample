...
`addons\godot_gdk_editortools\` is the editor tooling for PC GDK packaging:
...
under `addons\godot_gdk_editortools\`. There is no new PowerShell wrapper
...
addons\godot_gdk_editortools\
  ...
  core\                                  # headless-safe; no EditorInterface, no Control
    ...
    editortools_config.gd                  # settings resolver (CLI > .cfg > MSGame.config > project)
    editortools_service.gd                 # verb facade (one run_* method per verb)
    editortools_result.gd                  # typed Dictionary builder + exit-code constants
    ...
    editortools_content_preparer.gd        # copies config + logos into content dir
    wdapp_manager.gd                     # wraps wdapp register/install/launch/terminate/uninstall
    editortools_settings_store.gd          # reads/writes res://.gdk_packaging.cfg
    export_preset_catalog.gd             # enumerates Windows export presets
  editor\                                # UI-only — preloads from core\
    gdk_editortools_plugin.gd              # EditorPlugin entry; top-level GDK menu
...
- `core\editortools_cli.gd` is **pure**: argv -> structured dict. No file or
  process IO. Owns the declarative `VERBS` table that is the single source
  of truth for the verb-flag matrix.
- `core\editortools_config.gd` collapses CLI overrides + the .gdk_packaging.cfg
...
- `core\editortools_service.gd` is the verb **facade**: `run_pack`, `run_genmap`,
...
Every verb returns a `EditorToolsResult` dictionary:
...
Exit-code categories live in `core/editortools_result.gd`:
...
`settings_path` exists only on `EditorToolsConfig.resolve(...)` as a resolver/test
...
`editortools_config.resolve(cli_options, project_root="", settings_path=..., config_path_override="")`
...
1. CLI flags (kebab-case in argv -> snake_case in the resolved dict via
   `_CLI_KEY_REMAP`).
2. `res://.gdk_packaging.cfg` (via `EditorToolsSettingsStore`). Empty strings
...
`EditorToolsContentPreparer.ensure_content_dir_ready()` validates logo
...
The editor plugin (`editor\gdk_editortools_plugin.gd`) drives this automatically:
...
`tests\godot\gdk\tests\editortools\test_game_config_xml_rewriting.gd`.
...
2. Add `core\editortools_result.gd` (typed dict + exit-code constants).
...
9. Keep the spec, user-facing reference (`docs\editortools\plugin.md`), editor
   menu reference (`docs\editortools\editor-menu.md`), and path-scoped
...
- **Headless workflow**: shipped in branch `packaging-headless-revamp`. All
  ...
  `editortools_result.gd`, `editortools_cli.gd`, `editortools_config.gd`,
  `editortools_service.gd`, `run.gd`, `gdkpkg.cmd`, and `gdkpkg.sh` shipped with
  that work. GUT coverage under `tests\godot\gdk\tests\editortools\` covers the
