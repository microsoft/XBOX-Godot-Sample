...
| `invalid_response` | `create_match_ticket_async` returned `ok` with a null ticket or an empty `ticket_id`. The addon contract is that `create_match_ticket_async` resolves only after the SDK assigns a non-empty `ticket_id`, so this indicates a regression below the test-client helper. |
