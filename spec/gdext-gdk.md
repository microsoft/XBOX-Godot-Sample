...
- Native `uint64` configuration values and statistics counters do not round-trip through GDScript's signed `int`. Convert them with the same `to_variant_u64()` clamping helper used by `GDK.package` (`UINT64_MAX` → `-1`, anything else above `INT64_MAX` → `INT64_MAX`) rather than an unchecked `static_cast`, and have `query_configuration_setting()` report a companion `unlimited: bool` as the authoritative flag for the `UINT64_MAX` case.
