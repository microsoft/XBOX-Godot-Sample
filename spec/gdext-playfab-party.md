...
- The title owns the entire establishment deadline, including guest handshake waiting; the addon does not add a handshake timer or automatic retries. A host reply-send failure reports a host-local error and does not register that guest, but cannot settle the remote guest's pending operation. Its title must enforce the deadline and safely cancel an unreturned join through scoped shutdown, which affects all networks on that Party service.
