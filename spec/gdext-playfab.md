...
`PFGameSaveFilesInitialize` requires a packaged GDK title identity, which custom-ID / unpackaged sessions and unpackaged CI runners do not have (it returns `E_INVALIDARG`). `PlayFab.initialize()` treats **only `E_INVALIDARG`** as **non-fatal**: it prints an informational line (via `UtilityFunctions::print`, deliberately not the engine warning/error system, so GUT does not fail tests on this expected condition), leaves Game Saves unavailable, and keeps PlayFab Core/Services running so the rest of the runtime stays usable. Any other failure HRESULT is treated as a genuine fault (e.g. on a packaged title where Game Saves should be available): `initialize()` tears down Services and Core and returns a failed `PlayFabResult` (`playfab_game_save_initialize_failed`) rather than masking the error. Because Game Saves methods first require an Xbox-backed local user handle, non-Xbox callers still receive `xbox_user_required` in this degraded state rather than reaching the uninitialized Game Saves provider.
...
`create_match_ticket_async` resolves only after the SDK assigns a non-empty `ticket_id` to the returned `PlayFabMatchTicket`; half-created local handles remain internal. `PlayFabMatchmakingTicketConfig.members` means local signed-in users, while `members_to_match_with` carries validated remote premade entity keys. `join_match_ticket_async` joins those remote users to the existing ticket through the entity-handle API. Ticket statuses remain raw integers exposed through `STATUS_*` constants, and titles reconcile the cached status immediately and on every state change because the signal stream is not lossless.

Match tickets report `match_id` and `arranged_lobby_connection_string` through `PlayFabMatchTicketStateChange`; title code decides whether to call `join_arranged_lobby_async(...)`. The addon does not automatically join arranged lobbies. `PlayFabLobbyJoinConfig` initializes a newly created arranged lobby's capacity, access policy, owner migration policy, and PlayFab Lobby owner-only invitation restriction; later joiners do not reconfigure it. The canonical defaults, validation, and compile-time invitation gate are documented on `PlayFabLobbyJoinConfig`.
...
- Premade-group matchmaking (#171): ✅ implemented — ticket creation accepts validated remote `members_to_match_with` entity keys, remote clients can join by ticket id with local entity handles and attributes, raw status constants are public, and ticket consumers use a level-triggered cached-status contract.
- Arranged-lobby initialization configuration (#177): ✅ implemented — `PlayFabLobbyJoinConfig` forwards presence-tracked capacity, access policy, owner migration policy, and owner-only PlayFab Lobby invitation restriction. Older GDK builds preserve omitted/false behavior and reject only an explicit unsupported `true`.
