...
- `tools\run_all_tests.ps1 -Hosts tests\godot\playfab -PlayFabTitleId "10D176" -PlayFabCustomId "godot-gdk-ext-live-smoke" -PlayFabMatchmakingQueue "godot_gdk_ext_live_smoke_queue"` also runs the PlayFab Multiplayer multi-client orchestrator across the C1 P0/P1 lobby, matchmaking, arranged-lobby, Party, and end-to-end scenarios. The live-write tier (which persists leaderboard entries and other title-side state) runs unconditionally; always supply a dedicated sandbox title id.
...
- MP-test-automation: ✅ complete in the `feat/mpta-c5-c6-finalize` line. The C1 P0/P1 matrix is represented as one `mp_orchestrator` scenario file per scenario, P2/P3 remain deferred, and `tools\run_all_tests.ps1` is wired to run the P0/P1 set as part of the always-on live tier.
