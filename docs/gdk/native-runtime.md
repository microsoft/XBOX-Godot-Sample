...
`XboxRuntime` is the native core behind the root singleton.
...
## XBOX services scaffold: `XboxServices`

`XboxServices` is the shared native helper for features that sit on top of XBOX services rather than the raw game runtime.
...
- `XboxResult` — normalized result payload
- `XboxPendingSignal` — internal completion emitter retained until a request resolves
- `XboxSignalXAsyncContext` — internal base class for XAsync-backed signal-returning work
...
`XboxUsers` is the first service implemented on top of the shared runtime.
...
`XboxUser` is the script-visible wrapper around a local user. It stores:
...
`XboxGameUI` is the runtime-level wrapper for PC-supported `XGameUI` surfaces.
...
- `resolve_privilege_with_ui_async()` (delegates to `XboxUsers`)

The async UI methods use `XboxSignalXAsyncContext` and return completion `Signal` values. When native APIs report `E_ABORT`, the wrapper returns `XboxResult.code == "cancelled"` so scripts can distinguish user cancellation from native failures.

## Accessibility service

`XboxAccessibility` is a synchronous service for concrete APIs from `XAccessibility.h`.
...
`XboxClosedCaptionProperties` is the script-visible wrapper around native
...
`XboxAchievements` is the first service implemented on top of the XBOX services scaffold.
...
`XboxAchievement` is the script-visible wrapper around one cached achievement. It stores:
...
`XboxPresence` is the XAsync-backed presence layer implemented on top of the shared XBOX services scaffold.
...
`XboxPresenceRecord` is the script-visible wrapper around one cached presence snapshot. It stores:
...
`XboxSocial` is the dispatch-driven Social Manager layer.
...
- `XboxSocialFilter` for filter-based group creation
- `XboxSocialGroup` for tracked Social Manager groups
- `XboxSocialUser` for copied social-graph user snapshots

## Profile service

`XboxProfile` is the XSAPI-backed profile layer for XBOX Services profile reads.
...
`XboxUserProfile` is the script-visible wrapper around one XBOX Services profile
record. Calls duplicate from the shared cached XBOX services context via
`XboxServices::duplicate_context_for_user` (which calls
`XblContextDuplicateHandle(...)`); methods return typed errors when the
scaffold isn't initialized.

## Privacy service

`XboxPrivacy` is the XSAPI-backed privacy layer for permission, avoid-list, and
mute-list reads.
...
`XboxStats` is the XSAPI-backed user statistics layer wired through the XBOX
services scaffold.
...
`XboxLeaderboards` is the XSAPI-backed read-only leaderboards layer backed by
...
`XboxLeaderboard` carries `query_type`, `total_row_count`, `has_next`, columns,
and rows. `XboxLeaderboardRow.column_values` carries JSON-encoded column values
returned by XBOX Services.

## String verification service

`XboxStringVerify` is the XSAPI-backed wrapper around XBOX Live string
verification.
...
`XboxTitleStorage` is the XSAPI-backed XBOX Services Title Storage layer
...
`XboxTitleStorageBlobMetadata` and `XboxTitleStorageBlobMetadataResult` are the
...
`XboxMultiplayerActivity` is the XSAPI-backed Multiplayer Activity (MPA) layer
plus recent-players staging. Invite launch notifications are forwarded from
`XboxActivation`, which owns the addon's single native activation subscription
...
`XboxMultiplayerActivityInfo` is the script-visible wrapper around one cached
activity snapshot. Pending invites are surfaced when `XboxActivation` receives an
invite URI on launch; `GDK.activation` and `GDK.multiplayer_activity` emit the
same parsed invite dictionary.

## Package service

`XboxPackage` is the `XPackage`-backed package metadata and DLC content service.
...
`XboxPackageMount` is the RAII wrapper around one mount handle returned by
`mount_package_async()`. `XboxPackageResourcePack` records resource packs
...
completion signal payload (`XboxResult`); there is no global `last_error`
mirror to poll.

## Store service

`XboxStore` is the `XStore` commerce layer with a per-product license cache.
...
`XboxStoreLicenseStatus` is the script-visible wrapper around an `XStore`
...
`XboxCapture` is the `XAppCapture`-backed capture-state and capture-metadata
...
`XboxCaptureMetaData` is the script-side write context around the process-wide
...
`XboxLauncher` is the `XLaunchUri`-only launcher service. The public surface is
...
`XboxErrorReporting` wraps the public Microsoft GDK `XError` callback and options
...
`XboxSystem` is the synchronous title/runtime metadata service. It exposes
...
`XboxDisplay` wraps PC-supported `XDisplay.h` display helpers.
...
`try_enable_hdr_mode()` returns an `XboxResult` with HDR mode details. A
successful `acquire_timeout_deferral()` returns an `XboxDisplayTimeoutDeferral`
in `XboxResult.data`; releasing that wrapper calls the native timeout-deferral
release path.

## Activation service

`XboxActivation` owns the addon's single native activation registration
...
`XboxMultiplayerActivity` listens to this service instead of registering a
...
2. `XboxUsers` checks runtime availability
3. it creates an `XboxPendingSignal`
4. `XboxRuntime` retains that pending request
...
10. `XboxSignalXAsyncContext` forwards the raw block to the concrete finalizer
11. the users service calls `XUserAddResult(...)`
12. the native user handle is wrapped into `XboxUser`
13. service cache and service signals are updated
14. the returned completion signal emits an `XboxResult`
...
The newer XUser-facing methods now reuse that same flow as well: privilege resolution, issue resolution, gamer-picture fetches, and token/signature requests all allocate a concrete `XboxSignalXAsyncContext`, translate native results into Godot `Dictionary` or `Image` payloads, and complete a retained `XboxPendingSignal` on the main-thread dispatch path.
...
1. `XboxAchievements` ensures XBOX services are initialized from title metadata.
...
`XboxSocial` follows that same dispatch-driven model: it registers local users with Social Manager, creates `XboxSocialGroup` wrappers around Social Manager group handles, reacts to `XblSocialManagerDoWork()` events on `GDK.dispatch()`, refreshes cached group/user/presence state, emits service signals, and only then completes pending friends-group ops.
...
- surface payload through `XboxResult.data`
...
5. reuse `XboxRuntime`, `XboxPendingSignal`, `XboxResult`, and `XboxSignalXAsyncContext` as appropriate
