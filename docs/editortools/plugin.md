...
[Godot Microsoft GDK Editor Tools — Editor `GDK` Menu](editor-menu.md).
...
```pwsh
godot --headless -s res://addons/godot_gdk_editortools/run.gd -- `
...
godot --path C:\Repos\MyGame --headless `
    -s res://addons/godot_gdk_editortools/run.gd -- sandbox
```
...
addons\godot_gdk_editortools\gdkpkg.cmd pack --source-dir Build\content --output-dir Build\out

# POSIX (bash / zsh)
addons/godot_gdk_editortools/gdkpkg.sh pack --source-dir Build/content --output-dir Build/out
```
...
| `install`          | `--package`                    | `--bootstrapper` (default on)                                                   | wdapp install on an .msixvc file. `--bootstrapper` routes through the full PC Bootstrapper flow (license, update check, cloud saves) for a retail-style launch; pass `--bootstrapper=false` for a bare developer install. |
| `uninstall`        | `--package-name`               | (none)                                                                          | wdapp uninstall by package full name.                             |
| `launch`           | `--package-name`               | `--aumid`                                                                       | wdapp launch. `run_launch()` uses `--aumid` directly when present; otherwise it resolves the AUMID from `wdapp list` by matching `--package-name`. The current verb matrix in `editortools_cli.gd` still marks `--package-name` as required. |
...
```pwsh
addons\godot_gdk_editortools\gdkpkg.cmd pack --help
```
...
`editortools_config.gd` collapses every source into a single flat dict
...
`editortools_config.gd` only pulls the identity, executable, and product fields
...
addons\godot_gdk_editortools\gdkpkg.cmd config_template
addons\godot_gdk_editortools\gdkpkg.cmd config_template --output Configs\Alt.config
...
addons\godot_gdk_editortools\gdkpkg.cmd sandbox --action set --sandbox-id XDKS.1

# Export, prepare, and pack in three steps.
addons\godot_gdk_editortools\gdkpkg.cmd export --preset "Windows Desktop" --output-dir Build\content
addons\godot_gdk_editortools\gdkpkg.cmd prepare_content --content-dir Build\content --config Configs\Alt.config
addons\godot_gdk_editortools\gdkpkg.cmd pack --source-dir Build\content --output-dir Build\out --config Configs\Alt.config

# Register a loose-files build, launch, then terminate.
addons\godot_gdk_editortools\gdkpkg.cmd register_loose --content-dir Build\content
addons\godot_gdk_editortools\gdkpkg.cmd launch --package-name MyPublisher.MyGame_1.0.0.0_x64__abc123
addons\godot_gdk_editortools\gdkpkg.cmd terminate --package-name MyPublisher.MyGame_1.0.0.0_x64__abc123
```
...
- [Godot Microsoft GDK Editor Tools — Editor `GDK` Menu](editor-menu.md) — what each
  editor menu item does and what prerequisites it needs.
- `.github\instructions\godot-gdk-editortools.instructions.md` — repo-wide
