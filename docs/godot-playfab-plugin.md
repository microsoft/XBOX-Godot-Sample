...
`godot_playfab` is the PlayFab-focused GDExtension addon in this repository. It currently implements a single `PlayFab` root singleton, manual user sign-in keyed by local Xbox user id or title-defined custom id, PlayFab Game Saves wrappers, and PlayFab leaderboard submission/query flows.
