...
access to the title — fix that before moving on. Only `GDK.users` and
the XBOX sandbox matter here; this is a GDK-only track with no PlayFab
dependency.
...
The GDK-track reference implementation lives in
[`sample/tutorial_gdk/`](../../../sample/tutorial_gdk/README.md):
...
Continue to [GDK Tutorial 3 — Title Storage and stats](03-storage-stats.md).
