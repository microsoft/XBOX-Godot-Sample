...
- an **Achievements** panel that exercises GDK Tutorial 2, [Unlock an achievement](../gdk/02-achievement.md) (unlock the next

  progress step on a button press, observe `achievement_unlocked`).

- a **Leaderboards** panel that exercises PlayFab Tutorial 2, [Leaderboard](../playfab/02-leaderboard.md) (live top-10 plus

  around-user paging, refresh on demand).

- a **Game Saves** panel that demonstrates Game Saves in this capstone (write a timestamped

  blob, upload to the cloud, read back).

- a **Lobby** panel that exercises PlayFab Tutorial 3, [Lobby](../playfab/03-lobby.md) (create / join / leave,

  member list).

- an **MPA** panel that exercises GDK Tutorial 4, [Multiplayer Activity](../gdk/04-mpa.md) (advertised state +

  invite-sent log + accepted-invite log).

- a **Party** panel that exercises PlayFab Tutorial 4, [Party](../playfab/04-party.md) (peer list + text-chat box
...
- The cumulative `Auth` and `Lobby` autoloads from [Integrated Tutorial 1](01-signin.md) and the [PlayFab Lobby tutorial](../playfab/03-lobby.md),

  extended with [GDK Multiplayer Activity](../gdk/04-mpa.md) wiring, are registered in the project's
...
- A signed-in XBOX test account with at least one **declared
  ...
  `"high_score"` for both, matching the [PlayFab Leaderboard tutorial](../playfab/02-leaderboard.md)).
...
- `Auth.xbox_user`, `Auth.playfab_user`, `Auth.state_changed`,
  ...
  `Auth.sign_in()`, `Auth.is_signed_in()` — your Integrated 1 state-machine
...
- [`GDK.users`](../../../addons/godot_gdk/doc_classes/GDKUsers.xml)
  ...
  from the PlayFab Lobby tutorial) and the `Party` autoload (Communications +

  CommunicationVoiceIngame from the PlayFab Party tutorial). See the
...
- [`GDK.privacy`](../../../addons/godot_gdk/doc_classes/GDKPrivacy.xml)
  ...
  Per-target permission filtering for invites (from the [GDK Multiplayer Activity tutorial](../gdk/04-mpa.md)) and

  per-peer chat permissions (the PlayFab Party tutorial); inherited through the
...
Create `res://i02_integration/i02_integration.tscn` with this
...
Attach `res://i02_integration/i02_integration.gd` to the root
...
func _ready() -> void:
    ...
    # Kick sign-in for cold Integrated 2 entry. Idempotent — joins the
...
Attach `res://i02_integration/panel_achievements.gd` to the

`Achievements` VBoxContainer. The panel mirrors the [GDK achievement tutorial](../gdk/02-achievement.md) happy path
...
The four buttons (25 / 50 / 75 / Unlock) write the canonical achievement
...
[GDK Tutorial 2](../gdk/02-achievement.md), just behind a button.
...
Attach `res://i02_integration/panel_leaderboard.gd` to the
...
- The submit button drives the same `update_statistics_async` flow
  ...
  from the [PlayFab Leaderboard tutorial](../playfab/02-leaderboard.md), except the score increments every click instead of being
...
Attach `res://i02_integration/panel_game_saves.gd` to the
...
that Game Saves resolves, mirroring the Game Saves flow demonstrated in this capstone. Game Saves is the cloud
...
Attach `res://i02_integration/panel_lobby.gd` to the `Lobby`
...
> Integrated Tutorial 2 used a hard-coded `JOIN_STRING` constant; the
...
> **Inherited from the PlayFab Lobby and GDK Multiplayer Activity tutorials (no extra panel code).** The Lobby

> autoload's `host_lobby` / `join_lobby` / `leave_lobby` already

> drive `GDK.presence.set_presence_async` / `clear_presence_async`

> from [PlayFab Lobby tutorial](../playfab/03-lobby.md),
...
> from [GDK MPA tutorial](../gdk/04-mpa.md)
...
func _on_send_pressed() -> void:
    ...
    # invite_friend returns a bool, so this panel can
...
the `Lobby` autoload from the GDK Multiplayer Activity tutorial. The MPA panel exists to make the
...
Attach `res://i02_integration/panel_party.gd` to the `Party`
...
the active `PlayFabPartyNetwork` (introduced in the PlayFab Party tutorial). Wire the
...
endpoint mute) instead. See the [PlayFab Party tutorial](../playfab/04-party.md) for the canonical create / join
...
- The **Lobby** autoload (the PlayFab Lobby tutorial) gates `host_lobby` /
...
- The **MPA** panel's targeted invite (Step 6) inherits the
  ...
  [GDK MPA tutorial](../gdk/04-mpa.md)
...
- The **Party** autoload (the PlayFab Party tutorial) decides whether to enable
...
| HUD says "Signing in…" forever | `Auth` is stuck in `SIGNING_IN_*`. | Check the Output panel for a `[Auth] sign-in failed at <stage>: <message>` line and run the matching Integrated 1 fix. The HUD retry button re-runs `Auth.sign_in()` without restarting the scene; `sign_in()` resets stale failure state on each retry. |
...
[`sample/tutorial_integrated/`](../../../sample/tutorial_integrated/README.md):



- Scene: [`sample/tutorial_integrated/i02_integration/i02_integration.tscn`](../../../sample/tutorial_integrated/i02_integration/i02_integration.tscn)

- Root script: [`sample/tutorial_integrated/i02_integration/i02_integration.gd`](../../../sample/tutorial_integrated/i02_integration/i02_integration.gd)

- Per-surface panels (one per Step):

  - [`panel_achievements.gd`](../../../sample/tutorial_integrated/i02_integration/panel_achievements.gd) (Step 2)

  - [`panel_leaderboard.gd`](../../../sample/tutorial_integrated/i02_integration/panel_leaderboard.gd) (Step 3)

  - [`panel_game_saves.gd`](../../../sample/tutorial_integrated/i02_integration/panel_game_saves.gd) (Step 4)

  - [`panel_lobby.gd`](../../../sample/tutorial_integrated/i02_integration/panel_lobby.gd) (Step 5)

  - [`panel_mpa.gd`](../../../sample/tutorial_integrated/i02_integration/panel_mpa.gd) (Step 6)

  - [`panel_party.gd`](../../../sample/tutorial_integrated/i02_integration/panel_party.gd) (Step 7)

- Consumes the `Auth`, `Lobby`, and `Party` autoloads in

  [`sample/tutorial_integrated/autoload/`](../../../sample/tutorial_integrated/autoload/).



Tutorial paths in this step (`res://i02_integration/...`) match the integrated sample exactly.
