extends GutTest
## GUT coverage for the static helpers on `editortools_content_preparer.gd`
...
const _FIXTURE_DIR := "user://test_editortools_content_preparer"
...
func _new_preparer() -> RefCounted:
	return EditorToolsContentPreparerScript.new(GameConfigManagerScript.new(_FakeToolchain.new()))
...
func test_patch_executable_replaces_existing_attribute() -> void:
	...
	var patched: String = EditorToolsContentPreparerScript.patch_executable_name(input, "NewName.exe")
...
func test_patch_executable_no_match_passthrough() -> void:
	...
	var patched: String = EditorToolsContentPreparerScript.patch_executable_name(input, "Anything.exe")
...
func test_patch_executable_replaces_all_attributes() -> void:
	...
	var patched: String = EditorToolsContentPreparerScript.patch_executable_name(input, "C.exe")
...
func test_patch_executable_escapes_xml_special_chars() -> void:
	...
	var patched: String = EditorToolsContentPreparerScript.patch_executable_name(input, unsafe)
...
func test_patch_executable_preserves_dollar_characters() -> void:
	var input := '<Executable Name="OldName.exe" Id="Game" />'
	var patched: String = EditorToolsContentPreparerScript.patch_executable_name(input, "Cash$1Game$N.exe")
...
func test_patch_executable_handles_realistic_config_block() -> void:
	...
	input += '  </ExecutableList>\n'
	input += '</Game>\n'
	var patched: String = EditorToolsContentPreparerScript.patch_executable_name(input, "MyGame.exe")
...
func test_inject_vc14_inserts_block_before_close_game() -> void:
	...
	var injected: String = EditorToolsContentPreparerScript.inject_vc14_dependency(input)
...
func test_inject_vc14_idempotent_when_dependency_name_already_present() -> void:
	...
	existing += "  </DesktopRegistration>\n"
	existing += "</Game>"
	var injected: String = EditorToolsContentPreparerScript.inject_vc14_dependency(existing)
...
func test_inject_vc14_runs_again_after_dependency_stripped() -> void:
	var input := "<Game>\n</Game>\n"
	var first: String = EditorToolsContentPreparerScript.inject_vc14_dependency(input)
	var stripped := first.replace('<KnownDependency Name="VC14"/>', '<KnownDependency Name="OTHER"/>')
	var second: String = EditorToolsContentPreparerScript.inject_vc14_dependency(stripped)
...
func test_copy_addon_runtime_dlls_refreshes_stale_destination() -> void:
	...
	var preparer = EditorToolsContentPreparerScript.new(RefCounted.new())
...
func test_pipeline_inject_then_patch_matches_caller_order() -> void:
	...
	input += '  </ExecutableList>\n'
	input += '</Game>\n'
	var step1: String = EditorToolsContentPreparerScript.inject_vc14_dependency(input)
	var step2: String = EditorToolsContentPreparerScript.patch_executable_name(step1, "RealGame.exe")
