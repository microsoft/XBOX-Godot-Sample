extends GutTest
## GUT coverage for `core/editortools_result.gd` — the typed Dictionary
## builder used by every `EditorToolsService` verb.
##
## Pins:
...
func test_exit_code_constants_are_stable() -> void:
	assert_eq(EditorToolsResult.EXIT_OK, 0, "EXIT_OK is 0")
	assert_eq(EditorToolsResult.EXIT_FAIL, 1, "EXIT_FAIL is 1")
	assert_eq(EditorToolsResult.EXIT_USAGE, 2, "EXIT_USAGE is 2")
	assert_eq(EditorToolsResult.EXIT_CONFIG, 3, "EXIT_CONFIG is 3")
	assert_eq(EditorToolsResult.EXIT_TOOL, 4, "EXIT_TOOL is 4")
	assert_eq(EditorToolsResult.EXIT_UNIMPLEMENTED, 5, "EXIT_UNIMPLEMENTED is 5")


func test_json_line_prefix_is_stable() -> void:
	assert_eq(EditorToolsResult.JSON_LINE_PREFIX, "PACKAGING_RESULT_JSON:",
...
func test_make_default_is_ok_with_empty_fields() -> void:
	var r: Dictionary = EditorToolsResult.make("pack")
...
func test_make_with_nonzero_exit_code_flips_ok() -> void:
	var r: Dictionary = EditorToolsResult.make("pack", EditorToolsResult.EXIT_TOOL, "boom")
...
func test_make_duplicates_details_so_caller_mutations_dont_leak() -> void:
	...
	var r: Dictionary = EditorToolsResult.make("pack", 0, "", details)
	details["key"] = "mutated"
...
func test_ok_builds_success_result() -> void:
	var r: Dictionary = EditorToolsResult.ok("validate", "looks good")
...
func test_fail_promotes_zero_exit_code_to_generic_fail() -> void:
	var r: Dictionary = EditorToolsResult.fail("pack", "no", EditorToolsResult.EXIT_OK)
	...
	# non-zero.
	assert_eq(r["exit_code"], EditorToolsResult.EXIT_FAIL,
		"fail() refuses to build a zero-exit result")
	assert_false(r["ok"], "ok flag flipped")


func test_fail_carries_stderr_and_details() -> void:
	var r: Dictionary = EditorToolsResult.fail("install", "nope",
		EditorToolsResult.EXIT_TOOL, "stderr text", {"package": "Foo"})
	assert_eq(r["exit_code"], EditorToolsResult.EXIT_TOOL, "exit_code carried")
...
func test_json_round_trip_preserves_fields() -> void:
	var src: Dictionary = EditorToolsResult.ok("pack", "done",
		{"map": "/tmp/layout.xml"}, "tool stdout", 17)
	var line: String = EditorToolsResult.to_json_line(src)
	assert_true(line.begins_with(EditorToolsResult.JSON_LINE_PREFIX),
		"line carries the canonical prefix")
	var parsed: Dictionary = EditorToolsResult.from_json_line(line)
...
func test_from_json_line_rejects_non_marker_lines() -> void:
	assert_eq(EditorToolsResult.from_json_line("not a marker"), {},
		"plain text is rejected")
	assert_eq(EditorToolsResult.from_json_line(""), {}, "empty string is rejected")


func test_from_json_line_rejects_invalid_json() -> void:
	var bad: String = EditorToolsResult.JSON_LINE_PREFIX + "{not valid json"
	assert_eq(EditorToolsResult.from_json_line(bad), {},
...
func test_is_valid_shape_accepts_make_output() -> void:
	assert_true(EditorToolsResult.is_valid_shape(EditorToolsResult.make("pack")),
		"make() output passes shape check")
	assert_true(EditorToolsResult.is_valid_shape(
		EditorToolsResult.fail("pack", "x", EditorToolsResult.EXIT_TOOL)),
