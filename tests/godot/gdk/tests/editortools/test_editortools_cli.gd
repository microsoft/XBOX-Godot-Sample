extends GutTest
## GUT coverage for `core/editortools_cli.gd` — the pure argv parser used by
...
func test_verbs_table_covers_every_expected_verb() -> void:
	...
	for verb: String in expected:
		assert_true(EditorToolsCli.VERBS.has(verb), "verb '%s' is defined" % verb)
	assert_eq(EditorToolsCli.VERBS.size(), expected.size(),
...
func test_every_verb_entry_has_doc_and_flags() -> void:
	for verb: String in EditorToolsCli.VERBS:
		var entry: Dictionary = EditorToolsCli.VERBS[verb]
...
func test_verb_table_pins_documented_non_required_flags() -> void:
	assert_true(EditorToolsCli.VERBS["validate"]["flags"].has("output-dir"),
		"validate documents --output-dir")
	assert_true(EditorToolsCli.VERBS["export"]["flags"].has("no-prepare"),
...
func test_help_before_verb_sets_help_flag() -> void:
	var parsed: Dictionary = EditorToolsCli.parse(PackedStringArray(["--help"]))
	assert_true(parsed["help"], "--help is recognized")


func test_help_after_verb_sets_help_flag() -> void:
	var parsed: Dictionary = EditorToolsCli.parse(PackedStringArray(["pack", "--help"]))
...
func test_unknown_verb_fails_with_usage_exit_code() -> void:
	var parsed: Dictionary = EditorToolsCli.parse(PackedStringArray(["bogus"]))
	assert_false(parsed["ok"], "unknown verb fails")
	assert_eq(parsed["error_code"], EditorToolsResult.EXIT_USAGE,
...
func test_pack_requires_source_dir_and_output_dir() -> void:
	var parsed: Dictionary = EditorToolsCli.parse(PackedStringArray(["pack"]))
...
func test_render_usage_lists_every_verb() -> void:
	var text: String = EditorToolsCli.render_usage()
	for verb: String in EditorToolsCli.VERBS:
...
func test_render_verb_usage_for_unknown_verb_returns_error() -> void:
	var text: String = EditorToolsCli.render_verb_usage("nope")
...
func test_render_verb_usage_includes_flag_names() -> void:
	var text: String = EditorToolsCli.render_verb_usage("pack")
