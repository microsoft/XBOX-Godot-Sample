...
func test_no_json_suppresses_packaging_result_json_line() -> void:
	...
	assert_string_contains(run["output"], "[editortools] sandbox ok", "summary still prints")
	assert_eq(run["output"].find(EditorToolsResult.JSON_LINE_PREFIX), -1, "JSON marker is omitted under --no-json")
...
func _build_fake_runner_project(name: String) -> String:
	...
	var addon_dir: String = root.path_join("addons").path_join("godot_gdk_editortools")
	...
	_copy_text("res://addons/godot_gdk_editortools/run.gd", addon_dir.path_join("run.gd"))
	_copy_text("res://addons/godot_gdk_editortools/core/editortools_cli.gd", core_dir.path_join("editortools_cli.gd"))
	_copy_text("res://addons/godot_gdk_editortools/core/editortools_result.gd", core_dir.path_join("editortools_result.gd"))
	_write_text(core_dir.path_join("editortools_config.gd"), _fake_editortools_config_source())
	_write_text(core_dir.path_join("editortools_service.gd"), _fake_editortools_service_source())
	return root
...
func _run_child(project_dir: String, user_args: PackedStringArray) -> Dictionary:
	var args := PackedStringArray([
		"--headless",
		"--path", project_dir,
		"-s", "res://addons/godot_gdk_editortools/run.gd",
		"--",
	])
...
func _fake_editortools_config_source() -> String:
	return """
@tool
extends RefCounted
...
func _fake_editortools_service_source() -> String:
	return """
@tool
extends RefCounted
const EditorToolsResult = preload("res://addons/godot_gdk_editortools/core/editortools_result.gd")
func dispatch(verb: String, resolved: Dictionary) -> Dictionary:
	...
	return EditorToolsResult.make(verb, exit_code, "fake dispatched " + verb, {"resolved": resolved}, "fake stdout", "fake stderr" if exit_code != 0 else "", 0)
"""
