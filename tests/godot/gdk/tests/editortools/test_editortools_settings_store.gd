...
const _FIXTURE_DIR := "user://test_editortools_settings_store"
...
func test_load_missing_file_returns_full_default_shape() -> void:
	var state: Dictionary = EditorToolsSettingsStoreScript.new().load_state(_fixture_path("missing.cfg"))
...
func test_load_ignores_unknown_keys_and_fills_omitted_defaults() -> void:
	...
	var loaded: Dictionary = EditorToolsSettingsStoreScript.new().load_state(path)
