...
const _FIXTURE_DIR := "user://test_editortools_service"
...
func _new_service() -> RefCounted:
	return EditorToolsServiceScript.new(_FakeToolchain.new())


func test_method_for_verb_returns_expected_method_names() -> void:
	var service = _new_service()
	assert_eq(EditorToolsCli.VERBS.size(), 14, "EditorToolsService exposes the full CLI verb set")
	for verb: String in EditorToolsCli.VERBS:
...
func test_dispatch_routes_every_cli_verb_to_a_result_shaped_run_method() -> void:
	...
	var service = EditorToolsServiceScript.new(fake)

	for verb: String in EditorToolsCli.VERBS:
		var result: Dictionary = service.dispatch(verb, _resolved_for_verb(verb, root))
		assert_true(EditorToolsResult.is_valid_shape(result), "%s result has EditorToolsResult shape" % verb)
		assert_eq(result["verb"], verb, "dispatch(%s) reaches run_%s" % [verb, verb])
		assert_eq(result["exit_code"], EditorToolsResult.EXIT_OK, "%s succeeds with the fake toolchain" % verb)
...
func test_dispatch_unknown_verb_returns_unimplemented_result() -> void:
	...
	assert_true(EditorToolsResult.is_valid_shape(result))
	assert_eq(result["verb"], "missing_verb")
	assert_eq(result["exit_code"], EditorToolsResult.EXIT_UNIMPLEMENTED)
...
func test_tool_backed_verbs_normalize_underlying_failures_to_exit_tool() -> void:
	...
	for verb: String in operations:
		...
		var result: Dictionary = EditorToolsServiceScript.new(fake).dispatch(verb, _resolved_for_verb(verb, root))

		assert_true(EditorToolsResult.is_valid_shape(result), "%s failure still has EditorToolsResult shape" % verb)
		assert_eq(result["verb"], verb)
		assert_eq(result["exit_code"], EditorToolsResult.EXIT_TOOL, "%s maps dependency failure to EXIT_TOOL" % verb)
...
const _ConfigMgr = preload("res://addons/godot_gdk_editortools/core/game_config_manager.gd")
...
func test_to_filesystem_path_globalizes_user_paths() -> void:
	var normalized: String = _ConfigMgr.to_filesystem_path("user://test_editortools_service/some.config")
	assert_eq(normalized, ProjectSettings.globalize_path("user://test_editortools_service/some.config"),
...
func test_config_template_accepts_res_output_path() -> void:
	# res://test_editortools_service/ResOut.config — write under user-test fixture
	# tree by globalizing a unique res-rooted path.
	var res_out: String = "res://test_editortools_service_res_out.config"
