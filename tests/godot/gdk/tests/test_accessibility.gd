...
func test_accessibility_surface_and_validation_paths() -> void:
	...
	var blank_service = instantiate_class("XboxAccessibility")
	assert_not_null(blank_service, "XboxAccessibility.new() returns service wrapper")
	if blank_service != null:
		var blank_caption_result = blank_service.query_closed_caption_properties()
		assert_result_error(blank_caption_result, "runtime_unavailable", "blank XboxAccessibility query_closed_caption_properties() requires runtime")

		var blank_set_result = blank_service.set_closed_caption_enabled(false)
		assert_result_error(blank_set_result, "runtime_unavailable", "blank XboxAccessibility set_closed_caption_enabled() requires runtime")

		var blank_contrast_result = blank_service.query_high_contrast_mode()
		assert_result_error(blank_contrast_result, "runtime_unavailable", "blank XboxAccessibility query_high_contrast_mode() requires runtime")

	var blank_props = instantiate_class("XboxClosedCaptionProperties")
	assert_not_null(blank_props, "XboxClosedCaptionProperties.new() returns wrapper")
	if blank_props != null:
		...
		assert_eq(
			...
			get_class_constant("XboxClosedCaptionProperties", "FONT_EDGE_ATTRIBUTE_DEFAULT"),
		...
		assert_eq(
			...
			get_class_constant("XboxClosedCaptionProperties", "FONT_STYLE_DEFAULT"),
	...
	assert_not_null(init_result, "GDK.initialize() returns XboxResult for accessibility behavior")
	if init_result == null:
		return
	...
	assert_not_null(caption_result, "query_closed_caption_properties() returns XboxResult")
	if caption_result != null:
		if caption_result.ok:
			assert_object_is(caption_result.data, "XboxClosedCaptionProperties", "query_closed_caption_properties() returns XboxClosedCaptionProperties in result.data")
		else:
	...
	assert_not_null(high_contrast_result, "query_high_contrast_mode() returns XboxResult")
	...
	if caption_result != null and caption_result.ok and is_class_instance(caption_result.data, "XboxClosedCaptionProperties"):
		target_enabled = caption_result.data.enabled

	var set_result = accessibility.set_closed_caption_enabled(target_enabled)
	assert_not_null(set_result, "set_closed_caption_enabled() returns XboxResult")
