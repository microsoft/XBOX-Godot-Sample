...
func test_settings_persistence() -> void:
	...
	var store = EditorToolsSettingsStoreScript.new()
...
func test_content_preparer_helpers() -> void:
	...
	var patched = EditorToolsContentPreparerScript.patch_executable_name(xml, "NewGame.exe")
	assert_true(patched.contains('Executable Name="NewGame.exe"'), "executable name patched")

	var with_dependency = EditorToolsContentPreparerScript.inject_vc14_dependency("<Game>\n</Game>\n")
	assert_true(with_dependency.contains('<KnownDependency Name="VC14"/>'), "VC14 dependency injected")

	var unchanged = EditorToolsContentPreparerScript.inject_vc14_dependency(with_dependency)
