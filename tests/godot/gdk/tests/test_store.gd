...
func test_store_surface_and_validation_paths() -> void:
	...
	var blank_license_status = instantiate_class("XboxStoreLicenseStatus")
	assert_not_null(blank_license_status, "XboxStoreLicenseStatus.new() returns wrapper")
	if blank_license_status != null:
		...
		assert_eq(blank_license_status.get_store_id(), "", "blank XboxStoreLicenseStatus store_id defaults empty")
		assert_eq(blank_license_status.get_licensable_sku(), "", "blank XboxStoreLicenseStatus licensable_sku defaults empty")
		assert_eq(blank_license_status.get_status(), 0, "blank XboxStoreLicenseStatus status defaults 0")
	...
	assert_not_null(invalid_cached_check, "check_cached_license_status('') returns XboxResult")
	...
	assert_not_null(init_result, "GDK.initialize() for store behavior returns XboxResult")
	if init_result == null:
		return
	...
	assert_not_null(uncached_check, "check_cached_license_status() returns XboxResult for uncached values")
