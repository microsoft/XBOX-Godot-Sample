...
## Surface + validation coverage for `XboxDisplay` (`GDK.display`).
...
func test_display_surface_and_validation_paths() -> void:
	...
	for constant_name in [
		...
		assert_true(
				ClassDB.class_has_integer_constant("XboxDisplay", constant_name),
				"XboxDisplay exposes %s" % constant_name)
	...
	assert_not_null(init_result, "GDK.initialize() for display behavior returns XboxResult")
	if init_result == null:
		return
	...
	var hdr_mode = display.try_enable_hdr_mode(get_class_constant("XboxDisplay", "HDR_MODE_PREFERENCE_PREFER_REFRESH_RATE"))
	assert_not_null(hdr_mode, "try_enable_hdr_mode() returns XboxResult after init")
	if hdr_mode != null and hdr_mode.ok:
		...
		if hdr_mode.data is Dictionary:
			...
			var valid_modes := [
				get_class_constant("XboxDisplay", "HDR_MODE_UNKNOWN"),
				get_class_constant("XboxDisplay", "HDR_MODE_ENABLED"),
				get_class_constant("XboxDisplay", "HDR_MODE_DISABLED"),
			]
			assert_true(mode_value in valid_modes, "try_enable_hdr_mode() mode is one of HDR_MODE_*")
			if mode_value == get_class_constant("XboxDisplay", "HDR_MODE_ENABLED"):
	...
	assert_not_null(deferral_result, "acquire_timeout_deferral() returns XboxResult after init")
	if deferral_result != null and deferral_result.ok:
		var deferral = deferral_result.data
		assert_true(is_class_instance(deferral, "XboxDisplayTimeoutDeferral"), "acquire_timeout_deferral() data is XboxDisplayTimeoutDeferral")
		if is_class_instance(deferral, "XboxDisplayTimeoutDeferral"):
			assert_true(deferral.is_valid(), "XboxDisplayTimeoutDeferral is valid after acquire")
			deferral.release()
			assert_false(deferral.is_valid(), "XboxDisplayTimeoutDeferral is invalid after release()")
			deferral.release()
