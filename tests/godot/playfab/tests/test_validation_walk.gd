...
##   - sign_in_async(user_or_local_id, create_account)   → "not_initialized", "invalid_user_or_local_id"
##
## PlayFabUsers:
##   - sign_in_async(user_or_local_id, create_account)   → "not_initialized", "invalid_user_or_local_id"
##
...
func test_root_sign_in_async_not_initialized() -> void:
	if pending_unless_playfab_available():
		return
	var playfab = get_playfab()
	reset_playfab_runtime()

	var sign_in_signal = playfab.sign_in_async(1)
	await _assert_signal_error(
		sign_in_signal, "not_initialized",
		"PlayFab.sign_in_async() before initialize()")


# ── PlayFabUsers: not_initialized + invalid ──────────────────────────────

func test_users_sign_in_async_not_initialized() -> void:
	if pending_unless_playfab_available():
		return
	var playfab = get_playfab()
	reset_playfab_runtime()
	...
	var sign_in_signal = users.sign_in_async(1)
	await _assert_signal_error(
		sign_in_signal, "not_initialized",
		"PlayFab.users.sign_in_async() before initialize()")
...
# ── invalid_user_or_local_id (PlayFab + PlayFabUsers) ─────────────────────
#
# These exercise the post-`not_initialized` validation paths. The runtime
# stays in `not_initialized` here too, so the `invalid_user_or_local_id`
# code is only emitted for ENV variants where users.sign_in_async runs the
...
func test_root_sign_in_invalid_user_or_local_id_zero_post_init() -> void:
	if pending_unless_playfab_available():
		return
	var playfab = get_playfab()
	reset_playfab_runtime()
	# Force the runtime into the "initialized=false" state. The current
	# guard order is runtime-first, so passing a 0 local id here surfaces
	# "not_initialized" — kept as a behavioural assertion that the engine
	# is consistently in one of the two documented branches.
	var sign_in_signal = playfab.sign_in_async(0)
	await _assert_signal_one_of(
		sign_in_signal,
		PackedStringArray(["not_initialized", "invalid_user_or_local_id"]),
		"PlayFab.sign_in_async(0) reports a documented validation code")


func test_root_sign_in_invalid_user_or_local_id_unsupported_type_post_init() -> void:
	if pending_unless_playfab_available():
		return
	var playfab = get_playfab()
	reset_playfab_runtime()

	var sign_in_signal = playfab.sign_in_async(Vector2(1, 2))
	await _assert_signal_one_of(
		sign_in_signal,
		PackedStringArray(["not_initialized", "invalid_user_or_local_id"]),
		"PlayFab.sign_in_async(Vector2) reports a documented validation code")
