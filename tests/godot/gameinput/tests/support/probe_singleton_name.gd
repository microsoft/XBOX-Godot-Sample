...
## `override.cfg` renaming the GameInput engine singleton. Prints a single
## `PROBE` line for the parent to parse and never asserts on its own — the
## parent owns the pass/fail decision so it can always clean the `override.cfg`
## back up.
##
...
func _initialize() -> void:
	...
	print("PROBE setting=%s configured_registered=%s default_registered=%s" % [
		...
		str(Engine.has_singleton("GameInput")),
	])
	quit(0)
