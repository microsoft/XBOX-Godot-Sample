...
- `addons\godot_playfab\`, `tests\godot\playfab\`, the PlayFab-facing sample tracks (`sample\tutorial_playfab\` and `sample\tutorial_integrated\`), `docs\playfab\`, `docs\tutorials\playfab\`, and `spec\gdext-playfab.md` are covered by `.github\instructions\godot-playfab.instructions.md`.
...
```
  ...
  The orchestrator runs 8 stages (in order): a preflight configuration check, the parse gate, `cmake --build --preset debug`, the C++ doctest exe (`gdk_unit_tests.exe`), the **live environment probe**, GUT for each coverage host (`tests\godot\gdk`, `tests\godot\playfab`, `tests\godot\gameinput`), the PlayFab Multiplayer orchestrator, the bootstrap mini-runners under each host's `tests\bootstrap\` directory, and the aggregate summary. Per-stage results land in `build\test-results\run-summary.{json,md}`. **The live and live-write tiers are mandatory and cannot be opted out of** — every run sets `LIVE_TESTS=1` and `LIVE_WRITE_TESTS=1` and mutates the configured PlayFab title, so always point it at a dedicated sandbox title. `-PlayFabTitleId`, `-PlayFabCustomId`, and `-PlayFabMatchmakingQueue` are required (each also readable from `PLAYFAB_TITLE_ID` / `PLAYFAB_CUSTOM_ID` / `PLAYFAB_MULTIPLAYER_MATCH_QUEUE`); the preflight hard-fails and lists everything missing before any stage runs. The live environment probe (`tools\ci\live_env_probe.gd`) then verifies the machine itself — GDK runtime, signed-in Xbox user, PlayFab reachability, GameInput runtime, connected gamepad — per host, and aborts naming the missing capability. See `tests\godot\README.md` for the test-tier contract.
- **`pending()` fails under the live tier.** The shared bases override `pending()` to fail whenever `LIVE_TESTS` is set, so a test can no longer degrade coverage to a green tick. Use `pending_tolerated(reason)` only for opt-in axes outside the live-tier contract (`GDK_CAPTURE_*`, `GDK_TEST_DLC_*`, `GameDKCoreLatest`, documented service variance). Never add a new inline `pending()` to work around a failing live precondition — fix the precondition or let it fail.
- New GUT suites live under each host's `tests\` directory as `test_*.gd` files (auto-discovered via `-gdir res://tests -ginclude_subdirs`) and should `extends` the appropriate shared base from `addons\godot_gdk_tests\` (`gdk_test_base.gd`, `playfab_test_base.gd`, or `gameinput_test_base.gd`). The bases live in `addons\godot_gdk\tests_support\bases\` and CMake mirrors them into each host. GUT is carried as **two** git submodules because GUT 9.6.0 hard-requires Godot 4.6+ (`required_godot_version = '4.6'`) but the repo also supports Godot 4.5.x: `third_party\Gut\` is pinned to upstream tag `v9.6.0` (the default, for Godot 4.6+), and `third_party\Gut-4.5\` is pinned to upstream commit `b366b70` (`v9.5.0` + the #778 `is_push_warning` fix, still `required_godot_version = '4.5'`, for Godot 4.5.x). CMake mirrors both into each host as `addons\gut\` (9.6.0) and `addons\gut-4.5\`; the 4.5.x test legs swap `gut-4.5` over `gut` (CI via `run-test-tier`, local via `run_all_tests.ps1`'s `Select-GutForGodotVersion`). Both submodules' contents and the mirrored copies are intentionally untouched — never edit them. To refresh either GUT, bump that submodule's pin (`cd third_party\Gut` or `third_party\Gut-4.5`, `git fetch && git checkout <ref>` from the repo root, then commit the updated submodule pointer); do not vendor copies into the superproject.
...
- **Live coverage is not optional.** The live and live-write tiers always run; there is no skip flag. State which sandbox PlayFab title id the run targeted, and call that out in the PR description. A run that lacks configuration aborts at preflight, and a machine that cannot reach the services aborts at the live environment probe, rather than reporting green. See `tests\godot\README.md` for the test-tier contract.
- **No new `pending()` guards.** If the change adds or moves a live-tier test, confirm it does not introduce an inline `pending()` that hides a missing precondition. `pending_tolerated(...)` is the only sanctioned escape hatch, and only for opt-in axes outside the live-tier contract.
- **Fork PRs need a maintainer re-run.** `test-tier` cannot pass on a fork PR because GitHub withholds the sandbox secrets. Push the PR head to `pr/<number>-<slug>` in this repository and link the resulting run. Never switch the workflow to `pull_request_target`. See `docs\ci\pr-gates.md#maintainer-re-run-policy-for-fork-prs`.
