...
- **No `Ref<>` parameters across addon DLL boundaries.** `godot_playfab` cannot accept a `Ref<XboxUser>` directly because `godot_gdk` and `godot_playfab` are separate GDExtension DLLs. Use `Object *` and duck-type via `has_method` / `call`. Same rule for any future cross-addon API.
...
- **No raw local Xbox user ids in PlayFab sign-in.** `PlayFab.users.sign_in_with_xuser_async` accepts an `XboxUser` object only. Plumbing local handles through the higher-level API is a step backwards — keep Xbox-facing identity inside the GDK side.
