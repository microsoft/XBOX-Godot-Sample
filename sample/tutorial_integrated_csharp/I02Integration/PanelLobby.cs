...
public partial class PanelLobby : VBoxContainer
{
    ...
    public override void _ExitTree()
    {
        ...
        if (!_initialized || _lobbyNode == null) return;
        _lobbyNode.StateChanged -= OnLobbyStateChanged;
        _lobbyNode.LobbyDisconnected -= OnLobbyDisconnected;
        PlayFab.Multiplayer.StateChanged -= OnMultiplayerStateChanged;
    }
    ...
    {
        if (_initialized) return; _initialized = true; _host.Pressed += async () => await OnHostPressed(); _join.Pressed += async () => await OnJoinPressed(); _leave.Pressed += async () => await OnLeavePressed(); _lobbyNode.StateChanged += OnLobbyStateChanged; _lobbyNode.LobbyDisconnected += OnLobbyDisconnected; PlayFab.Multiplayer.StateChanged += OnMultiplayerStateChanged; Refresh();
    }
    ...
    private void OnMultiplayerStateChanged(PlayFabMultiplayerStateChange change) { if (IsInsideTree()) Refresh(); }
}
