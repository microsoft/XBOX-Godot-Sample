...
public partial class PanelMpa : VBoxContainer
{
    ...
    {
        if (!_initialized && _auth != null && _auth.IsSignedIn()) _ = InitializeAfterSignInAsync();
    }
    ...
    private void OnMultiplayerStateChanged(GodotPlayFab.Types.PlayFabMultiplayerStateChange change) { if (IsInsideTree()) RefreshState(); }
    ...
    private void OnInviteAccepted(Godot.Collections.Dictionary invite) { _log.AppendText($"Accepted: {TutorialSupport.DictString(invite, "raw_uri")}\n"); }
    private void OnPendingInviteReceived(Godot.Collections.Dictionary invite) { _log.AppendText($"Pending: {TutorialSupport.DictString(invite, "raw_uri")}\n"); }
