...
public partial class PanelParty : VBoxContainer
{
    ...
    public override void _ExitTree()
    {
        ...
        if (_peer != null)
        {
            _peer.TextMessageReceived -= OnTextReceived;
            _peer.ChatControlAdded -= OnChatControlAdded;
            _peer.ChatControlRemoved -= OnChatControlRemoved;
        }
    }
    ...
    {
        if (_initialized) return; _initialized = true; _send.Pressed += async () => await OnSendPressed(); _muteRemotes.Toggled += OnMuteRemotesToggled; _partyNode.NetworkJoined += AttachNetwork; _partyNode.NetworkLeft += OnNetworkLeft; _partyNode.NetworkDestroyed += OnNetworkDestroyed; _partyNode.StateChanged += OnPartyStateChanged; AttachNetwork(_partyNode.Network);
    }
    private void AttachNetwork(PlayFabPartyNetwork network)
    {
        ...
        _network = network; _peer = _network?.LocalPeer; if (_peer != null) { _peer.TextMessageReceived += OnTextReceived; _peer.ChatControlAdded += OnChatControlAdded; _peer.ChatControlRemoved += OnChatControlRemoved; } RefreshPeers();
    }
    ...
    private void OnChatControlAdded(int peerId, PlayFabPartyChatControl control) => RefreshPeers();
    private void OnChatControlRemoved(int peerId) => RefreshPeers();
