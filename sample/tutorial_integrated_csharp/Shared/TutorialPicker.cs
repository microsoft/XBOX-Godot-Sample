...
/// <summary>
/// Tutorial picker — default scene for the integrated C# tutorial track.
///
/// Lists only the integrated scenes (I1 sign-in and I2 integration tech demo),
/// pre-flights both MicrosoftGame.config and PlayFab title id, and drains
/// persistent Lobby/Party sessions before opening a new tutorial.
/// </summary>
public partial class TutorialPicker : Control
{
    private readonly (string Label, string Scene, bool NeedsAuth)[] _tutorials =
    {
        ("I1 — Sign in (Xbox → PlayFab)", "res://i01_signin.tscn", false),
        ("I2 — Integration tech demo", "res://I02Integration/i02_integration.tscn", true),
    };
    ...
    private Godot.Collections.Array DetectConfigProblems()
    {
        ...
        if (string.IsNullOrEmpty(pfTitle))
        {
            problems.Add(new Godot.Collections.Dictionary
            {
                ...
                ["fix"] = "Open Project → Project Settings → PlayFab → Runtime → Title Id, paste your PlayFab title id, then relaunch. The repo script tools\\setup_sample.ps1 can write this for you.",
            });
        }
        ...
        if (!Godot.FileAccess.FileExists(configPath))
        {
            problems.Add(new Godot.Collections.Dictionary
            {
                ...
                ["fix"] = "Copy MicrosoftGame.config.template to MicrosoftGame.config in this folder and fill in Partner Center values, OR run pwsh -File tools\\setup_sample.ps1 from the repo root.",
            });
        }
        else
        {
    ...
    private bool HasProblemOfKind(Godot.Collections.Array problems, string kind)
    {
        foreach (Variant problemValue in problems)
            if (TutorialSupport.DictString(problemValue.AsGodotDictionary(), "kind") == kind) return true;
        return false;
    }
    ...
    private void OnAuthStateChanged(Auth.State state)
    {
        ...
        {
            _status.Text = $"Sign-in failed ({_auth.GetLastErrorStage()}): {_auth.GetLastErrorMessage()} — I1 is still available.";
            SetSigninGated(true);
        }
        else
        {
