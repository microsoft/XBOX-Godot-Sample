...
func _build_packaging_ui(root: VBoxContainer) -> void:
	# ── Export ──
	_add_section_header(root, "Export")
	...
	_register_btn.text = "Register Loose"
...
func _populate_preset_selector() -> void:
	...
	if not EditorInterface.has_method("get_editor_export"):
		_preset_selector.add_item("Export presets unavailable")
		_export_btn.disabled = true
		_export_register_btn.disabled = true
		_export_package_btn.disabled = true
		return
	var export_mgr = EditorInterface.call("get_editor_export")
	var count = export_mgr.call("get_export_preset_count")
	var found_any := false
	for i in count:
		var preset = export_mgr.call("get_export_preset", i)
		var platform = preset.get_platform()
		if platform != null and platform.get_os_name() == "Windows":
			_preset_selector.add_item(preset.get_name(), i)
			found_any = true
	if not found_any:
...
func _on_export() -> void:
	...
	var preset_idx = _preset_selector.get_selected_id()
	if not EditorInterface.has_method("get_editor_export"):
		_export_status_label.text = "❌ Export API unavailable"
		return
	var export_mgr = EditorInterface.call("get_editor_export")
	if preset_idx < 0 or preset_idx >= export_mgr.call("get_export_preset_count"):
		_export_status_label.text = "❌ No valid export preset selected"
		return

	var preset = export_mgr.call("get_export_preset", preset_idx)
	var platform = preset.get_platform()
	if platform == null:
		_export_status_label.text = "❌ Export platform not found"
		return
	...
	_log("Exporting to: %s" % exe_path)
	var err = platform.export_project(preset, true, exe_path)
	...
	if err == OK:
		...
		_export_status_label.text = "❌ Export failed: %s" % error_string(err)
		_log("Export failed: %s" % error_string(err))
		push_error("[GDK] Export failed: %s" % error_string(err))
