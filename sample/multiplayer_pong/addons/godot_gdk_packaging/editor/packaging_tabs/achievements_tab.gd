...
func setup(coordinator: Variant) -> void:
	...
	var ach_btn_row: HBoxContainer = HBoxContainer.new()
	root.add_child(ach_btn_row)

	var achievement_save_btn: Button = Button.new()
...
func load_config() -> void:
	...
	if cfg.load(SAMPLE_CONFIG_PATH) == OK:
		var val: Variant = cfg.get_value("achievements", "demo_achievement_id", "")
...
func _on_save() -> void:
	...
	var err: Error = cfg.save(SAMPLE_CONFIG_PATH)
	if err == OK:
