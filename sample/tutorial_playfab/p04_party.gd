...
## a lobby first; once the Party autoload observes the lobby it
## creates/joins the network and exposes voice + text chat + RPC.
##
## Uses get_node_or_null indirection for the GDScript autoloads
## (PlayFabAuth, Lobby, Party) so the parse gate (which does not load
## autoloads) still resolves cleanly.
##
