...
## PlayFab Tutorial 2 reference scene — record a statistic + read its PlayFab
## leaderboard.
##
...
##   - Print the friend leaderboard (Step 5)
##
...
##
## Source: docs/tutorials/playfab/02-leaderboard.md
...
func _ready() -> void:
	...
	if _auth == null:
		_append("[color=red]PlayFabAuth autoload missing.[/color]")
...
func _print_friend_leaderboard() -> void:
	...
	var page: Dictionary = result.data
	var rankings: Array = page.get("rankings", [])
	_append("[Lead] Friend leaderboard: %d row(s)" % rankings.size())
	for entry in rankings:
		var row: Dictionary = entry
		_append("[Lead]   #%d  %s — %d" % [row.get("rank", 0), _display_name(row), _primary_score(row)])
