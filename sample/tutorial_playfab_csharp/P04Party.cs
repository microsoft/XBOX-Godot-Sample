...
/// <summary>PlayFab Tutorial 4 reference scene — Party voice + text + RPC on a lobby.</summary>
public partial class P04Party : Control
{
    ...
    public override async void _Ready()
    {
        ...
        _pingButton.Disabled = true;

        if (_auth == null || _lobbyNode == null || _partyNode == null)
        {
            _statusLabel.Text = "[ERR] PlayFabAuth/Lobby/Party autoload missing";
            return;
        }
        ...
        if (!signedIn)
        {
            _statusLabel.Text = $"Sign-in failed ({_auth.GetLastErrorStage()}): {_auth.GetLastErrorMessage()}";
            return;
        }
        ...
        _partyNode.PeerConnected += OnPeerConnected;
        _partyNode.PeerDisconnected += OnPeerDisconnected;
        _partyNode.ChatReceived += OnChatReceived;
        _partyNode.RpcReceived += OnRpcReceived;
    ...
    private void SetButtonsForState(bool signedIn)
    {
        _hostButton.Disabled = !signedIn;
        _joinButton.Disabled = !signedIn;
        _leaveButton.Disabled = true;
    }

    private void AppendLog(string line) => _chatLog.Text += line + "\n";

    private async Task OnHostPressed()
    {
        _statusLabel.Text = "Hosting lobby + Party network...";
        _hostButton.Disabled = true;
        _joinButton.Disabled = true;
        await _lobbyNode.HostLobbyAsync();
        if (!IsInsideTree()) return;
    }
    ...
    private async Task OnJoinPressed()
    {
        ...
        {
            _statusLabel.Text = "Paste a lobby connection string before Join.";
            return;
        }
        _statusLabel.Text = "Joining lobby + Party network...";
        _hostButton.Disabled = true;
        _joinButton.Disabled = true;
        await _lobbyNode.JoinLobbyAsync(connection);
        if (!IsInsideTree()) return;
    }
    ...
    private async Task OnLeavePressed()
    {
        ...
        if (_partyNode.GetCurrentNetwork() != null)
        {
            await _partyNode.LeavePartyAsync();
            if (!IsInsideTree()) return;
        }
    ...
    private async Task OnSendPressed()
    {
        ...
        _chatInput.Clear();
        bool sent = await _partyNode.SendChatAsync(text);
        if (!IsInsideTree()) return;
        AppendLog(sent ? "you> " + text : "[send failed]");
    }
    ...
    private async void OnLobbyJoined(PlayFabLobby lobby)
    {
        ...
        _joinLobbyId.SelectAll();
        _joinLobbyId.GrabFocus();
        _leaveButton.Disabled = false;
        if (_auth.PlayFabUser != null && lobby.IsOwner(_auth.PlayFabUser))
        {
            await _partyNode.HostPartyAsync();
            if (!IsInsideTree()) return;
        }
    }
    ...
    {
        _statusLabel.Text = "Disconnected from lobby (kicked or network error).";
        _hostButton.Disabled = false;
        _joinButton.Disabled = false;
        _leaveButton.Disabled = true;
        _sendButton.Disabled = true;
    ...
    private void OnNetworkJoined(PlayFabPartyNetwork network)
    {
        _networkLabel.Text = $"Network: {network.NetworkId}";
        _sendButton.Disabled = false;
        _pingButton.Disabled = false;
        _statusLabel.Text = "Party network up. Voice/text chat active.";
    }

    private void OnNetworkLeft()
    {
        _networkLabel.Text = "Network: (none)";
        _sendButton.Disabled = true;
        _pingButton.Disabled = true;
    ...
    {
        _networkLabel.Text = "Network: (lost)";
        _sendButton.Disabled = true;
        _pingButton.Disabled = true;
        _statusLabel.Text = "Party network destroyed (lobby host left, network error, or shutdown).";
    }

    private void OnLobbyStateChanged(Lobby.State state)
    {
        if (state == Lobby.State.Hosting) _statusLabel.Text = "Hosting lobby…";
        else if (state == Lobby.State.Joining) _statusLabel.Text = "Joining lobby…";
        else if (state == Lobby.State.Leaving) _statusLabel.Text = "Leaving lobby…";
    }

    private void OnPartyStateChanged(Party.State state)
    {
        if (state == Party.State.Hosting) _statusLabel.Text = "Bringing up Party network…";
        else if (state == Party.State.Joining) _statusLabel.Text = "Joining Party network…";
        else if (state == Party.State.Leaving) _statusLabel.Text = "Tearing down Party network…";
    }

    private void OnPeerConnected(int id) => AppendLog($"[peer connected] id={id}");
    private void OnPeerDisconnected(int id) => AppendLog($"[peer left] id={id}");
    private void OnChatReceived(string id, string text) => AppendLog($"{id}> {text}");
    private void OnRpcReceived(int id, string text) => AppendLog($"peer {id} (rpc)> {text}");
