...
public partial class P02Leaderboard : Control
{
    ...
    public override async void _Ready()
    {
        ...
        _friendBtn.Pressed += async () => await PrintFriendLeaderboardAsync();

        _auth = GetNodeOrNull<PlayFabAuth>("/root/PlayFabAuth");
        if (_auth == null || !PlayFab.IsAvailable)
        {
            Append("[color=red]PlayFabAuth autoload or PlayFab extension missing.[/color]");
    ...
    private async Task RecordScoreAsync(int score)
    {
        ...
        {
            ["statistics"] = new Godot.Collections.Array
            {
                new Godot.Collections.Dictionary
                {
                    ["name"] = StatisticName,
                    ["scores"] = new Godot.Collections.Array { score.ToString() },
                },
            },
        };
    ...
    private void PrintRow(Godot.Collections.Dictionary row) =>
        Append($"[Lead]   #{TutorialSupport.DictInt(row, "rank")}  {TutorialSupport.DisplayName(row)} — {TutorialSupport.PrimaryScore(row)}");

    private void SetButtonsEnabled(bool enabled)
    {
        _submitBtn.Disabled = !enabled;
        _topBtn.Disabled = !enabled;
        _pagesBtn.Disabled = !enabled;
        _aroundBtn.Disabled = !enabled;
        _friendBtn.Disabled = !enabled;
    }
