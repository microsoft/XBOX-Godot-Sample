...
String XboxServices::_build_default_scid(uint32_t p_title_id) {
    char buffer[37];
    std::snprintf(buffer, sizeof(buffer), "00000000-0000-0000-0000-0000%08x", p_title_id);
    return String(buffer);
}

String XboxServices::_extract_scid_override(const Variant &p_config) {
...
XboxServices::UserContextState *XboxServices::_find_user_context(XUserLocalId p_local_id) {
...
HRESULT XboxServices::_ensure_user_context(const Ref<XboxUser> &p_user, UserContextState **r_context_state) {
...
XboxServices::~XboxServices() {
    shutdown();
}

Ref<XboxResult> XboxServices::initialize(XTaskQueueHandle p_queue, const Variant &p_config) {
    shutdown();
    ...
    if (FAILED(hr)) {
        return XboxResult::hresult_error(hr, "Failed to initialize Xbox services.", "xbox_services_initialize_failed");
    }
...
bool XboxServices::is_initialized() const {
    return m_initialized;
}

uint32_t XboxServices::get_title_id() const {
    return m_title_id;
}

String XboxServices::get_scid() const {
    return m_scid;
}

HRESULT XboxServices::get_xbox_user_id(const Ref<XboxUser> &p_user, uint64_t *r_xbox_user_id) {
...
HRESULT XboxServices::duplicate_context_for_user(const Ref<XboxUser> &p_user, XblContextHandle *r_context, uint64_t *r_xbox_user_id) {
...
void XboxServices::forget_user(XUserLocalId p_local_id) {
