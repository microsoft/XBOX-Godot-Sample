#ifndef XBOX_ACHIEVEMENT_H
#define XBOX_ACHIEVEMENT_H

#ifndef WIN32_LEAN_AND_MEAN
#define WIN32_LEAN_AND_MEAN
#endif
#include <windows.h>

#include <vector>

#include <godot_cpp/classes/ref.hpp>
#include <godot_cpp/classes/ref_counted.hpp>
#include <godot_cpp/core/class_db.hpp>
#include <godot_cpp/variant/array.hpp>
#include <godot_cpp/variant/string.hpp>

#include <XUser.h>
#include <xsapi-c/services_c.h>

#include "xbox_pending_signal.h"

namespace godot {

class Xbox;
class XboxResult;
class XboxRuntime;
class XboxUser;
class XboxServices;

class XboxAchievement : public RefCounted {
    GDCLASS(XboxAchievement, RefCounted);

    String m_id;
    String m_name;
    String m_service_configuration_id;
    String m_progress_state;
    int64_t m_progress_percent = 0;
    bool m_unlocked = false;
    bool m_secret = false;
    String m_locked_description;
    String m_unlocked_description;

protected:
    static void _bind_methods();

public:
    String get_id() const;
    String get_name() const;
    String get_service_configuration_id() const;
    String get_progress_state() const;
    int64_t get_progress_percent() const;
    bool is_unlocked() const;
    bool is_secret() const;
    String get_locked_description() const;
    String get_unlocked_description() const;

    bool matches_id(const String &p_id) const;
    void populate_from_native(const XblAchievement &p_native_achievement);
};

class XboxAchievements : public RefCounted {
    GDCLASS(XboxAchievements, RefCounted);

    struct UserState {
        Ref<XboxUser> user;
        uint64_t xbox_user_id = 0;
        bool manager_added = false;
        bool initialized = false;
        std::vector<Ref<XboxAchievement>> achievements;
    };

    struct PendingQueryOp {
        uint64_t xbox_user_id = 0;
        Ref<XboxPendingSignal> request;
    };

    struct PendingUpdateOp {
        uint64_t xbox_user_id = 0;
        String achievement_id;
        uint32_t percent_complete = 0;
        bool submitted = false;
        Ref<XboxPendingSignal> request;
    };

    Xbox *m_owner = nullptr;
    bool m_runtime_ready = false;
    std::vector<UserState> m_user_states;
    std::vector<PendingQueryOp> m_pending_query_ops;
    std::vector<PendingUpdateOp> m_pending_update_ops;

    XboxRuntime *_get_runtime() const;
    XboxServices *_get_xbox_services() const;
    Signal _make_completed_signal(const Ref<XboxResult> &p_result) const;
    Signal _make_error_signal(HRESULT p_hresult, const String &p_code, const String &p_message) const;
    UserState *_find_user_state_by_xuid(uint64_t p_xbox_user_id);
    UserState *_find_user_state_by_local_id(XUserLocalId p_local_id);
    Ref<XboxAchievement> _find_cached_achievement(const UserState &p_state, const String &p_achievement_id) const;
    Array _get_cached_achievements_array(const UserState &p_state) const;
    Ref<XboxResult> _ensure_user_state(const Ref<XboxUser> &p_user, UserState **r_state);
    Ref<XboxResult> _refresh_user_cache(UserState &p_state);
    Ref<XboxResult> _refresh_single_achievement(UserState &p_state, const String &p_achievement_id);
    Ref<XboxResult> _submit_update(PendingUpdateOp &p_pending_update);
    void _complete_pending_queries(UserState &p_state);
    void _fail_pending_queries(uint64_t p_xbox_user_id, const Ref<XboxResult> &p_result);
    void _complete_pending_updates(UserState &p_state, const String &p_achievement_id);
    void _fail_pending_updates(uint64_t p_xbox_user_id, const Ref<XboxResult> &p_result);
    void _cancel_pending_query_signal(XboxPendingSignal *p_request);
    void _cancel_pending_update_signal(XboxPendingSignal *p_request);
    void _submit_waiting_updates(UserState &p_state);
    void _erase_completed_updates();
    void _erase_user_state(uint64_t p_xbox_user_id);

protected:
    static void _bind_methods();

public:
    void set_owner(Xbox *p_owner);

    Ref<XboxResult> on_runtime_initialized();
    void shutdown();
    int dispatch();

    Signal query_player_achievements_async(const Ref<XboxUser> &p_user);
    Signal update_achievement_async(const Ref<XboxUser> &p_user, const String &p_achievement_id, int64_t p_percent_complete);
    Array get_cached_achievements(const Ref<XboxUser> &p_user) const;
    Ref<XboxResult> get_achievements_by_state(const Ref<XboxUser> &p_user, const String &p_progress_state);

    void on_user_removed(const Ref<XboxUser> &p_user);
};

} // namespace godot

#endif // XBOX_ACHIEVEMENT_H
