#ifndef XBOX_RUNTIME_H
#define XBOX_RUNTIME_H
...
class XboxRuntime {
    ...
    // only from ~XboxRuntime(). The init call resolves to
    ...
    std::vector<Ref<XboxPendingSignal>> m_active_pending_signals;
...
public:
    ...
    void retain_pending_signal(const Ref<XboxPendingSignal> &p_pending_signal);
    void release_pending_signal(XboxPendingSignal *p_pending_signal);
    Ref<XboxPendingSignal> make_pending_signal();
    Signal make_error_signal(HRESULT p_hresult, const String &p_code, const String &p_message, const Variant &p_data = Variant());
};

} // namespace godot

#endif // XBOX_RUNTIME_H
