// xbox_request_parsing.cpp — Implementation of the GDK request-input parsing
// seam.  See xbox_request_parsing.h for the contract.  This translation unit
// depends only on godot-cpp built-in types and XboxResult (no Xbox GDK service
...
Ref<XboxResult> parse_uint32(const String &p_name, int64_t p_value, uint32_t *r_value) {
    if (r_value == nullptr) {
        return XboxResult::error_result(E_POINTER, "invalid_output", "Output storage is unavailable.");
    }
    if (p_value < 0 || p_value > static_cast<int64_t>(UINT32_MAX)) {
        return XboxResult::error_result(E_INVALIDARG, String("invalid_") + p_name, p_name + String(" must fit in a non-negative 32-bit unsigned integer."));
    }
...
Ref<XboxResult> copy_utf8_to_buffer(const String &p_value, char *p_buffer, size_t p_buffer_size, const String &p_field_name) {
    if (p_buffer == nullptr || p_buffer_size == 0) {
        return XboxResult::error_result(E_POINTER, "invalid_output", "String output storage is unavailable.");
    }

    const CharString utf8 = p_value.utf8();
    const size_t length = std::strlen(utf8.get_data());
    if (length >= p_buffer_size) {
        return XboxResult::error_result(E_INVALIDARG, String("invalid_") + p_field_name, p_field_name + String(" is too long."));
    }
