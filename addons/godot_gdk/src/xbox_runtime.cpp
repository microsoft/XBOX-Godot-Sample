...
XboxRuntime::~XboxRuntime() {
    ...
    // at process exit (~XboxRuntime is reached via memdelete(gdk_singleton) in
...
Ref<XboxResult> XboxRuntime::initialize() {
    if (m_initialized) {
        Ref<XboxResult> result = XboxResult::error_result(E_FAIL, "already_initialized", "GDK runtime is already initialized.");
        return result;
    }
    ...
    if (!m_xgame_runtime_initialized) {
        ...
        if (FAILED(hr)) {
            ...
            Ref<XboxResult> result = XboxResult::hresult_error(hr, message, "runtime_initialize_failed");
            return result;
        }
    ...
    if (FAILED(hr)) {
        ...
        // Uninitialize runs from ~XboxRuntime().
        Ref<XboxResult> result = XboxResult::hresult_error(hr, "Failed to create the shared XTaskQueue.", "task_queue_create_failed");
        return result;
    }
...
void XboxRuntime::shutdown() {
    ...
    std::vector<Ref<XboxPendingSignal>> active_pending_signals = m_active_pending_signals;
    for (const Ref<XboxPendingSignal> &pending_signal : active_pending_signals) {
        if (pending_signal.is_valid()) {
            ...
            if (!pending_signal->is_done()) {
                ...
                // the cancel handler no-ops via XboxPendingSignal::complete's
                // m_done check.
                pending_signal->complete(XboxResult::cancelled("GDK runtime shutdown cancelled the async request."));
            }
        }
    }
    ...
    for (Ref<XboxPendingSignal> &pending_signal : m_active_pending_signals) {
        if (pending_signal.is_valid()) {
    ...
    // ~XboxRuntime() at extension teardown. See class-level comment.

    m_initialized = false;
    m_shutting_down = false;
}

int XboxRuntime::dispatch() {
...
bool XboxRuntime::is_initialized() const {
    return m_initialized;
}

bool XboxRuntime::is_shutting_down() const {
    return m_shutting_down;
}

bool XboxRuntime::is_available() const {
    return GDK_PLATFORM_AVAILABLE;
}

XTaskQueueHandle XboxRuntime::get_task_queue() const {
    return m_task_queue;
}

void XboxRuntime::retain_pending_signal(const Ref<XboxPendingSignal> &p_pending_signal) {
    if (!p_pending_signal.is_valid() || p_pending_signal->is_done()) {
        return;
    }

    p_pending_signal->set_release_handler([this](XboxPendingSignal *p_completed_signal) {
        release_pending_signal(p_completed_signal);
    });
    m_active_pending_signals.push_back(p_pending_signal);
}

void XboxRuntime::release_pending_signal(XboxPendingSignal *p_pending_signal) {
    m_active_pending_signals.erase(
        std::remove_if(
            m_active_pending_signals.begin(),
            m_active_pending_signals.end(),
            [p_pending_signal](const Ref<XboxPendingSignal> &candidate) {
                return candidate.is_null() || candidate.operator->() == p_pending_signal;
            }),
        m_active_pending_signals.end());
}

Ref<XboxPendingSignal> XboxRuntime::make_pending_signal() {
...
Signal XboxRuntime::make_error_signal(HRESULT p_hresult, const String &p_code, const String &p_message, const Variant &p_data) {
    Ref<XboxPendingSignal> pending_signal = make_pending_signal();
...
void CALLBACK XboxRuntime::_queue_terminated(void *p_context) {
