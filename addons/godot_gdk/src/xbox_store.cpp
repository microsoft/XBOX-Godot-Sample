...
class StoreXAsyncContext : public XboxSignalXAsyncContext {
protected:
    Ref<XboxStore> m_service;
    String m_store_id;
...
protected:
    void finalize(XAsyncBlock *p_async_block) override {
        ...
        if (get_runtime()->is_shutting_down() || get_pending_signal()->was_cancel_requested()) {
            result = XboxResult::cancelled("Store license status request cancelled.");
        ...
        if (!m_service.is_valid() || !m_service->is_runtime_ready()) {
            result = XboxResult::cancelled("XboxStore is shutting down.");
            get_pending_signal()->complete(result);
            return;
        }

        Ref<XboxStoreLicenseStatus> status;
        status.instantiate();
        status->set_values(get_store_id(), String::utf8(native_result.licensableSku), static_cast<int64_t>(native_result.status));
        Ref<XboxStoreLicenseStatus> cached_status = m_service->_cache_license_status(status);
        get_pending_signal()->complete(XboxResult::ok_result(cached_status));
    }
...
protected:
    void finalize(XAsyncBlock *p_async_block) override {
        Ref<XboxResult> result;
        if (get_runtime()->is_shutting_down() || get_pending_signal()->was_cancel_requested()) {
            result = XboxResult::cancelled("Store purchase UI request cancelled.");
            get_pending_signal()->complete(result);
            return;
        }
        ...
        if (result_hr == E_ABORT) {
            result = XboxResult::cancelled("Store purchase UI was dismissed.");
            get_pending_signal()->complete(result);
            return;
        }
        if (FAILED(result_hr)) {
...
protected:
    void finalize(XAsyncBlock *p_async_block) override {
        if (get_runtime()->is_shutting_down() || get_pending_signal()->was_cancel_requested()) {
            get_pending_signal()->complete(XboxResult::cancelled("Store product page UI request cancelled."));
            return;
        }

        HRESULT result_hr = XStoreShowProductPageUIResult(p_async_block);
        if (result_hr == E_ABORT) {
            get_pending_signal()->complete(XboxResult::cancelled("Store product page UI was dismissed."));
            return;
        }
        if (FAILED(result_hr)) {
...
protected:
    void finalize(XAsyncBlock *p_async_block) override {
        if (get_runtime()->is_shutting_down() || get_pending_signal()->was_cancel_requested()) {
            get_pending_signal()->complete(XboxResult::cancelled("Store associated products UI request cancelled."));
            return;
        }

        HRESULT result_hr = XStoreShowAssociatedProductsUIResult(p_async_block);
        if (result_hr == E_ABORT) {
            get_pending_signal()->complete(XboxResult::cancelled("Store associated products UI was dismissed."));
            return;
        }
        if (FAILED(result_hr)) {
            get_pending_signal()->complete(XboxResult::hresult_error(
...
protected:
    void finalize(XAsyncBlock *p_async_block) override {
        if (get_runtime()->is_shutting_down() || get_pending_signal()->was_cancel_requested()) {
            get_pending_signal()->complete(XboxResult::cancelled("Store rate and review UI request cancelled."));
            return;
        }
        ...
        if (result_hr == E_ABORT) {
            get_pending_signal()->complete(XboxResult::cancelled("Store rate and review UI was dismissed."));
            return;
        }
        if (FAILED(result_hr)) {
            get_pending_signal()->complete(XboxResult::hresult_error(
...
protected:
    void finalize(XAsyncBlock *p_async_block) override {
        if (get_runtime()->is_shutting_down() || get_pending_signal()->was_cancel_requested()) {
            get_pending_signal()->complete(XboxResult::cancelled("Store redeem token UI request cancelled."));
            return;
        }

        HRESULT result_hr = XStoreShowRedeemTokenUIResult(p_async_block);
        if (result_hr == E_ABORT) {
            get_pending_signal()->complete(XboxResult::cancelled("Store redeem token UI was dismissed."));
            return;
        }
        if (FAILED(result_hr)) {
            get_pending_signal()->complete(XboxResult::hresult_error(
        ...
        get_pending_signal()->complete(XboxResult::ok_result(Dictionary()));
    }
...
protected:
    void finalize(XAsyncBlock *p_async_block) override {
        if (get_runtime()->is_shutting_down() || get_pending_signal()->was_cancel_requested()) {
            get_pending_signal()->complete(XboxResult::cancelled("Store gifting UI request cancelled."));
            return;
        }

        HRESULT result_hr = XStoreShowGiftingUIResult(p_async_block);
        if (result_hr == E_ABORT) {
            get_pending_signal()->complete(XboxResult::cancelled("Store gifting UI was dismissed."));
            return;
        }
        if (FAILED(result_hr)) {
            get_pending_signal()->complete(XboxResult::hresult_error(
...
void XboxStoreLicenseStatus::_bind_methods() {
    ClassDB::bind_method(D_METHOD("get_store_id"), &XboxStoreLicenseStatus::get_store_id);
    ClassDB::bind_method(D_METHOD("get_licensable_sku"), &XboxStoreLicenseStatus::get_licensable_sku);
    ClassDB::bind_method(D_METHOD("get_status"), &XboxStoreLicenseStatus::get_status);
...
String XboxStoreLicenseStatus::get_store_id() const {
    return m_store_id;
}

String XboxStoreLicenseStatus::get_licensable_sku() const {
    return m_licensable_sku;
}

int64_t XboxStoreLicenseStatus::get_status() const {
    return m_status;
}

void XboxStoreLicenseStatus::set_values(const String &p_store_id, const String &p_licensable_sku, int64_t p_status) {
    m_store_id = p_store_id;
    m_licensable_sku = p_licensable_sku;
    m_status = p_status;
}

void XboxStore::_bind_methods() {
    ClassDB::bind_method(D_METHOD("query_license_status_async", "user", "store_id"), &XboxStore::query_license_status_async);
    ClassDB::bind_method(D_METHOD("refresh_entitlements_async", "user", "store_id"), &XboxStore::refresh_entitlements_async);
    ClassDB::bind_method(D_METHOD("show_purchase_ui_async", "user", "store_id"), &XboxStore::show_purchase_ui_async);
    ClassDB::bind_method(D_METHOD("show_product_page_ui_async", "user", "store_id"), &XboxStore::show_product_page_ui_async);
    ClassDB::bind_method(D_METHOD("show_associated_products_ui_async", "user", "store_id", "product_kinds"), &XboxStore::show_associated_products_ui_async, DEFVAL(String()));
    ClassDB::bind_method(D_METHOD("show_rate_and_review_ui_async", "user"), &XboxStore::show_rate_and_review_ui_async);
    ClassDB::bind_method(D_METHOD("show_redeem_token_ui_async", "user", "token", "allowed_store_ids", "disallow_csv_redemption"), &XboxStore::show_redeem_token_ui_async, DEFVAL(PackedStringArray()), DEFVAL(false));
    ClassDB::bind_method(D_METHOD("show_gifting_ui_async", "user", "store_id", "name", "extended_json"), &XboxStore::show_gifting_ui_async, DEFVAL(String()), DEFVAL(String()));
    ClassDB::bind_method(D_METHOD("get_cached_license_status", "store_id"), &XboxStore::get_cached_license_status);
    ClassDB::bind_method(D_METHOD("check_cached_license_status", "store_id"), &XboxStore::check_cached_license_status);
}
...
void XboxStore::set_owner(Xbox *p_owner) {
    m_owner = p_owner;
}

Ref<XboxResult> XboxStore::on_runtime_initialized() {
...
bool XboxStore::is_runtime_ready() const {
    return m_runtime_ready;
}

void XboxStore::on_user_removed(const Ref<XboxUser> &p_user) {
    (void)p_user;
    m_cached_license_status.clear();
}

Signal XboxStore::query_license_status_async(const Ref<XboxUser> &p_user, const String &p_store_id) {
    return _start_license_status_async(p_user, p_store_id, false);
}

Signal XboxStore::refresh_entitlements_async(const Ref<XboxUser> &p_user, const String &p_store_id) {
    return _start_license_status_async(p_user, p_store_id, true);
}

Signal XboxStore::show_purchase_ui_async(const Ref<XboxUser> &p_user, const String &p_store_id) {
    ...
    if (!p_user.is_valid() || p_user->get_handle() == nullptr) {
        return _make_error_signal(E_INVALIDARG, "invalid_user", "A signed-in XboxUser is required.");
    }
...
bool XboxStore::_prepare_store_ui_call(const Ref<XboxUser> &p_user, XboxRuntime *&r_runtime, XStoreContextHandle &r_store_context, Signal &r_error_signal) {
    ...
    if (!p_user.is_valid() || p_user->get_handle() == nullptr) {
        r_error_signal = _make_error_signal(E_INVALIDARG, "invalid_user", "A signed-in XboxUser is required.");
        return false;
    }
...
Signal XboxStore::show_product_page_ui_async(const Ref<XboxUser> &p_user, const String &p_store_id) {
    const String store_id = _normalize_store_id(p_store_id);
    if (store_id.is_empty()) {
        return _make_error_signal(E_INVALIDARG, "invalid_product_id", "A non-empty Store product ID is required.");
    }

    XboxRuntime *runtime = nullptr;
...
Signal XboxStore::show_associated_products_ui_async(const Ref<XboxUser> &p_user, const String &p_store_id, const String &p_product_kinds) {
    const String store_id = _normalize_store_id(p_store_id);
    if (store_id.is_empty()) {
        return _make_error_signal(E_INVALIDARG, "invalid_product_id", "A non-empty Store product ID is required.");
    }
...
Signal XboxStore::show_rate_and_review_ui_async(const Ref<XboxUser> &p_user) {
    XboxRuntime *runtime = nullptr;
    XStoreContextHandle store_context = nullptr;
    Signal error_signal;
    if (!_prepare_store_ui_call(p_user, runtime, store_context, error_signal)) {
...
Signal XboxStore::show_redeem_token_ui_async(const Ref<XboxUser> &p_user, const String &p_token, const PackedStringArray &p_allowed_store_ids, bool p_disallow_csv_redemption) {
...
Signal XboxStore::show_gifting_ui_async(const Ref<XboxUser> &p_user, const String &p_store_id, const String &p_name, const String &p_extended_json) {
    const String store_id = _normalize_store_id(p_store_id);
    if (store_id.is_empty()) {
        return _make_error_signal(E_INVALIDARG, "invalid_product_id", "A non-empty Store product ID is required.");
    }
...
Ref<XboxStoreLicenseStatus> XboxStore::get_cached_license_status(const String &p_store_id) const {
    return _find_cached_license_status(_normalize_store_id(p_store_id));
}

Ref<XboxResult> XboxStore::check_cached_license_status(const String &p_store_id) const {
    const String store_id = _normalize_store_id(p_store_id);
    if (store_id.is_empty()) {
        return XboxResult::error_result(E_INVALIDARG, "invalid_product_id", "A non-empty Store product ID is required.");
    }

    Ref<XboxStoreLicenseStatus> status = _find_cached_license_status(store_id);
    if (!status.is_valid()) {
        return XboxResult::error_result(E_FAIL, "license_status_not_cached", "No cached license status is available for the requested Store product ID.");
    }

    return XboxResult::ok_result(status);
}

XboxRuntime *XboxStore::_get_runtime() const {
    return m_owner != nullptr ? m_owner->get_runtime() : nullptr;
}

void XboxStore::_close_store_context() {
...
XStoreContextHandle XboxStore::_get_or_create_store_context(HRESULT &r_hresult) {
    if (m_store_context != nullptr) {
        r_hresult = S_OK;
...
Signal XboxStore::_make_error_signal(HRESULT p_hresult, const String &p_code, const String &p_message, const Variant &p_data) const {
    XboxRuntime *runtime = _get_runtime();
    if (runtime != nullptr) {
...
Signal XboxStore::_start_license_status_async(const Ref<XboxUser> &p_user, const String &p_store_id, bool p_is_refresh) {
    const String store_id = _normalize_store_id(p_store_id);
    if (store_id.is_empty()) {
        return _make_error_signal(E_INVALIDARG, "invalid_product_id", "A non-empty Store product ID is required.");
    }
...
Ref<XboxStoreLicenseStatus> XboxStore::_find_cached_license_status(const String &p_store_id) const {
    for (const CachedLicenseStatus &entry : m_cached_license_status) {
        if (entry.store_id == p_store_id) {
            return entry.status;
        }
    }
    return Ref<XboxStoreLicenseStatus>();
}

Ref<XboxStoreLicenseStatus> XboxStore::_cache_license_status(const Ref<XboxStoreLicenseStatus> &p_status) {
...
String XboxStore::_normalize_store_id(const String &p_store_id) {
