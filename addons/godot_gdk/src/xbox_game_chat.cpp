...
void XboxGameChat::_bind_methods() {
    ClassDB::bind_method(
            D_METHOD("initialize", "max_users", "default_relationship"),
            &XboxGameChat::initialize,
            DEFVAL(16),
            DEFVAL(RELATIONSHIP_SEND_AND_RECEIVE_ALL));
    ClassDB::bind_method(D_METHOD("is_initialized"), &XboxGameChat::is_initialized);
    ClassDB::bind_method(D_METHOD("cleanup"), &XboxGameChat::cleanup);
    ClassDB::bind_method(D_METHOD("add_local_user", "user"), &XboxGameChat::add_local_user);
    ClassDB::bind_method(D_METHOD("add_remote_user", "xuid", "endpoint_id"), &XboxGameChat::add_remote_user);
    ClassDB::bind_method(D_METHOD("remove_user", "xuid"), &XboxGameChat::remove_user);
    ClassDB::bind_method(
            D_METHOD("set_communication_relationship", "local_xuid", "target_xuid", "relationship"),
            &XboxGameChat::set_communication_relationship);
    ClassDB::bind_method(D_METHOD("set_microphone_muted", "local_xuid", "muted"), &XboxGameChat::set_microphone_muted);
    ClassDB::bind_method(
            D_METHOD("set_remote_user_muted", "local_xuid", "target_xuid", "muted"),
            &XboxGameChat::set_remote_user_muted);
    ClassDB::bind_method(
            D_METHOD("set_audio_render_volume", "local_xuid", "target_xuid", "volume"),
            &XboxGameChat::set_audio_render_volume);
    ClassDB::bind_method(D_METHOD("send_text", "local_xuid", "text"), &XboxGameChat::send_text);
    ClassDB::bind_method(
            D_METHOD("synthesize_text_to_speech", "local_xuid", "text"),
            &XboxGameChat::synthesize_text_to_speech);
    ClassDB::bind_method(
            D_METHOD("process_incoming_data_frame", "source_endpoint_id", "bytes"),
            &XboxGameChat::process_incoming_data_frame);
    ClassDB::bind_method(D_METHOD("get_chat_users"), &XboxGameChat::get_chat_users);
...
void XboxGameChat::set_owner(Xbox *p_owner) {
    m_owner = p_owner;
}

XboxRuntime *XboxGameChat::_get_runtime() const {
    return m_owner == nullptr ? nullptr : m_owner->get_runtime();
}

void *XboxGameChat::_find_user(const String &p_xuid) const {
...
Ref<XboxResult> XboxGameChat::on_runtime_initialized() {
...
void XboxGameChat::shutdown() {
    cleanup();
    m_runtime_ready = false;
}

int XboxGameChat::dispatch() {
    if (!m_initialized) {
        return 0;
    }
...
int XboxGameChat::_pump_data_frames_count() {
...
int XboxGameChat::_pump_state_changes_count() {
...
Ref<XboxResult> XboxGameChat::initialize(int p_max_users, int p_default_relationship) {
    XboxRuntime *runtime = _get_runtime();
...
bool XboxGameChat::is_initialized() const {
    return m_initialized;
}

void XboxGameChat::cleanup() {
    if (!m_initialized) {
        return;
    }
...
Ref<XboxResult> XboxGameChat::add_local_user(const Ref<XboxUser> &p_user) {
    if (!m_initialized) {
        return XboxResult::error_result(E_FAIL, "not_initialized", "Call GDK.game_chat.initialize() first.");
    }
    if (!p_user.is_valid() || p_user->get_handle() == nullptr || !p_user->is_signed_in()) {
        return XboxResult::error_result(E_INVALIDARG, "invalid_user", "A signed-in XboxUser is required to add a local chat user.");
    }

    const String xuid = p_user->get_xuid();
    if (xuid.is_empty()) {
        return XboxResult::error_result(E_INVALIDARG, "invalid_user", "The XboxUser does not have a valid Xbox User ID.");
    }
    ...
    if (user == nullptr) {
        return XboxResult::error_result(E_FAIL, "add_user_failed", "Game chat could not add the local user.");
    }
...
Ref<XboxResult> XboxGameChat::add_remote_user(const String &p_xuid, int64_t p_endpoint_id) {
    if (!m_initialized) {
        return XboxResult::error_result(E_FAIL, "not_initialized", "Call GDK.game_chat.initialize() first.");
    }
    if (p_xuid.is_empty()) {
        return XboxResult::error_result(E_INVALIDARG, "invalid_xuid", "A non-empty Xbox User ID is required.");
    }
    ...
    if (user == nullptr) {
        return XboxResult::error_result(E_FAIL, "add_user_failed", "Game chat could not add the remote user.");
    }
...
Ref<XboxResult> XboxGameChat::remove_user(const String &p_xuid) {
    if (!m_initialized) {
        return XboxResult::error_result(E_FAIL, "not_initialized", "Call GDK.game_chat.initialize() first.");
    }
    ...
    if (user == nullptr) {
        return XboxResult::error_result(E_INVALIDARG, "user_not_found", "No chat user matches the supplied Xbox User ID.");
    }
...
void XboxGameChat::on_user_removed(const Ref<XboxUser> &p_user) {
...
Ref<XboxResult> XboxGameChat::set_communication_relationship(const String &p_local_xuid, const String &p_target_xuid, int p_relationship) {
    if (!m_initialized) {
        return XboxResult::error_result(E_FAIL, "not_initialized", "Call GDK.game_chat.initialize() first.");
    }
    ...
    if (local == nullptr) {
        return XboxResult::error_result(E_INVALIDARG, "user_not_found", "No chat user matches the local Xbox User ID.");
    }
    chat_user::chat_user_local *local_state = local->local();
    if (local_state == nullptr) {
        return XboxResult::error_result(E_INVALIDARG, "not_local_user", "The local Xbox User ID does not refer to a local chat user.");
    }
    auto *target = static_cast<chat_user *>(_find_user(p_target_xuid));
    if (target == nullptr) {
        return XboxResult::error_result(E_INVALIDARG, "target_not_found", "No chat user matches the target Xbox User ID.");
    }
...
Ref<XboxResult> XboxGameChat::set_microphone_muted(const String &p_local_xuid, bool p_muted) {
    if (!m_initialized) {
        return XboxResult::error_result(E_FAIL, "not_initialized", "Call GDK.game_chat.initialize() first.");
    }
...
Ref<XboxResult> XboxGameChat::set_remote_user_muted(const String &p_local_xuid, const String &p_target_xuid, bool p_muted) {
    if (!m_initialized) {
        return XboxResult::error_result(E_FAIL, "not_initialized", "Call GDK.game_chat.initialize() first.");
    }
...
Ref<XboxResult> XboxGameChat::set_audio_render_volume(const String &p_local_xuid, const String &p_target_xuid, float p_volume) {
    if (!m_initialized) {
        return XboxResult::error_result(E_FAIL, "not_initialized", "Call GDK.game_chat.initialize() first.");
    }
...
Ref<XboxResult> XboxGameChat::send_text(const String &p_local_xuid, const String &p_text) {
    if (!m_initialized) {
        return XboxResult::error_result(E_FAIL, "not_initialized", "Call GDK.game_chat.initialize() first.");
    }
...
Ref<XboxResult> XboxGameChat::synthesize_text_to_speech(const String &p_local_xuid, const String &p_text) {
    if (!m_initialized) {
        return XboxResult::error_result(E_FAIL, "not_initialized", "Call GDK.game_chat.initialize() first.");
    }
    if (p_text.is_empty()) {
        return XboxResult::error_result(E_INVALIDARG, "invalid_text", "Text to synthesize must be non-empty.");
    }
...
Ref<XboxResult> XboxGameChat::process_incoming_data_frame(int64_t p_source_endpoint_id, const PackedByteArray &p_bytes) {
    if (!m_initialized) {
        return XboxResult::error_result(E_FAIL, "not_initialized", "Call GDK.game_chat.initialize() first.");
    }
    if (p_bytes.is_empty()) {
        return XboxResult::error_result(E_INVALIDARG, "invalid_data", "The incoming data frame must contain at least one byte.");
    }
    if (p_bytes.size() > static_cast<int64_t>(UINT32_MAX)) {
        return XboxResult::error_result(E_INVALIDARG, "invalid_data", "The incoming data frame exceeds the maximum supported size (UINT32_MAX bytes).");
    }
...
Array XboxGameChat::get_chat_users() const {
    Array result;
    if (!m_initialized) {
        return result;
    }
