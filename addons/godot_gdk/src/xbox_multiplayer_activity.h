#ifndef XBOX_MULTIPLAYER_ACTIVITY_H
#define XBOX_MULTIPLAYER_ACTIVITY_H
...
class XboxMultiplayerActivityInfo : public RefCounted {
    GDCLASS(XboxMultiplayerActivityInfo, RefCounted);
...
class XboxMultiplayerActivity : public RefCounted {
    GDCLASS(XboxMultiplayerActivity, RefCounted);

    struct CachedActivityState {
        String xuid;
        Ref<XboxMultiplayerActivityInfo> info;
    };
...
public:
    ...
    Signal get_activities_async(const Ref<XboxUser> &p_user, const PackedStringArray &p_xuids);
    Ref<XboxMultiplayerActivityInfo> get_cached_activity(const String &p_xuid) const;
    Signal delete_activity_async(const Ref<XboxUser> &p_user);
    ...
    Signal show_invite_ui_async(const Ref<XboxUser> &p_user);
    Ref<XboxResult> update_recent_players(
            const Ref<XboxUser> &p_user,
            const PackedStringArray &p_xuids,
            const String &p_encounter_type = "default");
    Signal flush_recent_players_async(const Ref<XboxUser> &p_user);
    Ref<XboxResult> accept_pending_invite(const String &p_invite_uri);

    XboxRuntime *get_runtime_internal() const;
    XboxServices *get_xbox_services_internal() const;
    Signal make_completed_signal_internal(const Ref<XboxResult> &p_result) const;
    Signal make_error_signal_internal(
            HRESULT p_hresult,
            const String &p_code,
            const String &p_message,
            const Variant &p_data = Variant()) const;
    Ref<XboxResult> duplicate_context_for_user_internal(
            const Ref<XboxUser> &p_user,
            XblContextHandle *r_context,
            uint64_t *r_xbox_user_id = nullptr) const;
    Ref<XboxMultiplayerActivityInfo> cache_activity_internal(const Ref<XboxMultiplayerActivityInfo> &p_info);
...
#endif // XBOX_MULTIPLAYER_ACTIVITY_H
