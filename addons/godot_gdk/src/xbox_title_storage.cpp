...
bool _try_parse_xuid(const String &p_xuid, uint64_t *r_xuid) {
    return xbox_request_parsing::try_parse_xuid(p_xuid, r_xuid, /*p_reject_zero=*/false);
}
...
Ref<XboxResult> _parse_uint32(const String &p_name, int64_t p_value, uint32_t *r_value) {
    return xbox_request_parsing::parse_uint32(p_name, p_value, r_value);
}

Ref<XboxResult> _copy_utf8_to_buffer(const String &p_value, char *p_buffer, size_t p_buffer_size, const String &p_field_name) {
    return xbox_request_parsing::copy_utf8_to_buffer(p_value, p_buffer, p_buffer_size, p_field_name);
}

Ref<XboxResult> _make_blob_metadata(
        const String &p_scid,
    ...
    if (r_metadata == nullptr) {
        return XboxResult::error_result(E_POINTER, "invalid_output", "Blob metadata output storage is unavailable.");
    }

    const String blob_path = p_blob_path.strip_edges();
    if (blob_path.is_empty()) {
        return XboxResult::error_result(E_INVALIDARG, "invalid_blob_path", "blob_path must be a non-empty string.");
    }
    ...
    Ref<XboxResult> copy_result = _copy_utf8_to_buffer(p_scid, metadata.serviceConfigurationId, sizeof(metadata.serviceConfigurationId), "service_configuration_id");
...
Ref<XboxTitleStorageBlobMetadata> _make_metadata_ref(const XblTitleStorageBlobMetadata &p_metadata) {
    Ref<XboxTitleStorageBlobMetadata> metadata;
...
PackedByteArray _make_packed_byte_array(const std::vector<uint8_t> &p_data) {
    return xbox_request_parsing::to_packed_byte_array(p_data);
}

std::vector<uint8_t> _make_byte_vector(const PackedByteArray &p_data) {
    return xbox_request_parsing::to_byte_vector(p_data);
}
...
class QuotaAsyncContext final : public XboxSignalXAsyncContext {
...
protected:
    void finalize(XAsyncBlock *p_async_block) override {
        if (get_runtime()->is_shutting_down() || get_pending_signal()->was_cancel_requested()) {
            Ref<XboxResult> result = XboxResult::cancelled("Title Storage quota query cancelled.");
        ...
        if (FAILED(result_hr)) {
            Ref<XboxResult> result = XboxResult::hresult_error(result_hr, "Failed to retrieve Title Storage quota.", "title_storage_quota_result_failed");
            get_pending_signal()->complete(result);
            return;
        }
...
public:
    QuotaAsyncContext(XboxRuntime *p_runtime, const Ref<XboxPendingSignal> &p_pending_signal, XblContextHandle p_context, const String &p_storage_type) :
...
class BlobMetadataAsyncContext final : public XboxSignalXAsyncContext {
    Ref<XboxUser> m_user;
    XblContextHandle m_context = nullptr;
    String m_storage_type;
    String m_blob_path;
...
protected:
    void finalize(XAsyncBlock *p_async_block) override {
        if (get_runtime()->is_shutting_down() || get_pending_signal()->was_cancel_requested()) {
            Ref<XboxResult> result = XboxResult::cancelled("Title Storage metadata query cancelled.");
            get_pending_signal()->complete(result);
            return;
        }
        ...
        if (FAILED(result_hr)) {
            Ref<XboxResult> result = XboxResult::hresult_error(result_hr, "Failed to retrieve Title Storage metadata.", "title_storage_metadata_result_failed");
            get_pending_signal()->complete(result);
            return;
        }

        Ref<XboxTitleStorageBlobMetadataResult> metadata_result;
        ...
        if (FAILED(result_hr)) {
            ...
            Ref<XboxResult> result = XboxResult::hresult_error(result_hr, "Failed to translate Title Storage metadata.", "title_storage_metadata_translate_failed");
            get_pending_signal()->complete(result);
            return;
        }

        get_pending_signal()->complete(XboxResult::ok_result(metadata_result));
    }
...
class UploadBlobAsyncContext final : public XboxSignalXAsyncContext {
    XblContextHandle m_context = nullptr;
...
protected:
    void finalize(XAsyncBlock *p_async_block) override {
        if (get_runtime()->is_shutting_down() || get_pending_signal()->was_cancel_requested()) {
            Ref<XboxResult> result = XboxResult::cancelled("Title Storage upload cancelled.");
            get_pending_signal()->complete(result);
            return;
        }
        ...
        if (FAILED(result_hr)) {
            Ref<XboxResult> result = XboxResult::hresult_error(result_hr, "Failed to upload Title Storage blob.", "title_storage_upload_result_failed");
            get_pending_signal()->complete(result);
            return;
        }

        get_pending_signal()->complete(XboxResult::ok_result(_make_metadata_ref(metadata)));
    }

public:
    UploadBlobAsyncContext(XboxRuntime *p_runtime, const Ref<XboxPendingSignal> &p_pending_signal, XblContextHandle p_context, const XblTitleStorageBlobMetadata &p_metadata, std::vector<uint8_t> p_data) :
            XboxSignalXAsyncContext(p_runtime, p_pending_signal),
            m_context(p_context),
...
class DeleteBlobAsyncContext final : public XboxSignalXAsyncContext {
    XblContextHandle m_context = nullptr;

protected:
    void finalize(XAsyncBlock *p_async_block) override {
        if (get_runtime()->is_shutting_down() || get_pending_signal()->was_cancel_requested()) {
            Ref<XboxResult> result = XboxResult::cancelled("Title Storage delete cancelled.");
            get_pending_signal()->complete(result);
            return;
        }
        ...
        if (FAILED(result_hr)) {
            Ref<XboxResult> result = XboxResult::hresult_error(result_hr, "Failed to delete Title Storage blob.", "title_storage_delete_failed");
            get_pending_signal()->complete(result);
            return;
        }

        get_pending_signal()->complete(XboxResult::ok_result());
    }

public:
    DeleteBlobAsyncContext(XboxRuntime *p_runtime, const Ref<XboxPendingSignal> &p_pending_signal, XblContextHandle p_context) :
            XboxSignalXAsyncContext(p_runtime, p_pending_signal),
            m_context(p_context) {}
...
class DownloadBlobAsyncContext final : public XboxSignalXAsyncContext {
    XblContextHandle m_context = nullptr;
    XblTitleStorageBlobMetadata m_metadata = {};
...
protected:
    void finalize(XAsyncBlock *p_async_block) override {
        if (get_runtime()->is_shutting_down() || get_pending_signal()->was_cancel_requested()) {
            Ref<XboxResult> result = XboxResult::cancelled("Title Storage download cancelled.");
            get_pending_signal()->complete(result);
            return;
        }
        ...
        if (FAILED(result_hr)) {
            Ref<XboxResult> result = XboxResult::hresult_error(result_hr, "Failed to download Title Storage blob.", "title_storage_download_result_failed");
            get_pending_signal()->complete(result);
            return;
        }
...
public:
    DownloadBlobAsyncContext(XboxRuntime *p_runtime, const Ref<XboxPendingSignal> &p_pending_signal, XblContextHandle p_context, const XblTitleStorageBlobMetadata &p_metadata) :
            XboxSignalXAsyncContext(p_runtime, p_pending_signal),
            m_context(p_context),
            m_metadata(p_metadata),
...
class DownloadMetadataAsyncContext final : public XboxSignalXAsyncContext {
    XblContextHandle m_context = nullptr;
    String m_blob_path;
...
protected:
    void finalize(XAsyncBlock *p_async_block) override {
        ...
        if (FAILED(result_hr)) {
            Ref<XboxResult> result = XboxResult::hresult_error(result_hr, "Failed to retrieve Title Storage blob metadata for download.", "title_storage_download_metadata_failed");
            get_pending_signal()->complete(result);
            return;
        }
        ...
        if (FAILED(result_hr)) {
            XblTitleStorageBlobMetadataResultCloseHandle(handle);
            Ref<XboxResult> result = XboxResult::hresult_error(result_hr, "Failed to read Title Storage blob metadata for download.", "title_storage_download_metadata_items_failed");
            get_pending_signal()->complete(result);
            return;
        }
        ...
        if (metadata == nullptr) {
            XblTitleStorageBlobMetadataResultCloseHandle(handle);
            Ref<XboxResult> result = XboxResult::error_result(E_BOUNDS, "blob_not_found", "No Title Storage blob metadata matched blob_path.");
            get_pending_signal()->complete(result);
            return;
        }
        ...
        if (FAILED(result_hr)) {
            ...
            Ref<XboxResult> result = XboxResult::hresult_error(result_hr, "Failed to start Title Storage blob download.", "title_storage_download_start_failed");
            get_pending_signal()->complete(result);
        }
    }

public:
    DownloadMetadataAsyncContext(XboxRuntime *p_runtime, const Ref<XboxPendingSignal> &p_pending_signal, XblContextHandle p_context, const String &p_blob_path, XblTitleStorageETagMatchCondition p_match_condition) :
            XboxSignalXAsyncContext(p_runtime, p_pending_signal),
            m_context(p_context),
            m_blob_path(p_blob_path),
...
void XboxTitleStorageBlobMetadata::_bind_methods() {
    ClassDB::bind_method(D_METHOD("get_blob_path"), &XboxTitleStorageBlobMetadata::get_blob_path);
    ClassDB::bind_method(D_METHOD("get_blob_type"), &XboxTitleStorageBlobMetadata::get_blob_type);
    ClassDB::bind_method(D_METHOD("get_storage_type"), &XboxTitleStorageBlobMetadata::get_storage_type);
    ClassDB::bind_method(D_METHOD("get_display_name"), &XboxTitleStorageBlobMetadata::get_display_name);
    ClassDB::bind_method(D_METHOD("get_e_tag"), &XboxTitleStorageBlobMetadata::get_e_tag);
    ClassDB::bind_method(D_METHOD("get_client_timestamp"), &XboxTitleStorageBlobMetadata::get_client_timestamp);
    ClassDB::bind_method(D_METHOD("get_length"), &XboxTitleStorageBlobMetadata::get_length);
    ClassDB::bind_method(D_METHOD("get_service_configuration_id"), &XboxTitleStorageBlobMetadata::get_service_configuration_id);
    ClassDB::bind_method(D_METHOD("get_xuid"), &XboxTitleStorageBlobMetadata::get_xuid);
...
String XboxTitleStorageBlobMetadata::get_blob_path() const {
    return m_blob_path;
}

String XboxTitleStorageBlobMetadata::get_blob_type() const {
    return m_blob_type;
}

String XboxTitleStorageBlobMetadata::get_storage_type() const {
    return m_storage_type;
}

String XboxTitleStorageBlobMetadata::get_display_name() const {
    return m_display_name;
}

String XboxTitleStorageBlobMetadata::get_e_tag() const {
    return m_e_tag;
}

int64_t XboxTitleStorageBlobMetadata::get_client_timestamp() const {
    return m_client_timestamp;
}

int64_t XboxTitleStorageBlobMetadata::get_length() const {
    return m_length;
}

String XboxTitleStorageBlobMetadata::get_service_configuration_id() const {
    return m_service_configuration_id;
}

String XboxTitleStorageBlobMetadata::get_xuid() const {
    return m_xuid;
}

void XboxTitleStorageBlobMetadata::populate_from_native(const XblTitleStorageBlobMetadata &p_metadata) {
...
void XboxTitleStorageBlobMetadataResult::_bind_methods() {
    ClassDB::bind_method(D_METHOD("get_items"), &XboxTitleStorageBlobMetadataResult::get_items);
    ClassDB::bind_method(D_METHOD("has_next"), &XboxTitleStorageBlobMetadataResult::has_next);
    ClassDB::bind_method(D_METHOD("get_storage_type"), &XboxTitleStorageBlobMetadataResult::get_storage_type);
    ClassDB::bind_method(D_METHOD("get_blob_path"), &XboxTitleStorageBlobMetadataResult::get_blob_path);
...
XboxTitleStorageBlobMetadataResult::~XboxTitleStorageBlobMetadataResult() {
    if (m_handle != nullptr) {
...
Array XboxTitleStorageBlobMetadataResult::get_items() const {
    return m_items;
}

bool XboxTitleStorageBlobMetadataResult::has_next() const {
    return m_has_next;
}

String XboxTitleStorageBlobMetadataResult::get_storage_type() const {
    return m_storage_type;
}

String XboxTitleStorageBlobMetadataResult::get_blob_path() const {
    return m_blob_path;
}

HRESULT XboxTitleStorageBlobMetadataResult::populate_from_handle(
        const Ref<XboxUser> &p_user,
        const String &p_storage_type,
        const String &p_blob_path,
        uint32_t p_max_items,
...
XblTitleStorageBlobMetadataResultHandle XboxTitleStorageBlobMetadataResult::get_handle_internal() const {
    return m_handle;
}

Ref<XboxUser> XboxTitleStorageBlobMetadataResult::get_user_internal() const {
    return m_user;
}

uint32_t XboxTitleStorageBlobMetadataResult::get_max_items_internal() const {
    return m_max_items;
}

void XboxTitleStorage::_bind_methods() {
    ClassDB::bind_method(D_METHOD("get_quota_async", "user", "storage_type"), &XboxTitleStorage::get_quota_async);
    ClassDB::bind_method(D_METHOD("list_blob_metadata_async", "user", "storage_type", "blob_path", "skip_items", "max_items"), &XboxTitleStorage::list_blob_metadata_async, DEFVAL(String()), DEFVAL(0), DEFVAL(25));
    ClassDB::bind_method(D_METHOD("get_next_blob_metadata_async", "result"), &XboxTitleStorage::get_next_blob_metadata_async);
    ClassDB::bind_method(D_METHOD("download_blob_async", "user", "storage_type", "blob_path"), &XboxTitleStorage::download_blob_async);
    ClassDB::bind_method(D_METHOD("upload_blob_async", "user", "storage_type", "blob_path", "data", "display_name", "e_tag", "match_condition"), &XboxTitleStorage::upload_blob_async, DEFVAL(String()), DEFVAL(String()), DEFVAL(String("not_used")));
    ClassDB::bind_method(D_METHOD("delete_blob_async", "user", "storage_type", "blob_path", "e_tag", "match_condition"), &XboxTitleStorage::delete_blob_async, DEFVAL(String()), DEFVAL(String("not_used")));
}

void XboxTitleStorage::set_owner(Xbox *p_owner) {
    m_owner = p_owner;
}

XboxRuntime *XboxTitleStorage::_get_runtime() const {
    return m_owner == nullptr ? nullptr : m_owner->get_runtime();
}

XboxServices *XboxTitleStorage::_get_xbox_services() const {
    return m_owner == nullptr ? nullptr : m_owner->get_xbox_services();
}

Signal XboxTitleStorage::_make_error_signal(HRESULT p_hresult, const String &p_code, const String &p_message, const Variant &p_data) const {
...
Ref<XboxResult> XboxTitleStorage::_ensure_ready_user(const Ref<XboxUser> &p_user) const {
    if (!m_runtime_ready) {
...
Ref<XboxResult> XboxTitleStorage::on_runtime_initialized() {
    m_runtime_ready = true;
    return XboxResult::ok_result();
}

void XboxTitleStorage::shutdown() {
    m_runtime_ready = false;
}

Signal XboxTitleStorage::get_quota_async(const Ref<XboxUser> &p_user, const String &p_storage_type) {
    XboxRuntime *runtime = _get_runtime();
    if (runtime == nullptr) {
        return Signal();
    }
...
Signal XboxTitleStorage::list_blob_metadata_async(const Ref<XboxUser> &p_user, const String &p_storage_type, const String &p_blob_path, int64_t p_skip_items, int64_t p_max_items) {
    XboxRuntime *runtime = _get_runtime();
    if (runtime == nullptr) {
        return Signal();
    }
    ...
    Ref<XboxResult> parse_result = _parse_uint32("skip_items", p_skip_items, &skip_items);
...
Signal XboxTitleStorage::get_next_blob_metadata_async(const Ref<XboxTitleStorageBlobMetadataResult> &p_result) {
    XboxRuntime *runtime = _get_runtime();
    if (runtime == nullptr) {
        return Signal();
    }
    ...
    Ref<XboxUser> user = p_result->get_user_internal();
...
Signal XboxTitleStorage::download_blob_async(const Ref<XboxUser> &p_user, const String &p_storage_type, const String &p_blob_path) {
    XboxRuntime *runtime = _get_runtime();
    if (runtime == nullptr) {
        return Signal();
    }
...
Signal XboxTitleStorage::upload_blob_async(const Ref<XboxUser> &p_user, const String &p_storage_type, const String &p_blob_path, const PackedByteArray &p_data, const String &p_display_name, const String &p_e_tag, const String &p_match_condition) {
    XboxRuntime *runtime = _get_runtime();
    if (runtime == nullptr) {
        return Signal();
    }
    ...
    XblTitleStorageBlobMetadata metadata = {};
    Ref<XboxResult> metadata_result = _make_blob_metadata(
...
Signal XboxTitleStorage::delete_blob_async(const Ref<XboxUser> &p_user, const String &p_storage_type, const String &p_blob_path, const String &p_e_tag, const String &p_match_condition) {
    XboxRuntime *runtime = _get_runtime();
    if (runtime == nullptr) {
        return Signal();
    }
...
void XboxTitleStorage::on_user_removed(const Ref<XboxUser> &p_user) {
    (void)p_user;
}
