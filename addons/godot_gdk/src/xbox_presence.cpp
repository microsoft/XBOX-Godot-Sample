...
XboxPresenceRecord::UserState _presence_user_state_to_enum(XblPresenceUserState p_state) {
    switch (p_state) {
        case XblPresenceUserState::Online:
            return XboxPresenceRecord::USER_STATE_ONLINE;
        case XblPresenceUserState::Away:
            return XboxPresenceRecord::USER_STATE_AWAY;
        case XblPresenceUserState::Offline:
            return XboxPresenceRecord::USER_STATE_OFFLINE;
        case XblPresenceUserState::Unknown:
        default:
            return XboxPresenceRecord::USER_STATE_UNKNOWN;
    }
}

String _presence_user_state_to_name(XboxPresenceRecord::UserState p_state) {
    switch (p_state) {
        case XboxPresenceRecord::USER_STATE_ONLINE:
            return "online";
        case XboxPresenceRecord::USER_STATE_AWAY:
            return "away";
        case XboxPresenceRecord::USER_STATE_OFFLINE:
            return "offline";
        case XboxPresenceRecord::USER_STATE_UNKNOWN:
        default:
            return "unknown";
    }
}
...
class SetPresenceAsyncContext final : public XboxSignalXAsyncContext {
    XboxPresence *m_presence = nullptr;
    Ref<XboxUser> m_user;
...
protected:
    void finalize(XAsyncBlock *p_async_block) override {
        ...
        if (get_runtime()->is_shutting_down() || get_pending_signal()->was_cancel_requested()) {
            result = XboxResult::cancelled("Presence update cancelled.");
        ...
        if (FAILED(result_hr)) {
            result = XboxResult::hresult_error(result_hr, "Failed to update presence.", "presence_update_failed");
            get_pending_signal()->complete(result);
            return;
        }

        Ref<XboxPresenceRecord> record;
        record.instantiate();
        record->set_data(
                m_user.is_valid() ? m_user->get_xuid() : String(),
                m_is_active ? XboxPresenceRecord::USER_STATE_ONLINE : XboxPresenceRecord::USER_STATE_AWAY,
                Array());
...
class GetPresenceAsyncContext final : public XboxSignalXAsyncContext {
public:
    enum QueryMode {
        MODE_XUIDS,
        MODE_SOCIAL_GROUP,
...
protected:
    void finalize(XAsyncBlock *p_async_block) override {
        ...
        if (get_runtime()->is_shutting_down() || get_pending_signal()->was_cancel_requested()) {
            result = XboxResult::cancelled("Presence query cancelled.");
            get_pending_signal()->complete(result);
            return;
        }
        ...
        if (m_mode == MODE_XUIDS && m_xuids.size() == 1) {
            ...
            if (FAILED(result_hr)) {
                result = XboxResult::hresult_error(result_hr, "Failed to retrieve the presence query result.", "presence_result_failed");
                get_pending_signal()->complete(result);
                return;
            }
            ...
            if (FAILED(result_hr)) {
                result = XboxResult::hresult_error(result_hr, "Failed to translate a presence record.", "presence_record_translate_failed");
                get_pending_signal()->complete(result);
                return;
            }
            ...
            if (FAILED(result_hr)) {
                result = XboxResult::hresult_error(result_hr, "Failed to retrieve the presence query result count.", "presence_result_count_failed");
                get_pending_signal()->complete(result);
                return;
            }
            ...
            if (result_count > 0) {
                ...
                if (FAILED(result_hr)) {
                    result = XboxResult::hresult_error(result_hr, "Failed to retrieve presence records.", "presence_results_failed");
                    get_pending_signal()->complete(result);
                    return;
                }
            }
        ...
        get_pending_signal()->complete(XboxResult::ok_result(records));
    }
...
void XboxPresenceRecord::_bind_methods() {
    ClassDB::bind_method(D_METHOD("get_xuid"), &XboxPresenceRecord::get_xuid);
    ClassDB::bind_method(D_METHOD("get_user_state"), &XboxPresenceRecord::get_user_state);
    ClassDB::bind_method(D_METHOD("get_user_state_name"), &XboxPresenceRecord::get_user_state_name);
    ClassDB::bind_method(D_METHOD("is_online"), &XboxPresenceRecord::is_online);
    ClassDB::bind_method(D_METHOD("get_title_records"), &XboxPresenceRecord::get_title_records);
...
String XboxPresenceRecord::get_xuid() const {
    return m_xuid;
}

XboxPresenceRecord::UserState XboxPresenceRecord::get_user_state() const {
    return m_user_state;
}

String XboxPresenceRecord::get_user_state_name() const {
    return _presence_user_state_to_name(m_user_state);
}

bool XboxPresenceRecord::is_online() const {
    return m_user_state == USER_STATE_ONLINE;
}

Array XboxPresenceRecord::get_title_records() const {
    return m_title_records;
}

void XboxPresenceRecord::set_data(const String &p_xuid, UserState p_user_state, const Array &p_title_records) {
    m_xuid = p_xuid;
    m_user_state = p_user_state;
    m_title_records = p_title_records;
}

HRESULT XboxPresenceRecord::populate_from_presence_record(XblPresenceRecordHandle p_record_handle) {
...
void XboxPresenceRecord::populate_from_social_manager_record(uint64_t p_xuid, const XblSocialManagerPresenceRecord &p_record) {
    Array title_records;
...
void XboxPresence::_bind_methods() {
    ClassDB::bind_method(D_METHOD("set_presence_async", "user", "state", "rich_presence"), &XboxPresence::set_presence_async, DEFVAL(Dictionary()));
    ClassDB::bind_method(D_METHOD("clear_presence_async", "user"), &XboxPresence::clear_presence_async);
    ClassDB::bind_method(D_METHOD("get_presence_async", "xuids"), &XboxPresence::get_presence_async);
    ClassDB::bind_method(D_METHOD("get_presence_for_social_group_async", "user", "social_group"), &XboxPresence::get_presence_for_social_group_async);
    ClassDB::bind_method(D_METHOD("track_presence", "user", "xuids", "title_ids"), &XboxPresence::track_presence, DEFVAL(PackedInt64Array()));
    ClassDB::bind_method(D_METHOD("stop_tracking_presence", "user", "xuids", "title_ids"), &XboxPresence::stop_tracking_presence, DEFVAL(PackedStringArray()), DEFVAL(PackedInt64Array()));
    ClassDB::bind_method(D_METHOD("get_cached_presence", "xuid"), &XboxPresence::get_cached_presence);

    ADD_SIGNAL(MethodInfo("presence_changed",
            PropertyInfo(Variant::STRING, "xuid"),
            PropertyInfo(Variant::OBJECT, "presence", PROPERTY_HINT_NONE, "", PROPERTY_USAGE_DEFAULT, "XboxPresenceRecord")));
    ADD_SIGNAL(MethodInfo("local_presence_set", PropertyInfo(Variant::OBJECT, "user", PROPERTY_HINT_NONE, "", PROPERTY_USAGE_DEFAULT, "XboxUser")));
...
void XboxPresence::set_owner(Xbox *p_owner) {
    m_owner = p_owner;
}

Ref<XboxResult> XboxPresence::on_runtime_initialized() {
    XboxRuntime *runtime = _get_runtime();
    if (runtime == nullptr || !runtime->is_initialized()) {
        return XboxResult::error_result(E_FAIL, "runtime_not_initialized", "Cannot initialize the presence service before the GDK runtime.");
    }
...
Signal XboxPresence::set_presence_async(const Ref<XboxUser> &p_user, const String &p_state, const Dictionary &p_rich_presence) {
    XboxRuntime *runtime = _get_runtime();
    if (runtime == nullptr || !runtime->is_initialized()) {
    ...
    if (!p_user.is_valid() || p_user->get_handle() == nullptr) {
        return _make_error_signal(E_INVALIDARG, "invalid_user", "A signed-in XboxUser is required for presence.");
    }
    ...
    if (FAILED(hr)) {
        Ref<XboxResult> result = XboxResult::hresult_error(hr, "Failed to resolve the Xbox services context for the presence update.", "presence_context_failed");
    ...
    if (FAILED(hr)) {
        ...
        Ref<XboxResult> result = XboxResult::hresult_error(hr, "Failed to start the presence update request.", "presence_update_start_failed");
        pending_signal->complete_deferred(result);
    }
...
Signal XboxPresence::clear_presence_async(const Ref<XboxUser> &p_user) {
    XboxRuntime *runtime = _get_runtime();
    if (runtime == nullptr || !runtime->is_initialized()) {
        return _make_error_signal(E_FAIL, "not_initialized", "GDK is not initialized. Call GDK.initialize() first.");
    }
    ...
    if (FAILED(hr)) {
        ...
        Ref<XboxResult> result = XboxResult::hresult_error(hr, "Failed to start the presence clear request.", "presence_clear_start_failed");
        pending_signal->complete_deferred(result);
    }
...
Signal XboxPresence::get_presence_async(const PackedStringArray &p_xuids) {
    XboxRuntime *runtime = _get_runtime();
    if (runtime == nullptr || !runtime->is_initialized()) {
        return _make_error_signal(E_FAIL, "not_initialized", "GDK is not initialized. Call GDK.initialize() first.");
    }
    ...
    Ref<XboxResult> parse_result = _parse_query_xuids(p_xuids, &xuids);
    ...
    Ref<XboxUser> calling_user = _get_presence_calling_user();
    ...
    if (FAILED(hr)) {
        Ref<XboxResult> result = XboxResult::hresult_error(hr, "Failed to resolve the Xbox services context for the presence query.", "presence_context_failed");
        pending_signal->complete_deferred(result);
        return pending_signal->get_completed_signal();
    }
    ...
    if (FAILED(hr)) {
        ...
        Ref<XboxResult> result = XboxResult::hresult_error(hr, "Failed to start the presence query.", "presence_query_start_failed");
        pending_signal->complete_deferred(result);
    }
...
Signal XboxPresence::get_presence_for_social_group_async(const Ref<XboxUser> &p_user, const String &p_social_group) {
    XboxRuntime *runtime = _get_runtime();
    if (runtime == nullptr) {
        return Signal();
    }
...
Ref<XboxResult> XboxPresence::track_presence(const Ref<XboxUser> &p_user, const PackedStringArray &p_xuids, const PackedInt64Array &p_title_ids) {
    Ref<XboxResult> validation = _ensure_ready_user(p_user);
    if (!validation->is_ok()) {
        return validation;
    }
    ...
    std::vector<uint64_t> xuids;
    Ref<XboxResult> parse_result = _parse_xuids(p_xuids, false, "missing_presence_xuids", "invalid_presence_xuid", &xuids);
    if (!parse_result->is_ok()) {
        return parse_result;
    }
    ...
    Ref<XboxResult> handler_result = _ensure_handler_state(p_user, &state);
    ...
    if (FAILED(hr)) {
        return XboxResult::hresult_error(hr, "Failed to track Xbox Services presence users.", "presence_track_users_failed");
    }

    if (!title_ids.empty()) {
        hr = XblPresenceTrackAdditionalTitles(state->context, title_ids.data(), title_ids.size());
        if (FAILED(hr)) {
            return XboxResult::hresult_error(hr, "Failed to track additional Xbox Services presence titles.", "presence_track_titles_failed");
        }
    }
...
Ref<XboxResult> XboxPresence::stop_tracking_presence(const Ref<XboxUser> &p_user, const PackedStringArray &p_xuids, const PackedInt64Array &p_title_ids) {
    Ref<XboxResult> validation = _ensure_ready_user(p_user);
    if (!validation->is_ok()) {
        return validation;
    }
    ...
    std::vector<uint64_t> xuids;
    Ref<XboxResult> parse_result = _parse_xuids(p_xuids, true, "missing_presence_xuids", "invalid_presence_xuid", &xuids);
    if (!parse_result->is_ok()) {
        return parse_result;
    }
    ...
    if (!xuids.empty()) {
        ...
        if (FAILED(hr)) {
            return XboxResult::hresult_error(hr, "Failed to stop tracking Xbox Services presence users.", "presence_stop_tracking_users_failed");
        }
    ...
    if (!title_ids.empty()) {
        ...
        if (FAILED(hr)) {
            return XboxResult::hresult_error(hr, "Failed to stop tracking additional Xbox Services presence titles.", "presence_stop_tracking_titles_failed");
        }
...
Ref<XboxPresenceRecord> XboxPresence::get_cached_presence(const String &p_xuid) const {
    return _find_cached_presence(p_xuid.strip_edges());
}

void XboxPresence::cache_presence_record(const Ref<XboxPresenceRecord> &p_record, bool p_emit_signal) {
    ...
    for (Ref<XboxPresenceRecord> &cached_record : m_cached_presence) {
...
void XboxPresence::on_user_removed(const Ref<XboxUser> &p_user) {
    if (!p_user.is_valid()) {
        return;
    }
...
XboxRuntime *XboxPresence::_get_runtime() const {
    return m_owner != nullptr ? m_owner->get_runtime() : nullptr;
}

XboxServices *XboxPresence::_get_xbox_services() const {
    return m_owner != nullptr ? m_owner->get_xbox_services() : nullptr;
}

Signal XboxPresence::_make_error_signal(HRESULT p_hresult, const String &p_code, const String &p_message, const Variant &p_data) const {
    return _make_presence_error_signal(_get_runtime(), p_hresult, p_code, p_message, p_data);
}

Ref<XboxResult> XboxPresence::_ensure_ready_user(const Ref<XboxUser> &p_user) const {
    XboxRuntime *runtime = _get_runtime();
    ...
    if (!p_user.is_valid() || p_user->get_handle() == nullptr || !p_user->is_signed_in()) {
        return XboxResult::error_result(E_INVALIDARG, "invalid_user", "A signed-in XboxUser is required for presence.");
    }
    return XboxResult::ok_result();
}

Ref<XboxUser> XboxPresence::_get_presence_calling_user() const {
    if (m_owner == nullptr) {
        return Ref<XboxUser>();
    }

    Ref<XboxUsers> users = m_owner->get_users();
    if (!users.is_valid()) {
        return Ref<XboxUser>();
    }
...
Ref<XboxResult> XboxPresence::_parse_query_xuids(const PackedStringArray &p_xuids, std::vector<uint64_t> *r_xuids) const {
    return _parse_xuids(p_xuids, false, "missing_presence_xuids", "invalid_presence_xuid", r_xuids);
}

Ref<XboxResult> XboxPresence::_parse_xuids(const PackedStringArray &p_xuids, bool p_allow_empty, const String &p_missing_code, const String &p_invalid_code, std::vector<uint64_t> *r_xuids) const {
    ERR_FAIL_COND_V(r_xuids == nullptr, XboxResult::error_result(E_POINTER, "internal_error", "Missing presence query output vector."));

    r_xuids->clear();
    if (p_xuids.is_empty()) {
        return p_allow_empty ? XboxResult::ok_result() : XboxResult::error_result(E_INVALIDARG, p_missing_code, "Presence operations require at least one XUID.");
    }
    ...
    for (int64_t i = 0; i < p_xuids.size(); ++i) {
        ...
        if (!_try_parse_xuid(p_xuids[i], &xuid)) {
            return XboxResult::error_result(E_INVALIDARG, p_invalid_code, "Presence operations require numeric XUID strings.");
        }
...
Ref<XboxResult> XboxPresence::_parse_title_ids(const PackedInt64Array &p_title_ids, std::vector<uint32_t> *r_title_ids) const {
    ERR_FAIL_COND_V(r_title_ids == nullptr, XboxResult::error_result(E_POINTER, "internal_error", "Missing presence title-id output vector."));
    ...
    for (int64_t i = 0; i < p_title_ids.size(); ++i) {
        ...
        if (title_id < 0 || title_id > UINT32_MAX) {
            return XboxResult::error_result(E_INVALIDARG, "invalid_title_id", "Presence title IDs must be unsigned 32-bit values.");
        }
...
Ref<XboxPresenceRecord> XboxPresence::_find_cached_presence(const String &p_xuid) const {
    for (const Ref<XboxPresenceRecord> &record : m_cached_presence) {
        if (record.is_valid() && record->get_xuid() == p_xuid) {
            return record;
        }
    }

    return Ref<XboxPresenceRecord>();
}

void XboxPresence::_remove_cached_presence(const String &p_xuid) {
    m_cached_presence.erase(
            std::remove_if(
                    m_cached_presence.begin(),
                    m_cached_presence.end(),
                    [&p_xuid](const Ref<XboxPresenceRecord> &record) {
                        return record.is_null() || record->get_xuid() == p_xuid;
                    }),
            m_cached_presence.end());
}

XboxPresence::HandlerState *XboxPresence::_find_handler_state(XUserLocalId p_local_id) {
    for (HandlerState &state : m_handler_states) {
...
Ref<XboxResult> XboxPresence::_ensure_handler_state(const Ref<XboxUser> &p_user, HandlerState **r_state) {
    ERR_FAIL_COND_V(r_state == nullptr, XboxResult::error_result(E_POINTER, "internal_error", "Missing presence handler-state output."));
    *r_state = nullptr;
    ...
    XboxServices *xbox_services = _get_xbox_services();
    if (xbox_services == nullptr || !xbox_services->is_initialized()) {
        return XboxResult::error_result(E_FAIL, "xbox_services_not_initialized", "Xbox services are unavailable. Ensure the title has a TitleId before using presence.");
    }
    ...
    if (FAILED(hr)) {
        return XboxResult::hresult_error(hr, "Failed to resolve the Xbox services context for presence tracking.", "presence_context_failed");
    }
...
void XboxPresence::_close_handler_state(HandlerState &p_state) {
...
void CALLBACK XboxPresence::_device_presence_changed_handler(void *p_context, uint64_t p_xuid, XblPresenceDeviceType p_device_type, bool p_is_user_logged_on_device) {
    ...
    XboxPresence *presence = callback_context->presence;
...
void CALLBACK XboxPresence::_title_presence_changed_handler(void *p_context, uint64_t p_xuid, uint32_t p_title_id, XblPresenceTitleState p_title_state) {
    HandlerState::CallbackToken *token = static_cast<HandlerState::CallbackToken *>(p_context);
    std::shared_ptr<HandlerState::CallbackContext> callback_context = token != nullptr ? token->context.lock() : nullptr;
    if (!callback_context || !callback_context->active.load(std::memory_order_acquire)) {
        return;
