#ifndef XBOX_PACKAGE_H
#define XBOX_PACKAGE_H
...
class XboxPackageMount : public RefCounted {
    GDCLASS(XboxPackageMount, RefCounted);
...
public:
    ...
    bool is_valid() const;
    Ref<XboxResult> resolve_path(const String &p_relative_path) const;
    Ref<XboxResult> close();
};

class XboxPackageResourcePack : public RefCounted {
    GDCLASS(XboxPackageResourcePack, RefCounted);
...
class XboxPackage : public RefCounted {
    GDCLASS(XboxPackage, RefCounted);
...
private:
    ...
    Ref<XboxResult> _ensure_runtime_ready() const;
    ...
    Ref<XboxResult> _start_mount_async(const String &p_package_identifier, bool p_load_resource_pack, const String &p_pack_relative_path, bool p_replace_files, int64_t p_offset, Signal *r_signal);
...
public:
    ...
    Ref<XboxResult> finish_resource_pack_load(
            const String &p_package_identifier,
            const Dictionary &p_package_details,
            const String &p_mount_path,
    ...
    Ref<XboxResult> enumerate_packages(
    ...
    Ref<XboxResult> find_package_by_identifier(
            const String &p_package_identifier,
            int64_t p_package_kind = static_cast<int64_t>(PACKAGE_KIND_CONTENT),
            int64_t p_scope = static_cast<int64_t>(ENUMERATION_SCOPE_THIS_AND_RELATED));
    Ref<XboxResult> get_current_process_package_identifier();
    ...
    Ref<XboxResult> get_install_progress(const String &p_package_identifier);
};

} // namespace godot

VARIANT_ENUM_CAST(godot::XboxPackage::PackageKind);
VARIANT_ENUM_CAST(godot::XboxPackage::EnumerationScope);

#endif // XBOX_PACKAGE_H
