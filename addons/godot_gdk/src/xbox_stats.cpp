...
Ref<XboxResult> _parse_stat_names(
    ...
    for (int64_t i = 0; i < p_stat_names.size(); ++i) {
        ...
        if (stat_name.is_empty()) {
            return XboxResult::error_result(E_INVALIDARG, "invalid_stat_name", "Statistic names must be non-empty strings.");
        }
    ...
    if (r_stat_names->empty()) {
        return XboxResult::error_result(E_INVALIDARG, "invalid_stat_names", "At least one statistic name is required.");
    }
...
class QueryStatsAsyncContext final : public XboxSignalXAsyncContext {
    XboxStats *m_stats = nullptr;
    Ref<XboxUser> m_user;
...
protected:
    void finalize(XAsyncBlock *p_async_block) override {
        ...
        if (get_runtime()->is_shutting_down() || get_pending_signal()->was_cancel_requested()) {
            result = XboxResult::cancelled("Statistic query cancelled.");
        ...
        if (FAILED(result_hr)) {
            result = XboxResult::hresult_error(result_hr, "Failed to retrieve statistic query result size.", "stats_result_size_failed");
            get_pending_signal()->complete(result);
            return;
        }
        ...
        if (FAILED(result_hr)) {
            result = XboxResult::hresult_error(result_hr, "Failed to retrieve statistic query results.", "stats_results_failed");
            get_pending_signal()->complete(result);
            return;
        }
...
class FlushStatsAsyncContext final : public XboxSignalXAsyncContext {
    XboxStats *m_stats = nullptr;
    Ref<XboxUser> m_user;
    XblContextHandle m_context = nullptr;
    std::vector<XboxStats::StagedStat> m_staged_stats;
    std::vector<CharString> m_stat_name_utf8;
...
protected:
    void finalize(XAsyncBlock *p_async_block) override {
        ...
        if (get_runtime()->is_shutting_down() || get_pending_signal()->was_cancel_requested()) {
            result = XboxResult::cancelled("Statistic flush cancelled.");
            get_pending_signal()->complete(result);
            return;
        }
        ...
        if (FAILED(result_hr)) {
            result = XboxResult::hresult_error(result_hr, "Failed to flush title-managed statistics.", "stats_flush_failed");
            get_pending_signal()->complete(result);
            return;
        }
        ...
        result = XboxResult::ok_result(data);
...
public:
    FlushStatsAsyncContext(
            ...
            const Ref<XboxPendingSignal> &p_pending_signal,
            XblContextHandle p_context,
            const std::vector<XboxStats::StagedStat> &p_staged_stats) :
            XboxSignalXAsyncContext(p_runtime, p_pending_signal),
            m_stats(p_stats),
            m_user(p_user),
            m_context(p_context),
        ...
        for (const XboxStats::StagedStat &staged_stat : m_staged_stats) {
...
class WriteStatsAsyncContext final : public XboxSignalXAsyncContext {
    Ref<XboxUser> m_user;
    XblContextHandle m_context = nullptr;
    uint64_t m_xbox_user_id = 0;
...
protected:
    void finalize(XAsyncBlock *p_async_block) override {
        if (get_runtime()->is_shutting_down() || get_pending_signal()->was_cancel_requested()) {
            get_pending_signal()->complete(XboxResult::cancelled("Statistic write cancelled."));
            return;
        }
        ...
        if (FAILED(result_hr)) {
            get_pending_signal()->complete(XboxResult::hresult_error(result_hr, "Failed to write title-managed statistics.", "stats_write_failed"));
            return;
        }
...
public:
    WriteStatsAsyncContext(
            ...
            XblContextHandle p_context,
            uint64_t p_xbox_user_id,
            const std::vector<XboxStats::StagedStat> &p_stats) :
            XboxSignalXAsyncContext(p_runtime, p_pending_signal),
            m_user(p_user),
            m_context(p_context),
            m_xbox_user_id(p_xbox_user_id) {
        ...
        for (const XboxStats::StagedStat &stat : p_stats) {
...
class DeleteStatsAsyncContext final : public XboxSignalXAsyncContext {
    Ref<XboxUser> m_user;
    XblContextHandle m_context = nullptr;
    std::vector<CharString> m_name_utf8;
...
protected:
    void finalize(XAsyncBlock *p_async_block) override {
        if (get_runtime()->is_shutting_down() || get_pending_signal()->was_cancel_requested()) {
            get_pending_signal()->complete(XboxResult::cancelled("Statistic delete cancelled."));
            return;
        }
        ...
        if (FAILED(result_hr)) {
            get_pending_signal()->complete(XboxResult::hresult_error(result_hr, "Failed to delete title-managed statistics.", "stats_delete_failed"));
            return;
        }
...
void XboxStats::_bind_methods() {
    ClassDB::bind_method(D_METHOD("query_user_stats_async", "user", "stat_names"), &XboxStats::query_user_stats_async, DEFVAL(PackedStringArray()));
    ClassDB::bind_method(D_METHOD("query_users_stats_async", "user", "xuids", "stat_names"), &XboxStats::query_users_stats_async, DEFVAL(PackedStringArray()));
    ClassDB::bind_method(D_METHOD("get_single_stat_async", "user", "stat_name"), &XboxStats::get_single_stat_async);
    ClassDB::bind_method(D_METHOD("set_stat_integer", "user", "stat_name", "value"), &XboxStats::set_stat_integer);
    ClassDB::bind_method(D_METHOD("set_stat_number", "user", "stat_name", "value"), &XboxStats::set_stat_number);
    ClassDB::bind_method(D_METHOD("flush_stats_async", "user"), &XboxStats::flush_stats_async);
    ClassDB::bind_method(D_METHOD("write_stats_async", "user", "stats"), &XboxStats::write_stats_async);
    ClassDB::bind_method(D_METHOD("delete_stats_async", "user", "stat_names"), &XboxStats::delete_stats_async);
    ClassDB::bind_method(D_METHOD("track_stats", "user", "stat_names"), &XboxStats::track_stats);
    ClassDB::bind_method(D_METHOD("stop_tracking_stats", "user", "stat_names"), &XboxStats::stop_tracking_stats, DEFVAL(PackedStringArray()));
    ClassDB::bind_method(D_METHOD("get_cached_stats", "user"), &XboxStats::get_cached_stats);

    ADD_SIGNAL(MethodInfo("stats_updated", PropertyInfo(Variant::OBJECT, "user", PROPERTY_HINT_NONE, "", PROPERTY_USAGE_DEFAULT, "XboxUser"), PropertyInfo(Variant::DICTIONARY, "stats")));
    ADD_SIGNAL(MethodInfo("stat_changed", PropertyInfo(Variant::OBJECT, "user", PROPERTY_HINT_NONE, "", PROPERTY_USAGE_DEFAULT, "XboxUser"), PropertyInfo(Variant::STRING, "stat_name"), PropertyInfo(Variant::NIL, "value")));
    ADD_SIGNAL(MethodInfo("stats_flushed", PropertyInfo(Variant::OBJECT, "user", PROPERTY_HINT_NONE, "", PROPERTY_USAGE_DEFAULT, "XboxUser"), PropertyInfo(Variant::OBJECT, "result", PROPERTY_HINT_NONE, "", PROPERTY_USAGE_DEFAULT, "XboxResult")));
}

void XboxStats::set_owner(Xbox *p_owner) {
    m_owner = p_owner;
}

XboxRuntime *XboxStats::_get_runtime() const {
    return m_owner == nullptr ? nullptr : m_owner->get_runtime();
}

XboxServices *XboxStats::_get_xbox_services() const {
    return m_owner == nullptr ? nullptr : m_owner->get_xbox_services();
}

Signal XboxStats::_make_error_signal(HRESULT p_hresult, const String &p_code, const String &p_message, const Variant &p_data) const {
...
Ref<XboxResult> XboxStats::_ensure_ready_user(const Ref<XboxUser> &p_user) const {
    if (!m_runtime_ready) {
...
XboxStats::CachedStats *XboxStats::_find_cached_stats(const String &p_xuid) {
...
const XboxStats::CachedStats *XboxStats::_find_cached_stats(const String &p_xuid) const {
...
XboxStats::StagedStats *XboxStats::_find_staged_stats(XUserLocalId p_local_id) {
...
XboxStats::TrackingState *XboxStats::_find_tracking_state(XUserLocalId p_local_id) {
...
void XboxStats::_cache_stat_value(const String &p_xuid, const String &p_stat_name, const String &p_stat_type, const String &p_value, const String &p_scid) {
...
void XboxStats::_cache_results(const XblUserStatisticsResult *p_results, size_t p_result_count) {
...
Dictionary XboxStats::_make_query_payload(const XblUserStatisticsResult *p_results, size_t p_result_count, bool p_single_user_payload) {
...
void XboxStats::_clear_staged_stats(const Ref<XboxUser> &p_user) {
    if (!p_user.is_valid()) {
        return;
    }
...
void XboxStats::_apply_staged_stats_to_cache(const Ref<XboxUser> &p_user, const std::vector<StagedStat> &p_stats) {
...
void XboxStats::_close_tracking_state(TrackingState &p_state) {
...
Ref<XboxResult> XboxStats::on_runtime_initialized() {
    m_runtime_ready = true;
    return XboxResult::ok_result();
}

void XboxStats::shutdown() {
...
Signal XboxStats::query_user_stats_async(const Ref<XboxUser> &p_user, const PackedStringArray &p_stat_names) {
    XboxRuntime *runtime = _get_runtime();
    if (runtime == nullptr) {
        return Signal();
    }
    ...
    Ref<XboxResult> parse_result = _parse_stat_names(p_stat_names, &stat_names, &stat_name_utf8, &stat_name_ptrs);
...
Signal XboxStats::query_users_stats_async(const Ref<XboxUser> &p_user, const PackedStringArray &p_xuids, const PackedStringArray &p_stat_names) {
    XboxRuntime *runtime = _get_runtime();
    if (runtime == nullptr) {
        return Signal();
    }
...
Ref<XboxResult> XboxStats::set_stat_integer(const Ref<XboxUser> &p_user, const String &p_stat_name, int64_t p_value) {
    return set_stat_number(p_user, p_stat_name, static_cast<double>(p_value));
}

Ref<XboxResult> XboxStats::set_stat_number(const Ref<XboxUser> &p_user, const String &p_stat_name, double p_value) {
    Ref<XboxResult> validation = _ensure_ready_user(p_user);
    if (!validation->is_ok()) {
        return validation;
    }
...
Signal XboxStats::flush_stats_async(const Ref<XboxUser> &p_user) {
    XboxRuntime *runtime = _get_runtime();
    if (runtime == nullptr) {
        return Signal();
    }
...
Signal XboxStats::get_single_stat_async(const Ref<XboxUser> &p_user, const String &p_stat_name) {
    XboxRuntime *runtime = _get_runtime();
    if (runtime == nullptr) {
        return Signal();
    }
...
Signal XboxStats::write_stats_async(const Ref<XboxUser> &p_user, const Dictionary &p_stats) {
    XboxRuntime *runtime = _get_runtime();
    if (runtime == nullptr) {
        return Signal();
    }
...
Signal XboxStats::delete_stats_async(const Ref<XboxUser> &p_user, const PackedStringArray &p_stat_names) {
    XboxRuntime *runtime = _get_runtime();
    if (runtime == nullptr) {
        return Signal();
    }
...
Ref<XboxResult> XboxStats::track_stats(const Ref<XboxUser> &p_user, const PackedStringArray &p_stat_names) {
    Ref<XboxResult> validation = _ensure_ready_user(p_user);
    if (!validation->is_ok()) {
        return validation;
    }
    ...
    if (state == nullptr) {
        ...
        if (FAILED(context_hr)) {
            return XboxResult::hresult_error(context_hr, "Failed to create an Xbox services context for statistics tracking.", "xbox_context_unavailable");
        }
    ...
    HRESULT hr = XblUserStatisticsTrackStatistics(
            ...
            stat_name_ptrs.size());
    if (FAILED(hr)) {
        return XboxResult::hresult_error(hr, "Failed to track statistic changes.", "stats_track_failed");
    }
...
Ref<XboxResult> XboxStats::stop_tracking_stats(const Ref<XboxUser> &p_user, const PackedStringArray &p_stat_names) {
    Ref<XboxResult> validation = _ensure_ready_user(p_user);
    if (!validation->is_ok()) {
        return validation;
    }
    ...
    if (FAILED(hr)) {
        return XboxResult::hresult_error(hr, "Failed to stop tracking statistic changes.", "stats_stop_tracking_failed");
    }

    return XboxResult::ok_result();
}

Dictionary XboxStats::get_cached_stats(const Ref<XboxUser> &p_user) const {
    if (!p_user.is_valid()) {
        return Dictionary();
    }
...
void XboxStats::on_user_removed(const Ref<XboxUser> &p_user) {
    if (!p_user.is_valid()) {
        return;
    }
...
void CALLBACK XboxStats::_statistic_changed_handler(XblStatisticChangeEventArgs p_args, void *p_context) {
    ...
    XboxStats *stats = callback_context->stats;
