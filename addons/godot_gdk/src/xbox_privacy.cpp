...
class PrivacyPermissionAsyncContext final : public XboxSignalXAsyncContext {
public:
    enum Mode {
        MODE_SINGLE,
        MODE_ANONYMOUS,
...
protected:
    void finalize(XAsyncBlock *p_async_block) override {
        ...
        if (get_runtime()->is_shutting_down() || get_pending_signal()->was_cancel_requested()) {
            result = XboxResult::cancelled("Privacy permission check cancelled.");
        ...
        if (FAILED(result_hr)) {
            result = XboxResult::hresult_error(result_hr, "Failed to retrieve privacy permission result size.", "privacy_result_size_failed");
            get_pending_signal()->complete(result);
            return;
        }
        ...
        if (m_mode == MODE_BATCH) {
            ...
            result_hr = XblPrivacyBatchCheckPermissionResult(
                    ...
                    &buffer_used);
            if (FAILED(result_hr)) {
                result = result_hr == E_ABORT ? XboxResult::cancelled("Privacy permission check cancelled.") : XboxResult::hresult_error(result_hr, "Failed to retrieve privacy permission results.", "privacy_results_failed");
                get_pending_signal()->complete(result);
                return;
            }
        ...
        result_hr = m_mode == MODE_ANONYMOUS ?
                ...
                XblPrivacyCheckPermissionResult(
                        ...
                        &buffer_used);
        if (FAILED(result_hr)) {
            result = result_hr == E_ABORT ? XboxResult::cancelled("Privacy permission check cancelled.") : XboxResult::hresult_error(result_hr, "Failed to retrieve privacy permission result.", "privacy_result_failed");
            get_pending_signal()->complete(result);
            return;
        }
...
public:
    PrivacyPermissionAsyncContext(XboxRuntime *p_runtime, const Ref<XboxPendingSignal> &p_pending_signal, XblContextHandle p_context, Mode p_mode) :
...
class PrivacyListAsyncContext final : public XboxSignalXAsyncContext {
    XblContextHandle m_context = nullptr;
...
protected:
    void finalize(XAsyncBlock *p_async_block) override {
        ...
        if (get_runtime()->is_shutting_down() || get_pending_signal()->was_cancel_requested()) {
            result = XboxResult::cancelled("Privacy list query cancelled.");
            get_pending_signal()->complete(result);
            return;
        }
        ...
        if (FAILED(result_hr)) {
            result = XboxResult::hresult_error(result_hr, "Failed to retrieve privacy list result count.", "privacy_list_count_failed");
            get_pending_signal()->complete(result);
            return;
        }
        ...
        if (FAILED(result_hr)) {
            result = result_hr == E_ABORT ? XboxResult::cancelled("Privacy list query cancelled.") : XboxResult::hresult_error(result_hr, "Failed to retrieve privacy list results.", "privacy_list_result_failed");
            get_pending_signal()->complete(result);
            return;
        }

        get_pending_signal()->complete(XboxResult::ok_result(_make_xuid_array(xuids)));
    }

public:
    PrivacyListAsyncContext(XboxRuntime *p_runtime, const Ref<XboxPendingSignal> &p_pending_signal, XblContextHandle p_context, bool p_mute_list) :
            XboxSignalXAsyncContext(p_runtime, p_pending_signal),
            m_context(p_context),
...
void XboxPrivacy::_bind_methods() {
    ClassDB::bind_method(D_METHOD("check_permission_async", "user", "permission", "target_xuid"), &XboxPrivacy::check_permission_async);
    ClassDB::bind_method(D_METHOD("check_permission_for_anonymous_user_async", "user", "permission", "anonymous_user_type"), &XboxPrivacy::check_permission_for_anonymous_user_async);
    ClassDB::bind_method(D_METHOD("batch_check_permission_async", "user", "permission", "target_xuids"), &XboxPrivacy::batch_check_permission_async);
    ClassDB::bind_method(D_METHOD("get_avoid_list_async", "user"), &XboxPrivacy::get_avoid_list_async);
    ClassDB::bind_method(D_METHOD("get_mute_list_async", "user"), &XboxPrivacy::get_mute_list_async);
}

void XboxPrivacy::set_owner(Xbox *p_owner) {
    m_owner = p_owner;
}

XboxRuntime *XboxPrivacy::_get_runtime() const {
    return m_owner == nullptr ? nullptr : m_owner->get_runtime();
}

XboxServices *XboxPrivacy::_get_xbox_services() const {
    return m_owner == nullptr ? nullptr : m_owner->get_xbox_services();
}

Signal XboxPrivacy::_make_error_signal(HRESULT p_hresult, const String &p_code, const String &p_message, const Variant &p_data) const {
...
Ref<XboxResult> XboxPrivacy::_ensure_ready_user(const Ref<XboxUser> &p_user) const {
    if (!m_runtime_ready) {
...
Ref<XboxResult> XboxPrivacy::on_runtime_initialized() {
    m_runtime_ready = true;
    return XboxResult::ok_result();
}

void XboxPrivacy::shutdown() {
    m_runtime_ready = false;
}

int XboxPrivacy::dispatch() {
    return 0;
}

Signal XboxPrivacy::check_permission_async(const Ref<XboxUser> &p_user, const String &p_permission, const String &p_target_xuid) {
    XboxRuntime *runtime = _get_runtime();
    if (runtime == nullptr) {
        return Signal();
    }
...
Signal XboxPrivacy::check_permission_for_anonymous_user_async(const Ref<XboxUser> &p_user, const String &p_permission, const String &p_anonymous_user_type) {
    XboxRuntime *runtime = _get_runtime();
    if (runtime == nullptr) {
        return Signal();
    }
...
Signal XboxPrivacy::batch_check_permission_async(const Ref<XboxUser> &p_user, const String &p_permission, const PackedStringArray &p_target_xuids) {
    XboxRuntime *runtime = _get_runtime();
    if (runtime == nullptr) {
        return Signal();
    }
...
Signal XboxPrivacy::get_avoid_list_async(const Ref<XboxUser> &p_user) {
    XboxRuntime *runtime = _get_runtime();
    if (runtime == nullptr) {
        return Signal();
    }
...
Signal XboxPrivacy::get_mute_list_async(const Ref<XboxUser> &p_user) {
    XboxRuntime *runtime = _get_runtime();
    if (runtime == nullptr) {
        return Signal();
    }
...
void XboxPrivacy::on_user_removed(const Ref<XboxUser> &p_user) {
    (void)p_user;
}
