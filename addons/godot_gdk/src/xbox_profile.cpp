...
Ref<XboxUserProfile> _make_user_profile(const XblUserProfile &p_profile) {
    Ref<XboxUserProfile> profile;
    profile.instantiate();
...
class ProfileAsyncContext final : public XboxSignalXAsyncContext {
public:
    enum Mode {
        MODE_SINGLE,
        MODE_MULTIPLE,
...
protected:
    void finalize(XAsyncBlock *p_async_block) override {
        ...
        if (get_runtime()->is_shutting_down() || get_pending_signal()->was_cancel_requested()) {
            result = XboxResult::cancelled("Profile query cancelled.");
        ...
        if (m_mode == MODE_SINGLE) {
            ...
            if (FAILED(result_hr)) {
                result = XboxResult::hresult_error(result_hr, "Failed to retrieve the profile result.", "profile_result_failed");
                get_pending_signal()->complete(result);
                return;
            }

            get_pending_signal()->complete(XboxResult::ok_result(_make_user_profile(profile)));
            return;
        }
        ...
        if (FAILED(result_hr)) {
            result = XboxResult::hresult_error(result_hr, "Failed to retrieve the profile result count.", "profile_result_count_failed");
            get_pending_signal()->complete(result);
            return;
        }
        ...
        if (profile_count > 0) {
            ...
            if (FAILED(result_hr)) {
                result = XboxResult::hresult_error(result_hr, "Failed to retrieve profile results.", "profile_results_failed");
                get_pending_signal()->complete(result);
                return;
            }
        }

        get_pending_signal()->complete(XboxResult::ok_result(_make_user_profile_array(profiles)));
    }

public:
    ProfileAsyncContext(XboxRuntime *p_runtime, const Ref<XboxPendingSignal> &p_pending_signal, XblContextHandle p_context, uint64_t p_target_xuid) :
            XboxSignalXAsyncContext(p_runtime, p_pending_signal),
            m_context(p_context),
            m_mode(MODE_SINGLE),
            m_target_xuid(p_target_xuid) {}

    ProfileAsyncContext(XboxRuntime *p_runtime, const Ref<XboxPendingSignal> &p_pending_signal, XblContextHandle p_context, std::vector<uint64_t> p_xuids) :
            XboxSignalXAsyncContext(p_runtime, p_pending_signal),
            m_context(p_context),
            m_mode(MODE_MULTIPLE),
            m_xuids(std::move(p_xuids)) {}

    ProfileAsyncContext(XboxRuntime *p_runtime, const Ref<XboxPendingSignal> &p_pending_signal, XblContextHandle p_context, const String &p_social_group) :
            XboxSignalXAsyncContext(p_runtime, p_pending_signal),
            m_context(p_context),
...
void XboxUserProfile::_bind_methods() {
    ClassDB::bind_method(D_METHOD("get_xuid"), &XboxUserProfile::get_xuid);
    ClassDB::bind_method(D_METHOD("get_app_display_name"), &XboxUserProfile::get_app_display_name);
    ClassDB::bind_method(D_METHOD("get_app_display_picture_resize_uri"), &XboxUserProfile::get_app_display_picture_resize_uri);
    ClassDB::bind_method(D_METHOD("get_game_display_name"), &XboxUserProfile::get_game_display_name);
    ClassDB::bind_method(D_METHOD("get_game_display_picture_resize_uri"), &XboxUserProfile::get_game_display_picture_resize_uri);
    ClassDB::bind_method(D_METHOD("get_gamerscore"), &XboxUserProfile::get_gamerscore);
    ClassDB::bind_method(D_METHOD("get_gamertag"), &XboxUserProfile::get_gamertag);
    ClassDB::bind_method(D_METHOD("get_modern_gamertag"), &XboxUserProfile::get_modern_gamertag);
    ClassDB::bind_method(D_METHOD("get_modern_gamertag_suffix"), &XboxUserProfile::get_modern_gamertag_suffix);
    ClassDB::bind_method(D_METHOD("get_unique_modern_gamertag"), &XboxUserProfile::get_unique_modern_gamertag);
...
String XboxUserProfile::get_xuid() const {
    return m_xuid;
}

String XboxUserProfile::get_app_display_name() const {
    return m_app_display_name;
}

String XboxUserProfile::get_app_display_picture_resize_uri() const {
    return m_app_display_picture_resize_uri;
}

String XboxUserProfile::get_game_display_name() const {
    return m_game_display_name;
}

String XboxUserProfile::get_game_display_picture_resize_uri() const {
    return m_game_display_picture_resize_uri;
}

String XboxUserProfile::get_gamerscore() const {
    return m_gamerscore;
}

String XboxUserProfile::get_gamertag() const {
    return m_gamertag;
}

String XboxUserProfile::get_modern_gamertag() const {
    return m_modern_gamertag;
}

String XboxUserProfile::get_modern_gamertag_suffix() const {
    return m_modern_gamertag_suffix;
}

String XboxUserProfile::get_unique_modern_gamertag() const {
    return m_unique_modern_gamertag;
}

void XboxUserProfile::populate_from_native(const XblUserProfile &p_profile) {
...
void XboxProfile::_bind_methods() {
    ClassDB::bind_method(D_METHOD("get_profile_async", "user", "xuid"), &XboxProfile::get_profile_async);
    ClassDB::bind_method(D_METHOD("get_profiles_async", "user", "xuids"), &XboxProfile::get_profiles_async);
    ClassDB::bind_method(D_METHOD("get_profiles_for_social_group_async", "user", "social_group"), &XboxProfile::get_profiles_for_social_group_async);
}

void XboxProfile::set_owner(Xbox *p_owner) {
    m_owner = p_owner;
}

XboxRuntime *XboxProfile::_get_runtime() const {
    return m_owner == nullptr ? nullptr : m_owner->get_runtime();
}

XboxServices *XboxProfile::_get_xbox_services() const {
    return m_owner == nullptr ? nullptr : m_owner->get_xbox_services();
}

Signal XboxProfile::_make_error_signal(HRESULT p_hresult, const String &p_code, const String &p_message, const Variant &p_data) const {
...
Ref<XboxResult> XboxProfile::_ensure_ready_user(const Ref<XboxUser> &p_user) const {
    if (!m_runtime_ready) {
...
Ref<XboxResult> XboxProfile::on_runtime_initialized() {
    m_runtime_ready = true;
    return XboxResult::ok_result();
}

void XboxProfile::shutdown() {
    m_runtime_ready = false;
}

Signal XboxProfile::get_profile_async(const Ref<XboxUser> &p_user, const String &p_xuid) {
    XboxRuntime *runtime = _get_runtime();
    if (runtime == nullptr) {
        return Signal();
    }
...
Signal XboxProfile::get_profiles_async(const Ref<XboxUser> &p_user, const PackedStringArray &p_xuids) {
    XboxRuntime *runtime = _get_runtime();
    if (runtime == nullptr) {
        return Signal();
    }
...
Signal XboxProfile::get_profiles_for_social_group_async(const Ref<XboxUser> &p_user, const String &p_social_group) {
    XboxRuntime *runtime = _get_runtime();
    if (runtime == nullptr) {
        return Signal();
    }
...
void XboxProfile::on_user_removed(const Ref<XboxUser> &p_user) {
    (void)p_user;
}
