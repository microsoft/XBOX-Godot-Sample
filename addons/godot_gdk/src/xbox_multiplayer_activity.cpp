...
Ref<XboxResult> _parse_xuids(
    ...
    ERR_FAIL_COND_V(r_xuids == nullptr, XboxResult::error_result(E_POINTER, "internal_error", "Missing XUID output buffer."));

    if (p_xuids.is_empty()) {
        return XboxResult::error_result(E_INVALIDARG, p_empty_code, "At least one XUID is required.");
    }
    if (p_max_count > 0 && static_cast<size_t>(p_xuids.size()) > p_max_count) {
        return XboxResult::error_result(E_INVALIDARG, p_invalid_code, "Too many XUIDs were provided for this request.");
    }
    ...
    for (int64_t i = 0; i < p_xuids.size(); ++i) {
        ...
        uint64_t xuid = 0;
        if (!XboxMultiplayerActivity::try_parse_xuid_internal(xuid_string, &xuid)) {
            return XboxResult::error_result(E_INVALIDARG, p_invalid_code, p_invalid_message);
        }
...
class MultiplayerActivityXAsyncContext : public XboxSignalXAsyncContext {
protected:
    XboxMultiplayerActivity *m_service = nullptr;
    XblContextHandle m_context = nullptr;

public:
    MultiplayerActivityXAsyncContext(
            XboxMultiplayerActivity *p_service,
...
protected:
    void finalize(XAsyncBlock *p_async_block) override {
        ...
        if (get_runtime()->is_shutting_down() || get_pending_signal()->was_cancel_requested()) {
            result = XboxResult::cancelled("Multiplayer activity update cancelled.");
        ...
        Ref<XboxMultiplayerActivityInfo> activity;
        ...
        get_pending_signal()->complete(XboxResult::ok_result(activity));
    }
...
protected:
    void finalize(XAsyncBlock *p_async_block) override {
        ...
        if (get_runtime()->is_shutting_down() || get_pending_signal()->was_cancel_requested()) {
            result = XboxResult::cancelled("Multiplayer activity query cancelled.");
            get_pending_signal()->complete(result);
            return;
        }
        ...
        get_pending_signal()->complete(XboxResult::ok_result(godot_activities));
    }
...
protected:
    void finalize(XAsyncBlock *p_async_block) override {
        ...
        if (get_runtime()->is_shutting_down() || get_pending_signal()->was_cancel_requested()) {
            result = XboxResult::cancelled("Multiplayer activity delete cancelled.");
            get_pending_signal()->complete(result);
            return;
        }
        ...
        m_service->emit_activities_updated_internal({ m_local_xuid });
        get_pending_signal()->complete(XboxResult::ok_result(m_local_xuid));
    }
...
protected:
    void finalize(XAsyncBlock *p_async_block) override {
        ...
        if (get_runtime()->is_shutting_down() || get_pending_signal()->was_cancel_requested()) {
            result = XboxResult::cancelled("Multiplayer invite send cancelled.");
            get_pending_signal()->complete(result);
            return;
        }
...
class InviteUiAsyncContext final : public XboxSignalXAsyncContext {
protected:
    void finalize(XAsyncBlock *p_async_block) override {
        Ref<XboxResult> result;

        if (get_runtime()->is_shutting_down() || get_pending_signal()->was_cancel_requested()) {
            result = XboxResult::cancelled("Invite UI cancelled.");
            get_pending_signal()->complete(result);
            return;
        }
...
public:
    InviteUiAsyncContext(XboxRuntime *p_runtime, const Ref<XboxPendingSignal> &p_pending_signal) :
            XboxSignalXAsyncContext(p_runtime, p_pending_signal) {}
};
...
protected:
    void finalize(XAsyncBlock *p_async_block) override {
        ...
        if (get_runtime()->is_shutting_down() || get_pending_signal()->was_cancel_requested()) {
            result = XboxResult::cancelled("Recent-player flush cancelled.");
            get_pending_signal()->complete(result);
            return;
        }
...
void XboxMultiplayerActivityInfo::_bind_methods() {
    ClassDB::bind_method(D_METHOD("get_xuid"), &XboxMultiplayerActivityInfo::get_xuid);
    ClassDB::bind_method(D_METHOD("get_connection_string"), &XboxMultiplayerActivityInfo::get_connection_string);
    ClassDB::bind_method(D_METHOD("get_join_restriction"), &XboxMultiplayerActivityInfo::get_join_restriction);
    ClassDB::bind_method(D_METHOD("get_max_players"), &XboxMultiplayerActivityInfo::get_max_players);
    ClassDB::bind_method(D_METHOD("get_current_players"), &XboxMultiplayerActivityInfo::get_current_players);
    ClassDB::bind_method(D_METHOD("get_group_id"), &XboxMultiplayerActivityInfo::get_group_id);
    ClassDB::bind_method(D_METHOD("get_platform"), &XboxMultiplayerActivityInfo::get_platform);
...
String XboxMultiplayerActivityInfo::get_xuid() const {
    return m_xuid;
}

String XboxMultiplayerActivityInfo::get_connection_string() const {
    return m_connection_string;
}

String XboxMultiplayerActivityInfo::get_join_restriction() const {
    return m_join_restriction;
}

int64_t XboxMultiplayerActivityInfo::get_max_players() const {
    return m_max_players;
}

int64_t XboxMultiplayerActivityInfo::get_current_players() const {
    return m_current_players;
}

String XboxMultiplayerActivityInfo::get_group_id() const {
    return m_group_id;
}

String XboxMultiplayerActivityInfo::get_platform() const {
    return m_platform;
}

void XboxMultiplayerActivityInfo::set_values(
        const String &p_xuid,
        const String &p_connection_string,
        const String &p_join_restriction,
        int64_t p_max_players,
...
void XboxMultiplayerActivityInfo::populate_from_native(const XblMultiplayerActivityInfo &p_native_activity) {
    set_values(
            String::num_uint64(p_native_activity.xuid),
            _utf8_or_empty(p_native_activity.connectionString),
            XboxMultiplayerActivity::join_restriction_to_string_internal(p_native_activity.joinRestriction),
            ...
            XboxMultiplayerActivity::platform_to_string_internal(p_native_activity.platform));
}

void XboxMultiplayerActivity::_bind_methods() {
    ClassDB::bind_method(
            ...
            &XboxMultiplayerActivity::set_activity_async,
            DEFVAL(String("followed")),
            DEFVAL(static_cast<int64_t>(0)),
            DEFVAL(static_cast<int64_t>(0)),
            DEFVAL(String()),
            DEFVAL(false));
    ClassDB::bind_method(D_METHOD("get_activities_async", "user", "xuids"), &XboxMultiplayerActivity::get_activities_async);
    ClassDB::bind_method(D_METHOD("get_cached_activity", "xuid"), &XboxMultiplayerActivity::get_cached_activity);
    ClassDB::bind_method(D_METHOD("delete_activity_async", "user"), &XboxMultiplayerActivity::delete_activity_async);
    ClassDB::bind_method(
            D_METHOD("send_invites_async", "user", "xuids", "allow_cross_platform_join", "connection_string"),
            &XboxMultiplayerActivity::send_invites_async,
            DEFVAL(true),
            DEFVAL(String()));
    ClassDB::bind_method(D_METHOD("show_invite_ui_async", "user"), &XboxMultiplayerActivity::show_invite_ui_async);
    ClassDB::bind_method(
            D_METHOD("update_recent_players", "user", "xuids", "encounter_type"),
            &XboxMultiplayerActivity::update_recent_players,
            DEFVAL(String("default")));
    ClassDB::bind_method(D_METHOD("flush_recent_players_async", "user"), &XboxMultiplayerActivity::flush_recent_players_async);
    ClassDB::bind_method(D_METHOD("accept_pending_invite", "invite_uri"), &XboxMultiplayerActivity::accept_pending_invite);
...
void XboxMultiplayerActivity::set_owner(Xbox *p_owner) {
    m_owner = p_owner;
}

Ref<XboxResult> XboxMultiplayerActivity::on_runtime_initialized() {
    ...
    if (m_activation_listener_id == 0 && m_owner != nullptr) {
        Ref<XboxActivation> activation = m_owner->get_activation();
...
void XboxMultiplayerActivity::shutdown() {
...
void XboxMultiplayerActivity::on_user_removed(const Ref<XboxUser> &p_user) {
    if (!p_user.is_valid()) {
        return;
    }
...
Signal XboxMultiplayerActivity::set_activity_async(
    ...
    Ref<XboxResult> context_result = duplicate_context_for_user_internal(p_user, &context, &xbox_user_id);
...
Signal XboxMultiplayerActivity::get_activities_async(const Ref<XboxUser> &p_user, const PackedStringArray &p_xuids) {
    XboxRuntime *runtime = get_runtime_internal();

    std::vector<uint64_t> query_xuids;
    Ref<XboxResult> parse_result = _parse_xuids(
            p_xuids,
            &query_xuids,
            nullptr,
            30,
    ...
    XblContextHandle context = nullptr;
    Ref<XboxResult> context_result = duplicate_context_for_user_internal(p_user, &context);
    if (!context_result->is_ok()) {
        return make_error_signal_internal(
                static_cast<HRESULT>(context_result->get_hresult()),
                context_result->get_code(),
...
Ref<XboxMultiplayerActivityInfo> XboxMultiplayerActivity::get_cached_activity(const String &p_xuid) const {
    ...
    return Ref<XboxMultiplayerActivityInfo>();
}

Signal XboxMultiplayerActivity::delete_activity_async(const Ref<XboxUser> &p_user) {
    XboxRuntime *runtime = get_runtime_internal();
...
Signal XboxMultiplayerActivity::send_invites_async(
        const Ref<XboxUser> &p_user,
        const PackedStringArray &p_xuids,
        bool p_allow_cross_platform_join,
        const String &p_connection_string) {
    ...
    if (resolved_connection_string.is_empty()) {
        Ref<XboxMultiplayerActivityInfo> cached_activity = get_cached_activity(String::num_uint64(xbox_user_id));
...
Signal XboxMultiplayerActivity::show_invite_ui_async(const Ref<XboxUser> &p_user) {
    XboxRuntime *runtime = get_runtime_internal();
    if (runtime == nullptr || !runtime->is_initialized()) {
        return make_error_signal_internal(
                E_FAIL,
    ...
    if (!p_user.is_valid() || p_user->get_handle() == nullptr) {
        return make_error_signal_internal(
                E_INVALIDARG,
                "invalid_user",
                "A signed-in XboxUser is required to show the invite UI.");
    }
...
Ref<XboxResult> XboxMultiplayerActivity::update_recent_players(
        const Ref<XboxUser> &p_user,
        const PackedStringArray &p_xuids,
...
Signal XboxMultiplayerActivity::flush_recent_players_async(const Ref<XboxUser> &p_user) {
    XboxRuntime *runtime = get_runtime_internal();
...
Ref<XboxResult> XboxMultiplayerActivity::accept_pending_invite(const String &p_invite_uri) {
    if (!m_runtime_ready) {
        return XboxResult::error_result(
                E_FAIL,
                "not_initialized",
    ...
    return XboxResult::ok_result(parse_invite_uri_internal(invite_uri, "pending_game_invite"));
}

XboxRuntime *XboxMultiplayerActivity::get_runtime_internal() const {
    return m_owner != nullptr ? m_owner->get_runtime() : nullptr;
}

XboxServices *XboxMultiplayerActivity::get_xbox_services_internal() const {
    return m_owner != nullptr ? m_owner->get_xbox_services() : nullptr;
}

Signal XboxMultiplayerActivity::make_completed_signal_internal(const Ref<XboxResult> &p_result) const {
    XboxRuntime *runtime = get_runtime_internal();
    Ref<XboxPendingSignal> pending_signal = runtime != nullptr ? runtime->make_pending_signal() : Ref<XboxPendingSignal>();
...
Signal XboxMultiplayerActivity::make_error_signal_internal(
        HRESULT p_hresult,
        const String &p_code,
    ...
    pending_signal.instantiate();
    pending_signal->complete_deferred(XboxResult::error_result(p_hresult, p_code, p_message, p_data));
    return pending_signal->get_completed_signal();
}

Ref<XboxResult> XboxMultiplayerActivity::duplicate_context_for_user_internal(
        const Ref<XboxUser> &p_user,
        XblContextHandle *r_context,
        uint64_t *r_xbox_user_id) const {
    ERR_FAIL_COND_V(r_context == nullptr, XboxResult::error_result(E_POINTER, "internal_error", "Missing multiplayer activity context output."));
    ...
    *r_context = nullptr;
    ...
    // used by xbox_presence.cpp set_presence_async (runtime -> user -> services).
    if (!p_user.is_valid() || p_user->get_handle() == nullptr) {
        return XboxResult::error_result(
                E_INVALIDARG,
                "invalid_user",
                "A signed-in XboxUser is required for multiplayer activity.");
    }

    XboxServices *xbox_services = get_xbox_services_internal();
...
Ref<XboxMultiplayerActivityInfo> XboxMultiplayerActivity::cache_activity_internal(const Ref<XboxMultiplayerActivityInfo> &p_info) {
    if (!p_info.is_valid()) {
        return Ref<XboxMultiplayerActivityInfo>();
    }
...
void XboxMultiplayerActivity::remove_cached_activity_internal(const String &p_xuid) {
    const String normalized_xuid = p_xuid.strip_edges();
...
void XboxMultiplayerActivity::emit_activities_updated_internal(const std::vector<String> &p_xuids) {
...
void XboxMultiplayerActivity::handle_activation_internal(const Dictionary &p_activation_info) {
    if (!m_runtime_ready) {
        return;
    }
    ...
    if (invite.is_empty() && !invite_uri.is_empty()) {
        if (activation_type == static_cast<int64_t>(XboxActivation::ACTIVATION_TYPE_PENDING_GAME_INVITE)) {
            invite = parse_invite_uri_internal(invite_uri, "pending_game_invite");
        } else if (activation_type == static_cast<int64_t>(XboxActivation::ACTIVATION_TYPE_ACCEPTED_GAME_INVITE)) {
            invite = parse_invite_uri_internal(invite_uri, "accepted_game_invite");
        }
    }

    switch (activation_type) {
        case XboxActivation::ACTIVATION_TYPE_PENDING_GAME_INVITE:
            emit_signal("pending_invite_received", invite);
            break;
        case XboxActivation::ACTIVATION_TYPE_ACCEPTED_GAME_INVITE:
...
String XboxMultiplayerActivity::join_restriction_to_string_internal(XblMultiplayerActivityJoinRestriction p_join_restriction) {
...
bool XboxMultiplayerActivity::try_parse_join_restriction_internal(
        const String &p_join_restriction,
...
String XboxMultiplayerActivity::platform_to_string_internal(XblMultiplayerActivityPlatform p_platform) {
    switch (p_platform) {
...
String XboxMultiplayerActivity::encounter_type_to_string_internal(XblMultiplayerActivityEncounterType p_encounter_type) {
...
bool XboxMultiplayerActivity::try_parse_encounter_type_internal(
        const String &p_encounter_type,
...
Dictionary XboxMultiplayerActivity::parse_invite_uri_internal(const String &p_uri, const String &p_activation_type) {
    return XboxActivation::make_invite_dictionary_internal(p_uri, p_activation_type);
}
