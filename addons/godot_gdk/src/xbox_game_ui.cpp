...
Ref<XboxResult> _parse_xuid_list(
    ...
    if (r_xuids == nullptr) {
        return XboxResult::error_result(E_POINTER, "internal_error", "Missing XUID list output.");
    }

    if (p_xuids.is_empty()) {
        return XboxResult::error_result(E_INVALIDARG, p_empty_code, p_empty_message);
    }
...
class MessageDialogAsyncContext final : public XboxSignalXAsyncContext {
protected:
    void finalize(XAsyncBlock *p_async_block) override {
        ...
        if (get_runtime()->is_shutting_down() || get_pending_signal()->was_cancel_requested()) {
            result = XboxResult::cancelled("Message dialog cancelled.");
...
public:
    MessageDialogAsyncContext(XboxRuntime *p_runtime, const Ref<XboxPendingSignal> &p_pending_signal) :
            XboxSignalXAsyncContext(p_runtime, p_pending_signal) {}
};

class PlayerProfileCardAsyncContext final : public XboxSignalXAsyncContext {
protected:
    void finalize(XAsyncBlock *p_async_block) override {
        Ref<XboxResult> result;

        if (get_runtime()->is_shutting_down() || get_pending_signal()->was_cancel_requested()) {
            result = XboxResult::cancelled("Player profile card UI cancelled.");
            get_pending_signal()->complete(result);
            return;
        }
...
public:
    PlayerProfileCardAsyncContext(XboxRuntime *p_runtime, const Ref<XboxPendingSignal> &p_pending_signal) :
            XboxSignalXAsyncContext(p_runtime, p_pending_signal) {}
};

class PlayerPickerAsyncContext final : public XboxSignalXAsyncContext {
protected:
    void finalize(XAsyncBlock *p_async_block) override {
        Ref<XboxResult> result;

        if (get_runtime()->is_shutting_down() || get_pending_signal()->was_cancel_requested()) {
            result = XboxResult::cancelled("Player picker UI cancelled.");
            get_pending_signal()->complete(result);
            return;
        }
...
public:
    PlayerPickerAsyncContext(XboxRuntime *p_runtime, const Ref<XboxPendingSignal> &p_pending_signal) :
            XboxSignalXAsyncContext(p_runtime, p_pending_signal) {}
};

class AchievementsUiAsyncContext final : public XboxSignalXAsyncContext {
protected:
    void finalize(XAsyncBlock *p_async_block) override {
        if (get_runtime()->is_shutting_down() || get_pending_signal()->was_cancel_requested()) {
            get_pending_signal()->complete(XboxResult::cancelled("Achievements UI cancelled."));
            return;
        }
...
public:
    AchievementsUiAsyncContext(XboxRuntime *p_runtime, const Ref<XboxPendingSignal> &p_pending_signal) :
            XboxSignalXAsyncContext(p_runtime, p_pending_signal) {}
};

class ErrorDialogAsyncContext final : public XboxSignalXAsyncContext {
protected:
    void finalize(XAsyncBlock *p_async_block) override {
        if (get_runtime()->is_shutting_down() || get_pending_signal()->was_cancel_requested()) {
            get_pending_signal()->complete(XboxResult::cancelled("Error dialog cancelled."));
            return;
        }
...
public:
    ErrorDialogAsyncContext(XboxRuntime *p_runtime, const Ref<XboxPendingSignal> &p_pending_signal) :
            XboxSignalXAsyncContext(p_runtime, p_pending_signal) {}
};

class SendGameInviteAsyncContext final : public XboxSignalXAsyncContext {
protected:
    void finalize(XAsyncBlock *p_async_block) override {
        if (get_runtime()->is_shutting_down() || get_pending_signal()->was_cancel_requested()) {
            get_pending_signal()->complete(XboxResult::cancelled("Send game invite UI cancelled."));
            return;
        }
...
public:
    SendGameInviteAsyncContext(XboxRuntime *p_runtime, const Ref<XboxPendingSignal> &p_pending_signal) :
            XboxSignalXAsyncContext(p_runtime, p_pending_signal) {}
};

class TextEntryAsyncContext final : public XboxSignalXAsyncContext {
protected:
    void finalize(XAsyncBlock *p_async_block) override {
        if (get_runtime()->is_shutting_down() || get_pending_signal()->was_cancel_requested()) {
            get_pending_signal()->complete(XboxResult::cancelled("Text entry UI cancelled."));
            return;
        }
...
public:
    TextEntryAsyncContext(XboxRuntime *p_runtime, const Ref<XboxPendingSignal> &p_pending_signal) :
            XboxSignalXAsyncContext(p_runtime, p_pending_signal) {}
};
...
void XboxGameUI::_bind_methods() {
    ClassDB::bind_method(
            ...
            &XboxGameUI::show_message_dialog_async,
            DEFVAL(String("OK")),
            DEFVAL(String()),
            DEFVAL(String()),
            DEFVAL(String("first")),
            DEFVAL(String("first")));
    ClassDB::bind_method(D_METHOD("set_notification_position_hint", "position"), &XboxGameUI::set_notification_position_hint);
    ClassDB::bind_method(D_METHOD("show_player_profile_card_async", "requesting_user", "target_xuid"), &XboxGameUI::show_player_profile_card_async);
    ClassDB::bind_method(
            ...
            &XboxGameUI::show_player_picker_async,
            DEFVAL(PackedStringArray()),
            DEFVAL(static_cast<int64_t>(1)),
            DEFVAL(static_cast<int64_t>(1)));
    ClassDB::bind_method(D_METHOD("resolve_privilege_with_ui_async", "user", "privilege"), &XboxGameUI::resolve_privilege_with_ui_async);
    ClassDB::bind_method(D_METHOD("show_achievements_async", "requesting_user"), &XboxGameUI::show_achievements_async);
    ClassDB::bind_method(D_METHOD("show_error_dialog_async", "error_code", "context"), &XboxGameUI::show_error_dialog_async, DEFVAL(String()));
    ClassDB::bind_method(
...
void XboxGameUI::shutdown() {
    m_runtime_ready = false;
}

Signal XboxGameUI::show_message_dialog_async(
...
Ref<XboxResult> XboxGameUI::set_notification_position_hint(const String &p_position) {
...
Signal XboxGameUI::show_player_profile_card_async(const Ref<XboxUser> &p_requesting_user, const String &p_target_xuid) {
    ...
    if (!p_requesting_user.is_valid() || p_requesting_user->get_handle() == nullptr) {
        return make_error_signal_internal(E_INVALIDARG, "invalid_user", "A signed-in XboxUser is required to show profile card UI.");
    }
...
Signal XboxGameUI::show_player_picker_async(
        const Ref<XboxUser> &p_requesting_user,
    ...
    Ref<XboxResult> selectable_parse_result = _parse_xuid_list(
            p_selectable_xuids,
            &selectable_xuids,
            nullptr,
            "missing_xuids",
    ...
    if (!p_preselected_xuids.is_empty()) {
        Ref<XboxResult> preselected_parse_result = _parse_xuid_list(
                p_preselected_xuids,
                &preselected_xuids,
                nullptr,
    ...
    if (!p_requesting_user.is_valid() || p_requesting_user->get_handle() == nullptr) {
        return make_error_signal_internal(E_INVALIDARG, "invalid_user", "A signed-in XboxUser is required to show player picker UI.");
    }
...
Signal XboxGameUI::resolve_privilege_with_ui_async(const Ref<XboxUser> &p_user, int64_t p_privilege) {
    XboxRuntime *runtime = get_runtime_internal();
    if (runtime == nullptr || !runtime->is_initialized() || !m_runtime_ready) {
        return make_error_signal_internal(E_FAIL, "not_initialized", "GDK is not initialized. Call GDK.initialize() first.");
    }
    if (!p_user.is_valid() || p_user->get_handle() == nullptr) {
        return make_error_signal_internal(E_INVALIDARG, "invalid_user", "A signed-in XboxUser is required to resolve privilege UI.");
    }

    XboxUsers *users = get_users_internal();
    if (users == nullptr) {
...
Signal XboxGameUI::show_achievements_async(const Ref<XboxUser> &p_requesting_user) {
    XboxRuntime *runtime = get_runtime_internal();
    if (runtime == nullptr || !runtime->is_initialized() || !m_runtime_ready) {
        return make_error_signal_internal(E_FAIL, "not_initialized", "GDK is not initialized. Call GDK.initialize() first.");
    }
    if (!p_requesting_user.is_valid() || p_requesting_user->get_handle() == nullptr) {
        return make_error_signal_internal(E_INVALIDARG, "invalid_user", "A signed-in XboxUser is required to show achievements UI.");
    }
...
Signal XboxGameUI::show_error_dialog_async(int64_t p_error_code, const String &p_context) {
    XboxRuntime *runtime = get_runtime_internal();
    if (runtime == nullptr || !runtime->is_initialized() || !m_runtime_ready) {
        return make_error_signal_internal(E_FAIL, "not_initialized", "GDK is not initialized. Call GDK.initialize() first.");
    }
...
Signal XboxGameUI::show_send_game_invite_async(
        const Ref<XboxUser> &p_requesting_user,
    ...
    if (!p_requesting_user.is_valid() || p_requesting_user->get_handle() == nullptr) {
        return make_error_signal_internal(E_INVALIDARG, "invalid_user", "A signed-in XboxUser is required to send a game invite.");
    }
...
Signal XboxGameUI::show_text_entry_async(
...
XboxUsers *XboxGameUI::get_users_internal() const {
    if (m_owner == nullptr) {
        return nullptr;
    }
...
Signal XboxGameUI::make_completed_signal_internal(const Ref<XboxResult> &p_result) const {
    XboxRuntime *runtime = get_runtime_internal();
...
Signal XboxGameUI::make_error_signal_internal(
        HRESULT p_hresult,
        const String &p_code,
