#ifndef XBOX_PRESENCE_H
#define XBOX_PRESENCE_H
...
class XboxPresenceRecord : public RefCounted {
    GDCLASS(XboxPresenceRecord, RefCounted);
...
class XboxPresence : public RefCounted {
    GDCLASS(XboxPresence, RefCounted);

    Xbox *m_owner = nullptr;
    bool m_runtime_ready = false;
    std::vector<Ref<XboxPresenceRecord>> m_cached_presence;

    struct HandlerState {
        struct CallbackContext {
            XboxPresence *presence = nullptr;
    ...
    Ref<XboxUser> _get_presence_calling_user() const;
    Ref<XboxResult> _parse_query_xuids(const PackedStringArray &p_xuids, std::vector<uint64_t> *r_xuids) const;
    Ref<XboxResult> _parse_xuids(const PackedStringArray &p_xuids, bool p_allow_empty, const String &p_missing_code, const String &p_invalid_code, std::vector<uint64_t> *r_xuids) const;
    Ref<XboxResult> _parse_title_ids(const PackedInt64Array &p_title_ids, std::vector<uint32_t> *r_title_ids) const;
    Ref<XboxPresenceRecord> _find_cached_presence(const String &p_xuid) const;
    void _remove_cached_presence(const String &p_xuid);
    HandlerState *_find_handler_state(XUserLocalId p_local_id);
    Ref<XboxResult> _ensure_handler_state(const Ref<XboxUser> &p_user, HandlerState **r_state);
...
public:
    ...
    Signal set_presence_async(const Ref<XboxUser> &p_user, const String &p_state, const Dictionary &p_rich_presence = Dictionary());
    Signal clear_presence_async(const Ref<XboxUser> &p_user);
    Signal get_presence_async(const PackedStringArray &p_xuids);
    Signal get_presence_for_social_group_async(const Ref<XboxUser> &p_user, const String &p_social_group);
    Ref<XboxResult> track_presence(const Ref<XboxUser> &p_user, const PackedStringArray &p_xuids, const PackedInt64Array &p_title_ids = PackedInt64Array());
    Ref<XboxResult> stop_tracking_presence(const Ref<XboxUser> &p_user, const PackedStringArray &p_xuids = PackedStringArray(), const PackedInt64Array &p_title_ids = PackedInt64Array());
    Ref<XboxPresenceRecord> get_cached_presence(const String &p_xuid) const;

    void cache_presence_record(const Ref<XboxPresenceRecord> &p_record, bool p_emit_signal = true);
    void on_user_removed(const Ref<XboxUser> &p_user);
};

} // namespace godot

VARIANT_ENUM_CAST(godot::XboxPresenceRecord::UserState);

#endif // XBOX_PRESENCE_H
