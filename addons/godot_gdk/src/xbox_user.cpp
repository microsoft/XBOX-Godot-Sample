...
XboxUser::SignInState _user_state_to_sign_in_state(XUserState p_user_state) {
    switch (p_user_state) {
        case XUserState::SignedIn:
            return XboxUser::SIGN_IN_STATE_SIGNED_IN;
        case XUserState::SigningOut:
            return XboxUser::SIGN_IN_STATE_SIGNING_OUT;
        case XUserState::SignedOut:
        default:
            return XboxUser::SIGN_IN_STATE_SIGNED_OUT;
    }
}

String _sign_in_state_to_name(XboxUser::SignInState p_sign_in_state) {
    switch (p_sign_in_state) {
        case XboxUser::SIGN_IN_STATE_SIGNED_IN:
            return "signed_in";
        case XboxUser::SIGN_IN_STATE_SIGNING_OUT:
            return "signing_out";
        case XboxUser::SIGN_IN_STATE_SIGNED_OUT:
        default:
            return "signed_out";
    }
}
...
XboxUser::AgeGroup _age_group_to_enum(XUserAgeGroup p_age_group) {
    switch (p_age_group) {
        case XUserAgeGroup::Child:
            return XboxUser::AGE_GROUP_CHILD;
        case XUserAgeGroup::Teen:
            return XboxUser::AGE_GROUP_TEEN;
        case XUserAgeGroup::Adult:
            return XboxUser::AGE_GROUP_ADULT;
        case XUserAgeGroup::Unknown:
        default:
            return XboxUser::AGE_GROUP_UNKNOWN;
    }
}

String _age_group_to_name(XboxUser::AgeGroup p_age_group) {
    switch (p_age_group) {
        case XboxUser::AGE_GROUP_CHILD:
            return "child";
        case XboxUser::AGE_GROUP_TEEN:
            return "teen";
        case XboxUser::AGE_GROUP_ADULT:
            return "adult";
        case XboxUser::AGE_GROUP_UNKNOWN:
        default:
            return "unknown";
    }
}
...
protected:
    void finalize(XAsyncBlock *p_async_block) override {
        ...
        if (FAILED(result_hr)) {
            result = XboxResult::hresult_error(result_hr, m_action, "user_add_result_failed");
            get_pending_signal()->complete(result);
            return;
        }
...
protected:
    void finalize(XAsyncBlock *p_async_block) override {
        ...
        if (get_runtime()->is_shutting_down() || get_pending_signal()->was_cancel_requested()) {
            result = XboxResult::cancelled("Privilege resolution cancelled.");
            get_pending_signal()->complete(result);
            return;
        }
        ...
        get_pending_signal()->complete(XboxResult::ok_result(_make_privilege_resolution_result(m_privilege)));
    }

public:
    ResolvePrivilegeAsyncContext(XboxUsers *p_users, const Ref<XboxUser> &p_user, XboxRuntime *p_runtime, const Ref<XboxPendingSignal> &p_pending_signal, int64_t p_privilege) :
            XboxSignalXAsyncContext(p_runtime, p_pending_signal),
            m_users(p_users),
            m_user(p_user),
...
class ResolveIssueAsyncContext final : public XboxSignalXAsyncContext {
    XboxUsers *m_users = nullptr;
    Ref<XboxUser> m_user;
    String m_url;
...
protected:
    void finalize(XAsyncBlock *p_async_block) override {
        ...
        if (get_runtime()->is_shutting_down() || get_pending_signal()->was_cancel_requested()) {
            result = XboxResult::cancelled("User issue resolution cancelled.");
            get_pending_signal()->complete(result);
            return;
        }
        ...
        get_pending_signal()->complete(XboxResult::ok_result(_make_issue_resolution_result(m_url)));
    }

public:
    ResolveIssueAsyncContext(XboxUsers *p_users, const Ref<XboxUser> &p_user, XboxRuntime *p_runtime, const Ref<XboxPendingSignal> &p_pending_signal, const String &p_url) :
            XboxSignalXAsyncContext(p_runtime, p_pending_signal),
            m_users(p_users),
            m_user(p_user),
            m_url(p_url) {
...
class GamerPictureAsyncContext final : public XboxSignalXAsyncContext {
    Ref<XboxUser> m_user;
...
protected:
    void finalize(XAsyncBlock *p_async_block) override {
        ...
        if (get_runtime()->is_shutting_down() || get_pending_signal()->was_cancel_requested()) {
            result = XboxResult::cancelled("Gamer picture request cancelled.");
            get_pending_signal()->complete(result);
            return;
        }
        ...
        if (png_bytes.resize(static_cast<int64_t>(buffer_used)) != 0) {
            result = XboxResult::error_result(E_OUTOFMEMORY, "gamer_picture_buffer_alloc_failed", "Failed to allocate a buffer for the gamer picture.");
            get_pending_signal()->complete(result);
            return;
        }
        if (buffer_used > 0) {
        ...
        if (image->load_png_from_buffer(png_bytes) != 0) {
            result = XboxResult::error_result(E_FAIL, "gamer_picture_decode_failed", "Failed to decode the gamer picture PNG into a Godot Image.");
            get_pending_signal()->complete(result);
            return;
        }

        get_pending_signal()->complete(XboxResult::ok_result(image));
    }

public:
    GamerPictureAsyncContext(const Ref<XboxUser> &p_user, XboxRuntime *p_runtime, const Ref<XboxPendingSignal> &p_pending_signal) :
            XboxSignalXAsyncContext(p_runtime, p_pending_signal),
            m_user(p_user) {}
};

class TokenAndSignatureAsyncContext final : public XboxSignalXAsyncContext {
    Ref<XboxUser> m_user;
...
protected:
    void finalize(XAsyncBlock *p_async_block) override {
        ...
        if (get_runtime()->is_shutting_down() || get_pending_signal()->was_cancel_requested()) {
            result = XboxResult::cancelled("Token and signature request cancelled.");
            get_pending_signal()->complete(result);
            return;
        }
...
void XboxUser::_bind_methods() {
    ...
    ClassDB::bind_method(D_METHOD("get_sign_in_state_name"), &XboxUser::get_sign_in_state_name);
...
int64_t XboxUser::get_local_id() const {
    return static_cast<int64_t>(m_local_id.value);
}

String XboxUser::get_xuid() const {
    return m_xuid;
}
...
String XboxUser::get_age_group_name() const {
    return _age_group_to_name(m_age_group);
}

XboxUser::SignInState XboxUser::get_sign_in_state() const {
    return m_sign_in_state;
}

String XboxUser::get_sign_in_state_name() const {
    return _sign_in_state_to_name(m_sign_in_state);
}

bool XboxUser::is_guest() const {
    return m_is_guest;
}

bool XboxUser::is_signed_in() const {
    return m_is_signed_in;
}

bool XboxUser::is_store_user() const {
...
HRESULT XboxUser::adopt_handle(XUserHandle p_user_handle) {
    clear();
...
bool XboxUser::matches_local_id(XUserLocalId p_user_local_id) const {
...
void XboxUsers::_bind_methods() {
    ...
    ClassDB::bind_method(D_METHOD("get_gamer_picture_async", "user", "size"), &XboxUsers::get_gamer_picture_async, DEFVAL(String("medium")));
    ClassDB::bind_method(
            D_METHOD("get_token_and_signature_async", "user", "method", "url", "headers", "body", "force_refresh"),
            &XboxUsers::get_token_and_signature_async,
            DEFVAL(Dictionary()),
...
Ref<XboxResult> XboxUsers::on_runtime_initialized() {
...
void XboxUsers::shutdown() {
    m_runtime_ready = false;

    if (m_change_event_registered) {
        // wait=true matches the pattern in xbox_activation.cpp and
        // xbox_multiplayer_activity.cpp: block until any in-flight change
        ...
        // xbox_runtime.cpp); making the change-event unregister synchronous
        // keeps a worker-thread callback from ever dereferencing an XboxUsers
...
Signal XboxUsers::add_user_with_ui_async(bool p_allow_guests) {
    ...
    // it to a cancelled XboxResult. Whether dismissing the system UI fires the
...
Ref<XboxUser> XboxUsers::get_primary_user() const {
    return m_primary_user;
}

Array XboxUsers::get_users() const {
    Array users;
    for (const Ref<XboxUser> &user : m_users) {
...
Signal XboxUsers::check_privilege_async(const Ref<XboxUser> &p_user, int64_t p_privilege) {
    ...
    Ref<XboxPendingSignal> pending_signal = runtime->make_pending_signal();
    pending_signal->complete_deferred(XboxResult::ok_result(data));
    return pending_signal->get_completed_signal();
}

Signal XboxUsers::resolve_privilege_with_ui_async(const Ref<XboxUser> &p_user, int64_t p_privilege) {
    XboxRuntime *runtime = _get_runtime();
    if (runtime == nullptr || !runtime->is_initialized()) {
        return _make_users_error_signal(runtime, E_FAIL, "not_initialized", "GDK is not initialized. Call GDK.initialize() first.");
    }
...
Signal XboxUsers::resolve_issue_with_ui_async(const Ref<XboxUser> &p_user, const String &p_url) {
    XboxRuntime *runtime = _get_runtime();
    if (runtime == nullptr || !runtime->is_initialized()) {
        return _make_users_error_signal(runtime, E_FAIL, "not_initialized", "GDK is not initialized. Call GDK.initialize() first.");
    }
...
Signal XboxUsers::get_gamer_picture_async(const Ref<XboxUser> &p_user, const String &p_size) {
    XboxRuntime *runtime = _get_runtime();
    if (runtime == nullptr || !runtime->is_initialized()) {
        return _make_users_error_signal(runtime, E_FAIL, "not_initialized", "GDK is not initialized. Call GDK.initialize() first.");
    }
...
Signal XboxUsers::get_token_and_signature_async(
        const Ref<XboxUser> &p_user,
        const String &p_method,
        const String &p_url,
        const Dictionary &p_headers,
...
void XboxUsers::on_user_change(XUserLocalId p_user_local_id, XUserChangeEvent p_event) {
    XboxRuntime *runtime = _get_runtime();
    if (!m_runtime_ready || runtime == nullptr || runtime->is_shutting_down()) {
        return;
    }

    Ref<XboxUser> user = _find_user_by_local_id(p_user_local_id);
    if (!user.is_valid()) {
        return;
    }
...
void XboxUsers::complete_add_user(XUserHandle p_user_handle, const Ref<XboxPendingSignal> &p_pending_signal) {
    ...
    p_pending_signal->complete(XboxResult::ok_result(user));
}
...
Signal XboxUsers::_start_add_user_async(XUserAddOptions p_options, const String &p_action) {
    XboxRuntime *runtime = _get_runtime();
    if (runtime == nullptr || !runtime->is_initialized()) {
        return _make_users_error_signal(runtime, E_FAIL, "not_initialized", "GDK is not initialized. Call GDK.initialize() first.");
    }
    ...
    if (FAILED(hr)) {
        ...
        Ref<XboxResult> result = XboxResult::hresult_error(hr, p_action, "user_add_start_failed");
        pending_signal->complete_deferred(result);
    }
...
bool XboxUsers::_add_or_update_user(const Ref<XboxUser> &p_user) {
    for (Ref<XboxUser> &existing : m_users) {
...
Ref<XboxUser> XboxUsers::_find_user_by_local_id(XUserLocalId p_user_local_id) const {
    for (const Ref<XboxUser> &user : m_users) {
...
void XboxUsers::_remove_user_by_local_id(XUserLocalId p_user_local_id) {
    m_users.erase(
            std::remove_if(
                    m_users.begin(),
                    m_users.end(),
                    [p_user_local_id](const Ref<XboxUser> &user) {
