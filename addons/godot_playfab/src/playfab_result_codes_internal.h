// Pure HRESULT formatting helpers extracted from playfab_result.cpp.
//
// These free functions live in the playfab_internal:: namespace and are
...
// This is the public-API-shape helper that PlayFabResult::format_hresult forwards to.
godot::String format_hresult_string(HRESULT p_hresult);

// Build the standard "<action> (HRESULT 0xXXXXXXXX)" message string used by
// PlayFabResult::hresult_error. If action is empty the leading "<action> " is omitted.
godot::String format_hresult_message(const godot::String &p_action, HRESULT p_hresult);

// Fallback resolver for the "code" field on a PlayFabResult: returns p_provided
...
#endif // PLAYFAB_RESULT_CODES_INTERNAL_H
