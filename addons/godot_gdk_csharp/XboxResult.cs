...
/// <c>XboxResult</c>: a success bit, a stable string <see cref="Code"/>, a
/// human-readable <see cref="Message"/>, and a typed <see cref="Data"/> payload.
/// </summary>
public sealed class XboxResult : XboxObject
{
    internal XboxResult(GodotObject o) : base(o)
    {
    }

    public static XboxResult From(GodotObject o) =>
        o == null
            ? Failed("null_native_result",
                "godot_gdk produced no result object (native call returned null; possible method/signature drift).")
            : new XboxResult(o);

    internal static XboxResult Failed(string code, string message)
    {
        ...
        return new XboxResult(null) { _syntheticCode = code, _syntheticMessage = message };
    }
    ...
    public T DataAs<T>() where T : XboxObject => Wrap<T>(DataObject);
}
