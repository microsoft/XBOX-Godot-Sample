...
## source of truth. Tests pin the matrix in `test_editortools_cli.gd`.
...
##   error_code: int           — one of EditorToolsResult.EXIT_*
static func parse(argv: PackedStringArray) -> Dictionary:
	var result: Dictionary = {
		...
		"help": false,
		"error": "",
		"error_code": EditorToolsResult.EXIT_OK,
	}
	...
	if not VERBS.has(verb):
		...
		result["error_code"] = EditorToolsResult.EXIT_USAGE
		return result
	result["verb"] = verb
