...
func launch_detached(exe_path: String, args: PackedStringArray) -> int:
	if not FileAccess.file_exists(exe_path):
		push_error("[GDK Editor Tools] Tool not found: " + exe_path)
		return -1
...
func _try_bin_dir(dir: String) -> void:
	...
	if FileAccess.file_exists(makepkg) and FileAccess.file_exists(config_editor):
		...
		_is_available = true
		print("[GDK Editor Tools] GDK tools found at: ", dir)
	else:
		if not FileAccess.file_exists(makepkg):
			push_warning("[GDK Editor Tools] makepkg.exe not found in: " + dir)
		if not FileAccess.file_exists(config_editor):
			push_warning("[GDK Editor Tools] GameConfigEditor.exe not found in: " + dir)
