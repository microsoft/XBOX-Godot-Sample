...
##     [code]editortools_service.gd[/code]; called inline by the editor tabs.
...
func dispose() -> void:
	...
	if _thread != null and _thread.is_started():
		push_warning("[GDK Editor Tools] Waiting for in-flight wdapp operation to finish before shutdown...")
...
func _start_async(target: Callable, args: Array) -> bool:
	...
	if err != OK:
		push_error("[GDK Editor Tools] Thread.start failed for wdapp async dispatch (err %d)" % err)
		return false
	_thread = thread
	_busy = true
	return true
