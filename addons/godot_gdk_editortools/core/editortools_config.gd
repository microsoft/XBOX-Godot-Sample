...
## flat dictionary the `EditorToolsService` can consume.
##
## Precedence (highest wins):
##   1. CLI flags                (from editortools_cli.gd → options)
##   2. res://.gdk_packaging.cfg (via EditorToolsSettingsStore)
##   3. MicrosoftGame.config     (identity / product fields)
##   4. project.godot            (only the app_name / app_version pair)
##   5. Built-in defaults         (EditorToolsSettingsStore.get_default_state)
...
static func resolve(
	...
	if not settings_path.is_empty():
		var store: RefCounted = EditorToolsSettingsStoreScript.new()
