...
##
## The runner (`addons/godot_gdk_editortools/run.gd`) instantiates this class
## with a freshly resolved config dict from `editortools_config.gd`, then
## dispatches to the right `run_*` method. Every verb returns a
## `EditorToolsResult` dictionary built by `editortools_result.gd`.
##
...
const WdappManagerScript = preload("res://addons/godot_gdk_editortools/core/wdapp_manager.gd")
...
func _init(toolchain: RefCounted = null) -> void:
	...
	_preparer = EditorToolsContentPreparerScript.new(_config_mgr)
...
func run_pack(resolved: Dictionary) -> Dictionary:
	...
	if not missing.is_empty():
		return EditorToolsResult.fail(verb, missing, EditorToolsResult.EXIT_USAGE)
	if _encrypt_key_missing(resolved):
		var message: String = "--encrypt=key requires --encrypt-key to point at an EKB file"
		push_error("[GDK Editor Tools] " + message)
		return EditorToolsResult.fail(verb, message, EditorToolsResult.EXIT_CONFIG)
	if not _toolchain.is_gdk_available():
		return EditorToolsResult.fail(verb, "GDK tools not found (set GDK_BIN or install the GDK)",
	...
	if not DirAccess.dir_exists_absolute(source_dir):
		return EditorToolsResult.fail(verb, "Source directory does not exist: %s" % source_dir,
			EditorToolsResult.EXIT_CONFIG)
	...
	if not skip_prepare:
		...
		if not ok:
			return EditorToolsResult.fail(verb,
	...
	if map_file.is_empty():
		...
		if int(genmap_result.get("exit_code", -1)) != 0:
			return EditorToolsResult.fail(verb, "Auto-genmap failed",
				EditorToolsResult.EXIT_TOOL,
...
func run_genmap(resolved: Dictionary) -> Dictionary:
	...
	if not missing.is_empty():
		return EditorToolsResult.fail(verb, missing, EditorToolsResult.EXIT_USAGE)
	if not _toolchain.is_gdk_available():
		return EditorToolsResult.fail(verb, "GDK tools not found",
			EditorToolsResult.EXIT_CONFIG)
	var source_dir: String = str(resolved["source_dir"])
	...
	var duration: int = Time.get_ticks_msec() - t0
	if exit_code != 0:
		return EditorToolsResult.fail(verb, "makepkg genmap failed (exit %d)" % exit_code,
			EditorToolsResult.EXIT_TOOL,
...
func run_prepare_content(resolved: Dictionary) -> Dictionary:
	...
	if not DirAccess.dir_exists_absolute(content_dir):
		return EditorToolsResult.fail(verb, "Content directory does not exist: %s" % content_dir,
			EditorToolsResult.EXIT_CONFIG)
	var prep_logs: Array[String] = []
	...
	var duration: int = Time.get_ticks_msec() - t0
	if not ok:
		return EditorToolsResult.fail(verb, _content_prep_error("Content prep failed", prep_logs),
			EditorToolsResult.EXIT_TOOL,
			"", {"content_dir": content_dir, "logs": prep_logs}, "", duration)
	return EditorToolsResult.ok(verb, "Prepared %s" % content_dir,
...
func run_export(resolved: Dictionary) -> Dictionary:
	...
	if godot_exe.is_empty():
		return EditorToolsResult.fail(verb, "Could not determine current Godot executable path",
			EditorToolsResult.EXIT_CONFIG)
...
func run_register_loose(resolved: Dictionary) -> Dictionary:
	...
	if not _get_wdapp().is_available():
		return EditorToolsResult.fail(verb, "wdapp.exe not found", EditorToolsResult.EXIT_CONFIG)
	var content_dir: String = str(resolved["content_dir"])
	if not DirAccess.dir_exists_absolute(content_dir):
		return EditorToolsResult.fail(verb, "Content directory does not exist: %s" % content_dir,
			EditorToolsResult.EXIT_CONFIG)
	...
	if exit_code != 0:
		return EditorToolsResult.fail(verb,
			...
			str(result.get("stdout", "")), duration)
	return EditorToolsResult.ok(verb, "Registered loose-files build at %s" % content_dir,
		{"content_dir": content_dir},
		str(result.get("stdout", "")), duration)
...
func run_uninstall(resolved: Dictionary) -> Dictionary:
	...
	if exit_code != 0:
		return EditorToolsResult.fail(verb,
			...
			str(result.get("stdout", "")), duration)
	return EditorToolsResult.ok(verb, "Uninstalled %s" % package_name,
		{"package_name": package_name},
		str(result.get("stdout", "")), duration)
...
func run_launch(resolved: Dictionary) -> Dictionary:
	...
	if exit_code != 0:
		return EditorToolsResult.fail(verb,
			...
			{"aumid": aumid},
			str(result.get("stdout", "")), duration)
	return EditorToolsResult.ok(verb, "Launched %s" % aumid,
...
func run_sandbox(resolved: Dictionary) -> Dictionary:
	...
	if sandbox_exe.is_empty():
		return EditorToolsResult.fail(verb, "XblPCSandbox.exe not found",
			EditorToolsResult.EXIT_CONFIG)
...
func run_config_template(resolved: Dictionary) -> Dictionary:
	...
	return EditorToolsResult.ok(verb, "Created template at %s" % fs_output,
...
func run_config_editor(resolved: Dictionary) -> Dictionary:
	...
	if editor_exe.is_empty():
		return EditorToolsResult.fail(verb, "GameConfigEditor.exe not found",
			EditorToolsResult.EXIT_CONFIG)
	...
	var duration: int = Time.get_ticks_msec() - t0
	if pid < 0:
		return EditorToolsResult.fail(verb, "Failed to launch GameConfigEditor",
			EditorToolsResult.EXIT_TOOL,
			"", {"editor_exe": editor_exe, "config_path": config_path}, "", duration)
	return EditorToolsResult.ok(verb, "Launched GameConfigEditor (pid %d)" % pid,
...
## Returns a [code]EditorToolsResult[/code] regardless of outcome.
func dispatch(verb: String, resolved: Dictionary) -> Dictionary:
	var method: String = method_for_verb(verb)
	if method.is_empty():
		return EditorToolsResult.fail(verb, "Verb not implemented: %s" % verb,
			EditorToolsResult.EXIT_UNIMPLEMENTED)
