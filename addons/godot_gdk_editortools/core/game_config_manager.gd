...
func parse_config(config_path: String = "") -> Dictionary:
	...
	if err != OK:
		push_error("[GDK Editor Tools] Failed to open MicrosoftGame.config: " + error_string(err))
		return {}
...
func create_template(game_name: String = "MyGodotGame",
	...
	if FileAccess.file_exists(fs_target):
		push_warning("[GDK Editor Tools] MicrosoftGame.config already exists — not overwriting.")
	...
	if file == null:
		push_error("[GDK Editor Tools] Failed to create MicrosoftGame.config: " + error_string(FileAccess.get_open_error()))
		return FileAccess.get_open_error()

	file.store_string(xml)
	file.close()
	print("[GDK Editor Tools] Created template MicrosoftGame.config at: ", fs_target)
...
func _ensure_placeholder_images(base_dir: String = "") -> void:
	...
	if not FileAccess.file_exists(default_png):
		push_warning("[GDK Editor Tools] Default PNG not found at: " + default_png)
		return
	...
	if err != OK:
		push_warning("[GDK Editor Tools] Failed to load default PNG: " + error_string(err))
		return
	...
	for filename: String in targets:
		...
		if err == OK:
			print("[GDK Editor Tools] Created placeholder: ", filename)
		else:
			push_warning("[GDK Editor Tools] Failed to create " + filename + ": " + error_string(err))
...
func sync_store_logos() -> int:
	...
	if not FileAccess.file_exists(master_path):
		push_warning("[GDK Editor Tools] 480x480 master logo not found at: " + master_path)
		return 0

	var source_image: Image = Image.new()
	var err: Error = source_image.load(master_path)
	if err != OK:
		push_warning("[GDK Editor Tools] Failed to load 480x480 master logo: " + error_string(err))
		return 0
	...
	for key: String in variants:
		...
		if err == OK:
			updated += 1
			print("[GDK Editor Tools] Synced logo: ", rel)
		else:
			push_warning("[GDK Editor Tools] Failed to sync " + rel + ": " + error_string(err))
...
func launch_editor() -> int:
	...
	if not FileAccess.file_exists(config_path):
		push_error("[GDK Editor Tools] MicrosoftGame.config not found — create one first.")
		return -1
	...
	if pid >= 0:
		print("[GDK Editor Tools] Launched GameConfigEditor (PID: ", pid, ")")
	return pid
